...
class smarty_care extends Smarty {
	...
	*/
	...
	function InitializeGUI(){
		...
		$this->assign('getShift',$this->getNurseShift());
		...
		# By default the window title is Care2x
		 ...
		 /*CLASS ASSIGNMENT
		   assigning a class for printRounds Button, for styling and javascript.
		   PETER ANTHONY DUOT
  		   MARCH 30, 2017
		 */
		...
		if ($QuickMenu) {
			$link = 1;
			$eclaims = $_GET['eclaims'];
			if ($link == $eclaims) {
				$newBreak = '../../index.php?r=eclaims/transmittal';
				#$newBreak = $breakfile;
			}else{
				$newBreak =$breakfile;
			} 
	...
	function getNurseShift(){
		global $db;
		$option = "";
		$sql = "SELECT sns.shift_code,sns.shift_time_from , sns.shift_time_to FROM seg_nursing_shift sns
		WHERE sns.shift_time_status NOT IN ('inactive','deleted')";

				if($result=$db->Execute($sql)) {
			while ($row = $result->FetchRow())
		
			$option.= '<option value='.$row['shift_code'].'>'.$row['shift_time_from'].''.'-'.$row['shift_time_to'].'</option>';
			return $option;
		} else { return false; }
