			...
			if (details) {
				...
				if (items) {
					if ($('row'+id)) {
						...
						xajax_isSpongeType(id,parseFloatEx(details.qty),'add'); //added by angelo m. 09.09.2010
						return true
					}
...
function removeItemSupplies(id) {
	...
	refreshTotalSupplies();
		...
		refreshAnesthesiaTable(id); //added by CHa, Jan 7, 2010
}

function removeSupplies(id) {
	var destTable, destRows;
	var table = $('supplies-list');
	var rmvRow=$('row'+id);
	if (table && rmvRow) {
...
//			adjust_sponge_quantity(id, parseFloatEx(obj.value));
		...
		//added by angelo m. 09.09.2010
		xajax_isSpongeType(id,parseFloatEx(obj.value),'replace');
		//if (classification=='Sponge') {
//			adjust_sponge_quantity(id, parseFloatEx(obj.value));
//		}
	//obj.value = formatNumber(obj.value,2);
