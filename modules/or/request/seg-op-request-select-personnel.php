...
#echo "seg-op-request-select_doctor.php : before : popUp ='".$popUp."; _GET['popUp'] = '".$_GET['popUp']."'; _POST['popUp'] = '".$_POST['popUp']."' <br> \n";
	...
	if (!isset($personnel_type) || !$personnel_type){
		if (isset($_GET['personnel_type']) && $_GET['personnel_type']){
			$personnel_type = $_GET['personnel_type'];
		}
		if (isset($_POST['personnel_type']) && $_POST['personnel_type']){
	...
	if (!isset($dept_nr) || !$dept_nr){
		...
		if (isset($_POST['dept_nr']) && $_POST['dept_nr']){
			$dept_nr = $_POST['dept_nr'];
		}
	}
/*
echo "seg-op-request-select-personnel.php : _GET : <br> \n"; print_r($_GET); echo " <br> \n";

#echo "seg-op-request-select-personnel.php : after : popUp ='".$popUp."; _GET['popUp'] = '".$_GET['popUp']."'; _POST['popUp'] = '".$_POST['popUp']."' <br> \n";
echo "seg-op-request-select-personnel.php : personnel_type ='".$personnel_type."' <br> \n";
echo "seg-op-request-select-personnel.php : list ='".$list."' <br> \n";
*/

#echo "dept_nr= ".$dept_nr;
$title=$LDOpPersonElements[$personnel_type];
...
switch($personnel_type)
{
	case 'surgeon':
							...
							//$maxelement=10;
							$quickid='doctor';
							#$quicklist=$pers_obj->getDoctorsOfDept($dept_nr, 7);
														$quicklist = $pers_obj->getDoctorsOfDept(117);    #Added by Cherry 05-19-10
														#echo "sql= ".$pers_obj->sql;
							break;
	case 'surgeon_assist':
							$element='assistant';
							$pers_type = 'Assisting Surgeon';
							//$maxelement=10;
							$quickid='doctor';
							#$quicklist=$pers_obj->getDoctorsOfDept($dept_nr, 8);
														$quicklist = $pers_obj->getDoctorsOfDept(117);   #Added by Cherry 05-19-10
							break;
	case 'nurse_scrub':
							$element='scrub_nurse';
							$pers_type = 'Scrub Nurse';
							//$maxelement=10;
							$quickid='nurse';
							$quicklist=$pers_obj->getNursesOfDept($dept_nr, 9);
							break;
	case 'nurse_rotating':
							$element='rotating_nurse';
							$pers_type = 'Circulating Nurse';
							//$maxelement=10;
							$quickid='nurse';
							$quicklist=$pers_obj->getNursesOfDept($dept_nr, 10);
							break;
	case 'anesthesiologist':
							$element='an_doctor';
							$pers_type = 'Anesthesiologist';
							//$maxelement=10;
							$quickid='doctor';
...
else   #---added by CHA, March 18, 2010---
{
	$list_personnel = '<tr>'.
	'<td colspan="5">No available personnel...</td>';
	'</tr>';
} #---end Cha---
...
# $smarty->assign('sToolbarTitle',"$LDRadiology::$LDDiagnosticTest");
 $smarty->assign('sToolbarTitle',"Operating Room :: Request :: ".$pers_type);
...
#	$smarty->assign('breakfile',$breakfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Operating Room :: Request :: ".$pers_type);
...
<script type="text/javascript" language="javascript">
	...
	function prepareSelect(nr){
		...
		//added by angelo m. 09.03.2010
		//start
		var surgeonId="rowID"+nr;
		if(window.parent.document.getElementById(surgeonId)!=null){
			var name=$('personnel'+nr).value;
		...
		//end
			...
			//alert("prepareSelect :: \n"+msg);
				...
				//result = window.parent.appendPersonnel(window.parent.$($F('table_name')),$F('personnel_type')+'[]',details);
						...
						var table_name = $F('table_name');
						var list = window.parent.document.getElementById(table_name);
					result = window.parent.appendPersonnel(list,$F('personnel_type')+'[]',details);
		}
