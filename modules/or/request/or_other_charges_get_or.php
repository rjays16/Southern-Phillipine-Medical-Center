...
$javascript_array = array('<script type="text/javascript" src="'.$root_path.'modules/or/js/flexigrid/lib/jquery/jquery.js"></script>'
													...
													, '<script type="text/javascript" src="'.$root_path.'js/gen_routines.js"></script> '	#--added by CHA, April 10,2010 *for billing
...
$is_lab = isset($_POST['from_lab']) ? $_POST['from_lab'] : $_GET['from_lab']; 
#$smarty->assign('is_laboratory','<input type="hidden" id="from_lab" value="'.$is_lab.'"/>');
#$smarty->assign('sOnLoadJs','onLoad="preSet();"'); 
#--end CHA

global $db;
#---added by CHA, April 16,2010---
$ortype_query = "select som.or_type from seg_or_main as som left join seg_ops_serv as sos".
							" on som.ceo_refno=sos.refno where sos.encounter_nr=".$db->qstr($encounter_nr);
$ortype = $db->GetOne($ortype_query);
if($ortype=="OR" || $ortype=="PACU")
{
	$orarea = "ORPACU";
}
else if($ortype="DR" || $ortype=="NICU")
{
	$orarea = "DRNICU";
}
...
$area_query = "select area_code, area_name from seg_areas where has_stocks='1' order by area_name";
$area_result = $db->Execute($area_query);
$charge_area = "";
while($row=$area_result->FetchRow())
{
	 if($row["area_code"]==$orarea)
	 {
		 $charge_area.="<option value='".$row["area_code"]."' selected=''>".$row["area_name"]."</option>";
	 }
}

#added by CHA, May 11,2010
if($is_lab){
	$row = $db->GetRow("select area_code, area_name from seg_areas where area_code='LAB'");
	$charge_area="<option value='".$row["area_code"]."' selected=''>".$row["area_name"]."</option>";
}
#--------------------------

/*$or_type = $seg_ops->get_encounter_or_type($encounter_nr);
echo "<br>or_type:".$or_type;
...
if (isset($_POST['submitted'])) {
 ...
 //start saving of medicine and supplies
...
/*	echo '<pre>';
	...
	if ($saveok) {
		...
		if ($saveok) {  //see maramag for sponge thingy
			...
			if (count($_POST['items']) > 0) {
				...
				$saveok = $seg_order->changeServeStatus($order_refno, $item_array, $prc_array, $prc_array, $status_array, $remarks_array);
			}
		} 
 ...
 //echo "<br>saveok_cnt=".$saveok_cnt;
//start saving of equipments
...
//start saving miscellaneous
	...
	if(count($_POST['misc'])==0)  
	 {
		 $no_items=1;
		 //echo "no_items=".$no_items;
	 }
//end saving miscellaneous
	// "<br>saveok_cnt=".$saveok_cnt;
	 
//start saving op room accommodation
	 ...
	 $array = array('encounter_nr'=>$encounter_nr,
									...
									'area' => $area); //edit;
	 $seg_ops->save_accommodation_charges($array);*/    //old codes
	 
	 $accomodation_array = array('encounter_nr'=>$encounter_nr,
									'opward_nr' => $_POST['opward_nr'],
									'oproom_nr' => $_POST['oproom_nr'],
									'total_rvu' => $_POST['total_rvu'],
									'multiplier' => $_POST['multiplier'],
									'room_charge' => $_POST['room_charge'],
									'area' => $area);
	 if(count($_POST['room_charge'])>0)
	 {
		 $saveok_cnt++;
	 }
	 /*echo "<pre>";
	 print_r($accomodation_array);
	 echo "</pre><br>"; */
	 //$saveok = $seg_ops->save_op_accommodation_charge($accomodation_array, $_POST['billdate']);
//end saving op room accommodation
	
	//echo "<br>saveok_cnt=".$saveok_cnt;
	if($saveok_cnt<=0 && $no_items==0) 
	{
...
#--added by CHa, May 11,2010
$smarty->assign('is_laboratory','<input type="hidden" id="from_lab" name="from_lab" value="'.$is_lab.'"/>');
$smarty->assign('sOnLoadJs','onLoad="preSet();"'); 
#--end CHA
...
#added by CHA, April 26,2010
$smarty->assign('sess_userid','<input type="hidden" id="userid" name"userid" value="'.$_SESSION['sess_user_name'].'"/>');
$smarty->assign('sess_add_dt','<input type="hidden" id="add_dt" name"add_dt" value="'.date('Y-m-d H:i:s').'"/>');
...
$smarty->assign('sSelectOps', '<a title="Procedures" href="#"><img id="ops_selected" 
		onclick="overlib(OLiframeContent(\'../../../modules/or/request/or_charged_accomodation.php'.URL_APPEND.'&enc_nr=\'+document.getElementById(\'encounter_nr\').value+\'&target=ac\', 725, 380, \'fSelOps4Acc\', 1, \'auto\'),
...
$smarty->assign('sMainBlockIncludeFile','or/other_clinic_charges_or.tpl'); //Assign the or_main template to the frameset
...
function preSet()
{
	var is_lab = $('from_lab').value;
	if(is_lab)
	{
		$('or_accomodation_field').style.display = "none";
		$('or_equipment_field').style.display = "none";
		$('or_misc_field').style.display = "";
		$('or_medicine_field').style.display = "";
	}
	else{
		$('or_accomodation_field').style.display = "";
		$('or_equipment_field').style.display = "";
		$('or_misc_field').style.display = "";
		$('or_medicine_field').style.display = "";
	}
}
...
function empty_accommodation2() {
	...
	table1.innerHTML = '<tr id="empty_accommodation_row"><td colspan="9">Additional accommodation empty...</td></tr>';
...
function empty_accommodation() {
	...
	var table1 = $('accommodation_list').getElementsByTagName('tbody').item(0)  
	table1.innerHTML = '<tr id="empty_accommodation_row"><td colspan="9">No O.R. accomodation charged...</td></tr>';
	
	xajax_delAllOpAccommodation($('encounter_nr').value,$('opchrg_refno').value);
	...
	J("input[@name='total_rvu[]']").remove();
	J("input[@name='multiplier[]']").remove();
	J("input[@name='oproom_nr[]']").remove();
	J("input[@name='opward_nr[]']").remove();
	update_total_charge_accommodation();               
}

function empty_misc() {
	 var table1 = $('misc_list').getElementsByTagName('tbody').item(0);  
	table1.innerHTML = '<tr id="empty_misc_row"><td colspan="9">Miscellaneous charges empty...</td></tr>';
