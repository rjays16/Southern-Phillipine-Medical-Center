...
function getRoomsByDept(objValue){
	//alert("dept nr = "+objValue);
}
-->
</script>
	...
	<!--  The department list block  -->
		...
		<?php
			...
			while(list($x,$v)=each($dept_DOC)){
				...
				echo ' onClick="getRoomsByDept(this.value);">&nbsp;&nbsp;'.$bold;
	...
	<!--  Start of the OR room numbers block -->
		...
		if(is_object($ORNrs)){
			...
			while($room=$ORNrs->FetchRow()){
				...
				echo '<input type="radio" name="saal" value="'.$room['room_nr'].'" id="'.$room['nr'].'" onClick="document.getElementById(\'op_nr\').value=this.id;"';
...
-->
<input type="hidden" name="target" value="or_new_request">
