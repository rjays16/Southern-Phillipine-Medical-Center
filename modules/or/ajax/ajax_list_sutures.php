...
function countRec($where) {
	...
	$sql = "SELECT COUNT(*) FROM seg_or_sutures $where";
...
$page = $_POST['page'];
$rp = $_POST['rp'];
$sortname = 'seg_or_sutures.'.$_POST['sortname'];
$sortorder = $_POST['sortorder'];
//$table_name = $_GET['table'];
if (!$sortname) $sortname = 'seg_or_sutures.date_created';
...
if ($query || $qtype) $where = " WHERE seg_or_sutures.is_deleted=0 AND seg_or_sutures.name LIKE '%$query%'";
...
/*$query = "SELECT DISTINCT seg_or_checklist.checklist_id, seg_or_checklist.checklist_question,
					...
					$where $sort $limit";   */
$query = "SELECT id, name FROM seg_or_sutures $where $sort $limit";
	 ...
	 if($result){
		 if($db->Affected_Rows())
		 {
			 while($row = $result->FetchRow()) {
				...
				$edit = '<a href="javascript:void(0)" onclick="open_edit_package_popup('.$row['id'].')" id="edit_charge"></a>';
				$delete = '<a href="javascript:void(0)" onclick="open_delete_package_popup('.$row['id'].')" id="delete_charge"></a>';   //
	...
	//			$delete = '<a href="javascript:void(0)" id="delete_charge"></a>';   //
				...
				$json .= "\n{";
				$json .= "id:'".$row['id']."',";
				$json .= "cell:['".htmlentities($row['name'])."'";
				$json .= ",'".$edit."'";
				$json .= ",'".$delete."']";
				$json .= "}";
				$rc = true;
