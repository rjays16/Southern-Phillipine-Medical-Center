...
WHERE som.status IN ('request', 'approved', 'disapproved')
AND som.is_main = '1'
 $where $sort $limit";
	#echo $sql;
...
#echo $query;
	 ...
	 while($row = $result->FetchRow()) {
		 if ($row['status'] == 'request') {
			 $status = 'Pending';
			 #$approve = '<a href="'.$root_path.'modules/or/or_main/or_main_approve_request.php'.URL_APPEND.'&refno='.$row['refno'].'" id="approve_request"></a>';
			 $approve = '<a href="'.$root_path.'modules/or/or_main/or_main_approve_request.php'.URL_APPEND.'&refno='.$row['refno'].'&pid='.$row['pid'].'&encounter_nr='.$row['encounter_nr'].'" id="approve_request"></a>';
		 }
		 elseif ($row['status'] == 'approved') {
			 $status = 'Approved';
			 #$approve = '<a href="'.$root_path.'modules/or/or_main/or_main_approve_request.php'.URL_APPEND.'&refno='.$row['refno'].'" id="disapprove_request"></a>';
			 $approve = '<a href="'.$root_path.'modules/or/or_main/or_main_approve_request.php'.URL_APPEND.'&refno='.$row['refno'].'&pid='.$row['pid'].'&encounter_nr='.$row['encounter_nr'].'" id="disapprove_request"></a>';
		 }
		 else {
			 $status = 'Disapproved';
			 #$approve = '<a href="'.$root_path.'modules/or/or_main/or_main_approve_request.php'.URL_APPEND.'&refno='.$row['refno'].'" id="approve_request"></a>';
