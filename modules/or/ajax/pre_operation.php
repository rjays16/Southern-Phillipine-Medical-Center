...
$query = "SELECT DISTINCT som.encounter_nr, sos.refno, som.or_main_refno, som.status,
CONCAT_WS(' ',DATE_FORMAT(sos.request_date, '%m-%d-%Y'), TIME_FORMAT(sos.request_time, '%h:%i %p')) as request_date,
 ...
 $where $sort $limit";
	# 	$objResponse->addAlert($query);
	 ...
	 while($row = $result->FetchRow()) {
		 ...
		 $schedule = '<a href="'.$root_path.'modules/or/or_asu/or_asu_pre_operative.php?refno='.$row['refno'].'&pid='.$row['pid'].'&encounter_nr='.$row['encounter_nr'].'" id="pre_operative"></a>';
