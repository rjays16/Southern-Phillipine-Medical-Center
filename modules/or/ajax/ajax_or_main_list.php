...
INNER JOIN care_department cd ON (cd.nr = som.dept_nr)
WHERE som.status IN ('request', 'disapproved', 'approved')
AND som.is_main='1' $where $sort $limit";
	//$obj $query;
	 ...
	 while($row = $result->FetchRow()) {
		 ...
		 if($status == 'Approved'){
				//$approved = 'approved';
				$edit = '<a href="'.$root_path.'modules/or/or_main/or_main_request_edit.php'.URL_APPEND.'&refno='.$row['refno'].'&encounter_nr='.$row['encounter_nr'].'&pid='.$row['pid'].'&mode='.$mode.'&status='.$status.'" id="view_charge"></a>';
		 }else{
				$edit = '<a href="'.$root_path.'modules/or/or_main/or_main_request_edit.php'.URL_APPEND.'&refno='.$row['refno'].'&encounter_nr='.$row['encounter_nr'].'&pid='.$row['pid'].'&mode='.$mode.'" id="edit_charge"></a>';
		 }
