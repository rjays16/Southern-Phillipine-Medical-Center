...
WHERE som.status IN ('approved', 'scheduled')
 $where $sort $limit";  */
 $sql = "SELECT COUNT(sos.refno)
FROM seg_ops_serv sos
INNER JOIN care_encounter_op ceo ON (sos.refno = ceo.refno)
...
WHERE som.status IN ('request', 'scheduled', 'approved')
AND is_main = '1'
 $where $sort $limit";
...
INNER JOIN seg_or_main som ON (som.ceo_refno = ceo.refno)
INNER JOIN care_department cd ON (cd.nr = ceo.dept_nr)
INNER JOIN care_room cr ON (cr.nr=ceo.op_room) WHERE som.status IN ('approved', 'scheduled') $where $sort $limit"; */
...
$where $sort $limit";
	 ...
	 $schedule = '<a href="'.$root_path.'modules/or/or_main/or_main_schedule_request.php?refno='.$row['refno'].'&pid='.$row['pid'].'" id="schedule_request"></a>';
