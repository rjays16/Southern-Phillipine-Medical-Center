				...
				if($ergebnis=$db->Execute($sql)){
					...
					if($rows==1){
							...
							if(!empty($diagnosis)){
								...
								$sql.=",diagnosis='".htmlspecialchars($diagnosis)."'";
...
#		 					$sql="SELECT op_nr FROM $dbtable WHERE dept_nr='$dept_nr' AND op_room='$saal' ORDER BY op_nr DESC";   # burn commented: December 20, 2007
		 					$sql="SELECT op_nr FROM $dbtable WHERE dept_nr='$dept_nr' AND op_room='$saal' AND op_date='$op_date' ORDER BY op_nr DESC";   # burn added: December 20, 2007
							//echo $sql;
...
function getinfo(m)
{
	urlholder="<?php echo "op-pflege-log-getinfo.php?sid=$sid&lang=$lang&dept_nr=$dept_nr&saal=$saal&op_nr=$op_nr&enc_nr=$enc_nr&pday=$pday&pmonth=$pmonth&pyear=$pyear&op_request_nr=".$arr['nr']."&winid=";?>"+m;
...
//test window
function newWinSched(pid){
	urlholder="<?php echo $root_path; ?>/modules/or_logbook/dialogSchedule.php<?php echo URL_REDIRECT_APPEND; ?>&pid="+pid+"&enc_nr=<?=$enc_nr?>&dept=<?=$dept_nr?>&encoder=<?php echo $HTTP_COOKIE_VARS['ck_op_pflegelogbuch_user'.$sid]?>";
	window.open(urlholder,pid,"width=310, height=450, menubar=no, resizeble=yes, scrollbars=yes");
}
...
<a 
	...
	alt="<?php echo $LDStartNewDocu ?>"></a>
-------------fdp--------------til here only--------->	
	<A href="op-pflege-logbuch-xtsuch-start.php?sid=<?php echo "$sid&lang=$lang&internok=$internok&dept_nr=$dept_nr&saal=$saal"; ?>&user=<?php echo str_replace(' ','+',$op_pflegelogbuch_user); ?>" target="_parent"><img <?php echo createLDImgSrc($root_path,'searchlamp.gif','0','absmiddle') ?> 
...
<!---------commented out by pet-----apr22,2008---------------		
		alt="'.str_replace("~tagword~",$lname,$LDOpenPatientFolder).'"></a>
-----------fdp--------------til here only------------------->	
		...
		<br>
<!---------commented out by pet-----apr22,2008---------------				
		<a href="javascript:newWinSched(\''.$pdata['pid'].'\')"><span style="fontsize:3">Schedule</span></a>
-----------fdp--------------til here only------------------->			
		';
...
#print_r($pdata);
	...
	<textarea name="diagnosis" cols=16 rows=8 wrap="physical" >'.$pdata['diagnosis'].'</textarea>';
}else{
	echo $LDDiagnosis;
}
?>
...
{
//	echo "<br>\ n";
//	echo "<br>\n  OPERATOR=".$pdata['operator']."<br>\n";
//	$tmp_unserialize = unserialize($pdata['operator']);
//	echo "<br>\n unserialize OPERATOR=".print_r($tmp_unserialize);
	...
	for($n=0; $n<sizeof($ebuf); $n++){
		...
		//unserialize data elements
		$mypData = unserialize($pdata[$ebuf[$n]]);
		
		//display the content of the element
		$g='';
		for($i=0;$i<sizeof($mypData); $i++){
			$g = $i+1;
			$person_nr = $mypData[$ebuf[$n]."+".$g];	
		    //retrieve personell data 				    
			if($pers_obj->loadPersonellData($person_nr)){
			 ...
			 if($mypData[$ebuf[$n]."+".$g] == ''){
			    ...
			    #echo '&nbsp;'.$elems[n]." ".$tbuf[$n].$elems[x]."<br><br>";		#just added one more <br>---pet =) ---
				 echo '&nbsp;'.$elems[n]."<br><br>";		#just added one more <br>---pet =) ---
			}
		}	
	}
	
	
//	--------- old comment by mark on July 20, 2007------------
//	for($n=0;$n<sizeof($ebuf);$n++)
//	{
//		echo '<a href="javascript:getinfo(\''.$jbuf[$n].'\')">'.$cbuf[$n].'</a><br>';
//		if(!$pdata[$ebuf[$n]])
...
//		}
//	}
//   --------- old end comment by mark on July 20, 2007 -----------	
}
 else
 {
	echo '
...
//        	echo "<br> dbuf=>>".$mydbuf."<br> \n";
//        	print_r($mydbuf);
        	$r = '';
        	for($i=0; $i<sizeof($mydbuf); $i++){
        		$r = $i + 1;
        		$pNr = $mydbuf["an_doctor+".$r];		
        		if($pers_obj->loadPersonellData($pNr)){
        			$lastName = $pers_obj->personell_data['name_last'];
        			$firstName = $pers_obj->personell_data['name_first'];	
        		}
        		$elems[n] = $lastName.", ".$firstName;
...
#	echo '" target="OPLOGMAIN">'.$LDTherapy.'/'.$LDOperation.'</a><br>
	echo '" target="OPLOGMAIN">'.$LDOperation.'/'.$LDSProcedure.'</a><br>	<!---replaced by pet, 2008apr22--->
	<TEXTAREA NAME="op_therapy" COLS="18" ROWS="8">'.$pdata['op_therapy'].'</TEXTAREA>';
	}
#	else echo $LDTherapy.'/'.$LDOperation;
	else echo $LDOperation.'/'.$LDSProcedure;	#---replaced by pet, 2008apr22---
?>
</TD>
...
<TEXTAREA NAME="result_info" Content-Type="text/html"
	COLS="18" ROWS="8">'.$pdata['result_info'].'</TEXTAREA>';
?>
</TD>
