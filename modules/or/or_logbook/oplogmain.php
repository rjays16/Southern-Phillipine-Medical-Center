...
<?php
 	echo '<script type="text/javascript" src ="'.$root_path.'modules/or_logbook/js/oplogmain.js"></script>';
...
<script type="text/javascript">
 	function delLogMainInfo(enc_nr, dept, op_nr){	
		//alert("enc_nr="+ enc_nr+ " dept_nr=" + dept_nr + " op_nr="+ op_nr);		
		var ans = confirm('Are you sure you want to delete the journal entry no. '+op_nr+'?');
		
		if (ans){
			xajax_delLogMainInfo(enc_nr,dept,op_nr);		
 		}
 	}
 	
 	function refreshLogInput(bol){
		var dept = document.getElementsByName("dept_nr");
 		var saal = document.getElementsByName("saal");	
		 		
 		if(bol){
 			alert("OR journal was deleted");
 			
 			if((dept.value == '<?= $dept_nr ?>') && (saal.value == '<?= $saal ?>')) return false;
 			window.location.reload();
 			window.top.LOGINPUT.location.replace('oploginput.php?sid=<?php echo "$sid&lang=$lang" ?>&mode=chgdept&dept_nr='+dept.value+'&saal='+saal.value);
 			return true;
 		}else{
 			alert("Patient journal is not deleted");
 			return false;
 		}
 	}
 </script>
...
echo '
<table id="oplogmainTable" name="oplogmainTable" cellpadding="3" cellspacing="1" border="0" width="100%">';	
...
echo '
		<tr class="wardlisttitlerow"><td colspan=2><nobr>
			...
			<b>';
				...
				echo ' '.$LDRoom.'-'.strtoupper($saal).' ('.formatDate2Local($thisday,$date_format).')
			</b></td>';
?>
			<td colspan=2>
			<nobr>
			<table cellpadding=0 cellspacing=0 border=0>	
				<form action="oplogmain.php" method="post" name="chgdept" id="chgdept" onSubmit="return pruf(this)">
					<tr>
						<td>
							<input type="hidden" name="thisday" value="<?php  echo $thisday; ?>">
			    			<input type="hidden" name="sid" value="<?php echo $sid; ?>">
			    			<input type="hidden" name="lang" value="<?php echo $lang; ?>">
			    			<input type="hidden" name="h_dept_nr" id="h_dept_nr" value="<?= $dept_nr?>">
			    			<!-- <script language="javascript">
							 	presetDept(<?= $dept_nr ?>);
							</script> -->
				<!-- <select name="dept_nr" size=1 onChange="syncDept(this,document.chgdept.saal)"> -->
							<select name="dept_nr" id="dept_nr" size=1>
					<?php
				...
				<select name="saal" id="saal" size=1>
				<?php
				if(is_object($ORNrs)){
...
<?php
	echo '
			...
			<form>
<!------commented out by pet-------2008april22------			
...
// '<?=$thisday'
?>
<script language="javascript">xajax_populateOpLogMain(<?= $dept_nr?>, <?=$saal?>)</script>
<tr id="orListTr" bgcolor="f9f9f9" >
	<tbody id="orListTbody"> 
	<!------------2008-04-08, fdp--------------
		<tr><td>Hello mark hello mark </td></tr>	
	--------------'til here only, fdp--------->
	</tbody>
</tr>

<input type="hidden" name="hsaal" id="hsaal" value="<?=$saal?>" />
<input type="hidden" name="internok" id="internok" value="<?=$internok?>" />
...
//-----------------mark----------
//echo "<br> pdata = >".$pdata=ergehis. "<br> \n";
//print_r($ergebnis);

#comment by mark for replace of innerHTML function using xajax 
...
<!-------commented out by pet, apr22,2008------------	
	<a href="javascript:getinfo(\''.$pdata['encounter_nr'].'\')">
-------------fdp--------------til here only--------->	
	<img '.createComIcon($root_path,'info2.gif','0').'
<!-------commented out by pet, apr22,2008------------	 
	alt="'.str_replace("~tagword~",$pdata['lastname'],$LDOpenPatientFolder).'"></a>
-------------fdp--------------til here only--------->
 ';	
	...
	<font color=black><b>'.$pdata['name_last'].', '.$pdata['name_first'].'</b><br>'.formatDate2Local($pdata['date_birth'],$date_format).'<p>
			<font color="#000000">'.$pdata['addr_str'].' '.$pdata['addr_str_nr'].'<br>'.$pdata['addr_zip'].' '.$pdata['citytown_name'].'
	</font>
		<a href="javascript:delLogMainInfo(\''.$pdata['encounter_nr'].'\',\''.$pdata['dept_nr'].'\',\''.$pdata['op_nr'].'\' )" >
		<img '.createComIcon($root_path,'dwnarrowgrnlrg.gif','0').' alt="'.str_replace("~tagword~",$pdata['lastname'],$LDEditPatientData).'">
		Delete</a>
		<br>
	</td>';
	echo '
	...
	#$ebuf=array("operator","assistant","scrub_nurse","rotating_nurse","an_doctor");

	for($n=0;$n<sizeof($ebuf);$n++){
		#echo "<br>? = ".$pdata[$ebuf[$n]];
		if(!$pdata[$ebuf[$n]]) continue;
		echo '<font color="#cc0000">'.$cbuf[$n].'</font><br>';
		
		$mybuffer = unserialize($pdata[$ebuf[$n]]);
		#print_r($mybuffer);
		$h ='';
		for($i=0;$i<sizeof($mybuffer);$i++){
			$h = $i + 1;
		    $myPersonell_nr = $mybuffer[$ebuf[$n]."+".$h];
		    #echo "nr = ".$myPersonell_nr;
		    if($pers_obj->loadPersonellData($myPersonell_nr)){
		   		$lastname = $pers_obj->personell_data['name_last']; 
		   		$firstname = $pers_obj->personell_data['name_first'];

		    }
		   # echo "<br>sql = ".$pers_obj->sql;
		    if($mybuffer[$ebuf[$n]."+".$h] == ""){
		    	continue;
		    }else{
		    	$elems[n]=$lastname.", ".$firstname;
				#print_r($elems);	
   ...
   echo '
    ...
    if($pdata['an_doctor']){
		...
		$mydbuf = unserialize($pdata['an_doctor']);
		$f='';
		for($i=0;$i<sizeof($mydbuf);$i++){
			$f = $i+1;
			$myData = $mydbuf['an_doctor+'.$f];
			if($pers_obj->loadPersonellData($myData)){
     			$lastname = $pers_obj->personell_data['name_last'];
				$firstname = $pers_obj->personell_data['name_first'];	
			}
	...
	for($i=0;$i<sizeof($cc);$i++)
	{
		...
		echo '<font face="verdana,arial" size="1" color="#cc0000">'.$LDOpCut.':</font><br>'.strtr($ccbuf['s'],'.',':').'<br><br>
		<font face="verdana,arial" size="1" color="#cc0000">'.$LDOpClose.':</font><br>'.strtr($ccbuf['e'],'.',':').'<br><br>';
...
*/	
	...
	<td valign=top><font face="verdana,arial" size="1" color="#cc0000">'.$LDOperation.'/'.$LDSProcedure.':<font color=black><br>'.nl2br($pdata['op_therapy']).'</td>';
	echo '
	...
	for($i=0;$i<sizeof($eo);$i++)
	{
		...
		echo '<font face="verdana,arial" size="1" color="#cc0000">'.$LDOpIn.':</font><br>'.strtr($eobuf['s'],'.',':').'<br><br>
		<font face="verdana,arial" size="1" color="#cc0000">'.$LDOpOut.':</font><br>'.strtr($eobuf['e'],'.',':').'<br><br>';
