...
$breakfile=$root_path.'modules/or/or_asu/or_asu_approve.php'.URL_APPEND;
...
if (isset($_POST['submitted'])) {
	...
	/*if(!$_POST['or_receipt']){
		echo "<script>alert('No Official Receipt Yet')</script>";
	} */
	...
	$data = array('room_nr'=> $_POST['or_room'],
								...
								'final_date_operation'=> date('Y-m-d', strtotime($_POST['date_operation']))
					);
	...
	if ($op == 'insert' && $status == 'approved' && $save)
		$smarty->assign('sysInfoMessage','OR ASU request was successfully approved.');
	elseif ($op == 'update' && $status == 'approved' && $save)
		$smarty->assign('sysInfoMessage', 'OR ASU Request approval reason successfully updated.');
	elseif ($op == 'insert' && $status = 'disapproved' && $save)
		$smarty->assign('sysInfoMessage', 'OR ASU Request was successfully disapproved.');
	elseif ($op == 'update' && $status == 'disapproved' && $save)
		$smarty->assign('sysInfoMessage', 'OR ASU Request disapproval reason successfully updated.');
...
$smarty->assign('sToolbarTitle',"Operating Room ASU :: ".$operation." Request for Surgery"); //Assign a toolbar title
...
$smarty->assign('pid', '<input type="hidden" name="pid" value="'.$pid.'"  id="pid"/>');
/** End **/
...
$smarty->assign('sMainBlockIncludeFile','or/or_asu_approve_request.tpl'); //Assign the or_main template to the frameset
...
function validate(e) {
	...
	//alert('transtype= '+$('transtype').innerHTML);
