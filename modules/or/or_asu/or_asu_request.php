...
$seg_room = new OPRoom();
//$dept_nr = isset($_GET['dept_nr']) ? $_GET['dept_nr'] : $_POST['dept_nr'];   Commented by Cherry 06-25-10
//$op_nr = isset($_GET['op_nr']) ? $_GET['op_nr'] : $_POST['op_nr'];
//$op_room = isset($_GET['op_room']) ? $_GET['op_room'] : $_POST['op_room'];   Commented by Cherry 06-25-10
//$department = $seg_department->FormalName($dept_nr);
#Uncommented by Cherry 02-16-10
#$operating_room = $seg_room->get_or_name($op_room);

$breakfile=$root_path.'main/op-doku.php'.URL_APPEND; /** See Maramag **/
...
#print_r($_SESSION['sess_user_name']);
$seg_ops = new SegOps();
...
if ($seg_ops->encOpsNrHasOpsServ($nr)) {
	...
	#print_r($or_main_basic_info);
}
$or_main_info = $seg_ops->get_or_main_basic_info($refno);
//print_r($or_main_info);

//print_r($_SESSION['sess_user_name']);     Commented by Cherry 06-29-10
...
//$smarty->assign('form_start', '<form id="main_or_form" name="main_or_form" method="POST" action="'.$_SERVER['PHP_SELF'].'" onsubmit="return validate()">');
...
#$smarty->assign('', '<input type="text" disabled="disabled" name="or_request_department" id="or_request_department" value="'.$department.'" />');
...
//print_r($department);
#echo "<br>";
#print_r($seg_department->getDeptDoesSurgery());
...
$person_gender = (strnatcasecmp($person_info['sex'], 'm') == 0) ? 'Male' : '';
...
#Commented by Cherry 02-16-10
$str = strlen($person_name);
//$patient_name = $str;
if(strlen($person_name) == 3){
	$patient_name = '';
}else{
	$patient_name = $person_name;
}
...
$smarty->assign('submitted', '<input type="hidden" value="TRUE" name="submitted" id="submitted" />');
...
$smarty->assign('sMainBlockIncludeFile','or/or_asu_request.tpl'); //Assign the or_main template to the frameset
...
function openScheduleView() {
	var dept = $('or_request_department').value; //added by cha, 11-18-2010
	overlib(
...
//var additional = '&var_include_enc='+($('or_transaction_type').checked?'0':'1');   Commented by Cherry 04-27-10
//alert(params+additional);
//alert(params);
overlib(
				OLiframeContent('<?=$root_path?>modules/registration_admission/seg-select-enc.php?'+params, 700, 400,
 ...
 function checkType(thisType){
	 ...
	 /* Added by Cherry 02-19-10 */
		function preset(){
				...
				//document.getElementById('or_main_print').disabled = true;
				checkDate();
		}
...
function validate(e) {
		...
		var array_elements = [ {field: J("input[@name='or_transaction_type']"),
																								 ...
																								 {field: J("input[@name='package_name']"),
																								 field_value: J("input[@name='package_name']").val(),
																								 msg: 'Please choose a procedure',
																								 msg_dest: J('#error_form_input', J('#procedure_name_msg')),
																								 is_textfield: true
																								 },
																								 ...
																								 field_value: J("input[@name='or_doctor']:selected").val(),
																								 ...
																								 field_value: J("input[@name='or_request_department']:selected").val(),
																								 msg: 'Please indicate the requesting department',
																								 msg_dest: J('#error_form_input', J('#or_request_department_msg')),
																								 },
																								 {field: J("input[@name='or_operation_date']"),
																								 field_value: J("input[@name='']").val(),
																								 },
