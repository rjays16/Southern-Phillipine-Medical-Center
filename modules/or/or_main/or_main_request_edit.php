...
$breakfile = 'or_main_list.php'.URL_APPEND;
...
/*if($_GET['status']=='approved'){
	echo "<script language=javascript>preset()</script>";
} */
#echo "mode= ".$mode;
...
if($mode == 'edit'){
	...
	//$_POST['or_request_priority'] = $edit_info['request_priority'];
	...
	$priority = $edit_info['request_priority'];
	...
	//$_POST['pre_operative_diagnosis'] = $info['er_opd_diagnosis'];
	//$_POST['remarks'] = $edit_info['remarks'];
	$preop_diag = $info['er_opd_diagnosis'];
	$rem = $edit_info['remarks'];
	$_POST['mode'] = 'edit';
}
...
if (isset($_POST['submitted'])) {
	...
	$requirements_selected = $_POST['or_special_requirements'];
	$priority = $_POST['or_request_priority'];
	$rem = $_POST['remarks'];
}
...
#$smarty->assign('or_request_', '<input type="text" disabled="disabled" name="or_request_department" id="or_request_department" value="'.$department.'" />');
...
$smarty->assign('or_request_priority_selected', $priority);
...
$smarty->assign('patient_hospital_number', $pid);
...
/*$smarty->assign('or_operation_date_display', '<div id="or_operation_date_display" class="date_display">'.$operation_date_display.'</div>');
$smarty->assign('or_operation_date', '<input type="hidden" name="or_operation_date" id="or_operation_date" value="'.$operation_date.'" checkDate(); />');
$smarty->assign('or_operation_dt_picker', '<img src="'.$root_path.'images/or_main_images/date_time_picker.png" id="or_operation_dt_picker" class="date_time_picker" onClick="alert(\'meow\')" />');
...
$pre_operative_diagnosis = isset($_POST['pre_operative_diagnosis']) ? $_POST['pre_operative_diagnosis'] : $preop_diag;
$remarks = isset($_POST['remarks']) ? $_POST['remarks'] : $rem;
...
$smarty->assign('pre_operative_diagnosis', '<textarea readonly="readonly" name="pre_operative_diagnosis" id="pre_operative_diagnosis">'.$patient_diagnosis.'</textarea>');
...
/*$ = array('cp_clearance'=>'CP Clearance', 'pulmo_clearance'=>'Pulmo Clearance', 'consent'=>'Consent',
...
#$special_requirements = array('cp_clearance'=>'CP Clearance', 'pulmo_clearance'=>'Pulmo Clearance', 'consent'=>'Consent','pedia_clearance'=>'Pedia Clearance', 'others'=>'Others');
#$smarty->assign('or_special_requirements', $special_requirements);

$special_requirements = array('cp_clearance'=>'CP Clearance', 'pulmo_clearance'=>'Pulmo Clearance', 'consent'=>'Consent',
															 'pedia_clearance'=>'Pedia Clearance', 'others'=>'Others');
$smarty->assign('or_special_requirements', $special_requirements);
$smarty->assign('or_special_requirements_selected', $requirements_selected);
//$smarty->assign('or_special_requirements_selected', $_POST['or_special_requirements']);
...
$smarty->assign('sMainBlockIncludeFile','or/or_main_request_get.tpl'); //Assign the or_main template to the frameset
...
function printRequest(encounter_nr, type) {
...
function openScheduleView() {
	...
	overlib(
				OLiframeContent('<?=$root_path?>modules/or/request/seg_or_schedules.php?m=ormain&dept='+dept, 800, 400,
					'schedView', 0, 'auto'),
