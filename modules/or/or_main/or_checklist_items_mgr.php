...
$smarty = new Smarty_Care('or_checklist_mgr');
$smarty->assign('sToolbarTitle',"OR::Checklist Manager"); //Assign a toolbar title
...
$smarty->assign('new_package', '<input type="button" id="new_package" value="Create New Checklist Item" onclick="open_new_package_popup()" />');
...
$smarty->assign('sMainBlockIncludeFile','or/or_checklist_mgr.tpl'); //Assign the select_or_request template to the frameset
...
$(document).ready(function() {
 $('#or_request_table').flexigrid({
	url: '<?=$root_path?>modules/or/ajax/ajax_list_checklist_items.php?target=<?=$target?>',
	dataType: 'json',
	colModel : [{display: 'Checklist Question', width:300, name : 'checklist_question', sortable : true, align: 'left'},
						 {display: 'Additional Detail', width:130, name:'label_data', sortable: true, align: 'center'},
						 {display: 'Mandatory', width:100, name:'is_mandatory', sortable: false, align: 'center'},
...
function open_new_package_popup() {
	overlib(
		OLiframeContent('<?=$root_path?>modules/or/or_main/or_checklist_items_mgr_tray.php?mode=new', 600, 300, 'fOrderTray', 0, 'yes'),
		WIDTH,600,HEIGHT,300,TEXTPADDING,0, BORDER,0,
		STICKY, SCROLL, CLOSECLICK, MODAL,
		CLOSETEXT, '<img src=<?=$root_path?>images/close_red.gif border=0 onclick="window.parent.reload()">',
		CAPTIONPADDING,2,DRAGGABLE,
		CAPTION,'New Checklist Item',
		MIDX,0, MIDY,0,
		STATUS,'New Checklist Item');
	return false
}
...
function open_edit_package_popup(checklist_id) {
	overlib(
		OLiframeContent('<?=$root_path?>modules/or/or_main/or_checklist_items_mgr_tray.php?id='+checklist_id+'&mode=edit', 600, 300, 'fOrderTray', 0, 'yes'),
		WIDTH,600,HEIGHT,300,TEXTPADDING,0, BORDER,0,
		STICKY,  SCROLL, CLOSECLICK, MODAL,
		CLOSETEXT, '<img src=<?=$root_path?>images/close_red.gif border=0 onclick="window.parent.reload()">',
		CAPTIONPADDING,2,DRAGGABLE,
		CAPTION,'Edit Checklist Item',
		MIDX,0, MIDY,0,
		STATUS,'Edit Checklist Item');
	return false
}

function open_delete_package_popup(checklist_id){
	if(confirm("Delete checklist item?"))
		xajax_delete_checklist_item(checklist_id);
}
