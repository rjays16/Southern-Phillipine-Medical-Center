		...
		return this.each(function() {
			...
			$(".timepicki-input").keydown(function (keyevent) {
			    ...
			    function validate() {
                   ...
                   //          Math.min(Math.max(parseInt(input.val()), 0), 12) : // for 12 hour date picker.
			                // Math.min(Math.max(parseInt(input.val()), 0), 23); // for 24 hours date picker.
			                
                            // clip number.
							var hours = (settings.show_meridian ?
											(parseInt(input.val()) <= 12? input.val() : 12) : // for 12 hour date picker.
											(parseInt(input.val()) <= 23 ? input.val() : 23) // for 24 hours date picker.
										);
			           		if(hours == '00')
			           			hours = 12;
			                // assign number.
			            ...
			            if (isValidNumber) {
			                ...
			                // clip number.
			                var minutes = (parseInt(input.val()) <= 59 ? input.val() : 59);
			...
			function set_date(start_time) {
				...
				if (ti < 10 && ti != 0) {
					...
					ele_next.find(".ti_tx input").val(12);
				}else {
			...
			function change_time(cur_ele, direction) {
				...
				if ((cur_ele && cur_ele.hasClass('action-next')) || direction === 'next') {					if (cur_time + step_size > ele_en) {
