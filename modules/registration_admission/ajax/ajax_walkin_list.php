...
$result = $walkin->searchWalkin($reqFilters);
/*echo "<pre>";
print_r($walkin->getLastQuery());
echo "</pre>";*/
...
if ($result !== FALSE) {
	...
	while ($row = $result->FetchRow()) {
		$options =
			"<button class='segButton' onclick='updateWalkinDetails(\"".$row['pid']."\"); return false;'><img src='../../gui/img/common/default/user_edit.png'/>Edit</button>".
			"<button class='segButton' onclick='deleteWalkin(\"".$row['pid']."\"); return false;'><img src='../../gui/img/common/default/user_delete.png'/>Delete</button>";

		$data[] = array(
			'walkin_name' => $row['name_last'].", ".$row['name_first']." ".$row['name_middle'],
