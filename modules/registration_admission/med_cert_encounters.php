...
function startAJAXSearch(searchID, page, pid) {
		...
		if (searchEL) {
				...
				//alert(pid);
				AJAXTimerID = setTimeout("xajax_populateMedCertEncRef('"+pid+"','"+searchID+"',"+page+",'"+keyword+"',false,"+isIPBM+")",100);
				//alert("here");
...
//alert('sulod listID, id, name, cash, charge, sservice = '+listID+" , "+id, name+" , "+cash+" , "+charge+" , "+sservice);
		...
		if (id) {
			 ...
			 var reagent ='onclick="return overlib(OLiframeContent(\'certificates/cert_med_interface.php?encounter_nr='+enc_nr+'&referral_nr='+ref_nr+'&showBrowser=1'+IPBMextend+'\', 720, 330, \'flab-list\', 1, \'auto\'), ' +
