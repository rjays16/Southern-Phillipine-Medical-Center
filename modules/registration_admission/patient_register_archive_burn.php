...
#		$sql="SELECT pid, date_reg, name_last, name_first, date_birth, addr_zip, sex, death_date, status FROM $dbtable WHERE ";
		$select_flds=" SELECT pid, date_reg, name_last, name_first, date_birth, addr_zip, sex, death_date, status ".
...
echo "header = <br>";
echo "Location:patient_register_show.php".URL_REDIRECT_APPEND."&target=archiv&origin=archiv&pid=".$result['pid'];
echo "<br>\n";
*/
...
#					$sql="SELECT COUNT(pid) AS maxnr FROM $dbtable WHERE ".$s2;
					$sql="SELECT COUNT(pid) AS maxnr $from_tb ".$s2;
