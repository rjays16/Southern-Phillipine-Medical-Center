...
if ($encounter_nr!=NULL){
	...
	#echo "patient_enc_cond = ".$patient_enc_cond['cond_code'];
	#echo "<br>patient_enc_disp =".$patient_enc_disp['disp_code'];
	#echo "<br>patient_enc_res = ".$patient_enc_res['result_code'];
	
	#$condition_classes=$encounter_obj->AllConditionClassesObject($patient_enc['encounter_type']);
	#$results_classes=$encounter_obj->AllResultsClassesObject($patient_enc['encounter_type']);
	#$disposition_classes=$encounter_obj->AllDispositionClassesObject($patient_enc['encounter_type']);
	             ...
	             {	
				      ...
				      if($update || $encounter_nr)
					  {
							...
							$itemno=$itemname;		
					  		 ...
					  		 #echo ("sulod : false update and encounter_nr");	
					  	    $newdata=1;
							...
							switch($HTTP_POST_VARS['encounter_class_nr'])
							{
								case '1':
								case '3': 
								case '4': $HTTP_POST_VARS['encounter_nr']=$encounter_obj->getNewEncounterNr($ref_nr+$GLOBAL_CONFIG['patient_inpatient_nr_adder'],1);
											break;
...
function chkform(d) {
	...
	if(dept_belong == "Admission"){
		...
		return false;
		...
		}else if(($update==1)&&!d.is_admit[0].checked){
			alert("<?php echo $LDPlsCheckAdmit; ?>");
			d.is_admit.focus();
			return false;
		}else if(d.current_att_dr_nr.value==0){
			alert("<?php echo $LDPlsSelectDr; ?>");
			...
			return false;
		}else if((!d.insurance_class_nr[2].checked) && (d.insurance_nr2.value==" ")){
			alert("<?php echo $LDPlsEnterInsuranceNo; ?>");
			d.insurance_nr2.focus();
			return false;
		}else if((!d.insurance_class_nr[2].checked) && (insurance_firm_name2.value==" ")){
...
function preset(){
	...
	if (encounter_class_nr == 2){
		if (dept_belong!="Admission"){
			...
			xajax_setALLDepartment(0);	// get all OPD Department
			xajax_setDepartments(<?php echo $current_att_dr_nr; ?>);
		}else{
			if (dept_nr)
	...
	//alert(d.current_att_dr_nr.value);
...
function jsGetDepartment(){
		...
		if (aPersonell_nr != 0) {
			xajax_setDepartments(aPersonell_nr);		
...
*/		
		...
		if ($HTTP_SESSION_VARS['sess_user_name'] == "Administrator"){	
			...
			if ((($encounter_class_nr==1)&&($patient_enc['encounter_type']!=3))||(($encounter_class_nr==2)&&($patient_enc['encounter_type']!=4))){
				...
				$sAdmit ="<input type=\"checkbox\" name=\"is_admit\" id=\"is_admit\" value=\"1\">";
			...
			if(($dept_belong['id'] == "Admission") && ((($encounter_class_nr==1)&&($patient_enc['encounter_type']!=3))||(($encounter_class_nr==2)&&($patient_enc['encounter_type']!=4)))){
				$sTemp = $sTemp.'<label id="admission_class"><b>'.$admission_class.'</b></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$sAdmit."&nbsp;&nbsp;Pls. check if you want to Admit the patient.";
			}else{
			...
			$smarty->assign('doctor_name2','<input name="consulting_dr" id="consulting_dr" type="hidden" size="60" value="'.$consulting_dr.'">');
			#-------------------------------------------
			...
			$smarty->assign('referrer_diagnosis','<input name="referrer_diagnosis" type="text" size="60" value="'.ucwords(strtolower(trim($referrer_diagnosis))).'">');
			
			$smarty->assign('LDTherapy',$LDTherapy);
			$smarty->assign('referrer_recom_therapy','<input name="referrer_recom_therapy" type="text" size="60" value="'.ucwords(strtolower(trim($referrer_recom_therapy))).'">');
			
			$smarty->assign('LDRecBy',$LDRecBy);
			
			if (($patient_enc['encounter_type']==2)&&($update==1)&&($dept_belong['id']=="Admission")){
				$smarty->assign('referrer_dr','<input name="referrer_dr" type="text" size="60" value="'.ucwords(strtolower(trim($dr))).'">');
				$smarty->assign('referrer_dept','<input name="referrer_dept" type="hidden" size="60" value="'.ucwords(strtolower(trim($dept_obj->FormalName($dept)))).'">');
				#$referrer_dept=$dept_obj->FormalName($dept);
			}else{
				$smarty->assign('referrer_dr','<input name="referrer_dr" type="text" size="60" value="'.ucwords(strtolower(trim($referrer_dr))).'">');
			}
			
			if ($encounter_class_nr!=2){
				$smarty->assign('LDRecIns',$LDRecIns);
				$smarty->assign('referrer_institution','<input name="referrer_institution" id="referrer_institution" type="text" size="60" value="'.ucwords(strtolower(trim($referrer_institution))).'">');
			}
			...
			$smarty->assign('insurance_firm_name2','<input name="insurance_firm_name2" id="insurance_firm_name2" disabled type="text" size="60" value="'.$sTemp.'"><span name="iconIns" id="iconIns" style="display:none">'.$sBuffer." Click Here.</span>");
			
			#---------added 03-06-07-------------
			$smarty->assign('insurance_firm_name','<input name="insurance_firm_name" id="insurance_firm_name" type="hidden" size="60" value="'.$sTemp.'">');
