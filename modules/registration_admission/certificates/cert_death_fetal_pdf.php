...
if ($fetalDeathCertInfo){
	...
	$data[0]['parent_marriage_info_tmp'] = "";
	...
	# 3. DATE OF DELIVERY
	$data[0]['delivery_date'] = $birthDay."          ".$birthMonth."     ".$birthYear;
	...
	# 5a. TYPE OF DELIVERY
	if ($birth_type=="1")
		$data[0]['birth_type_s'] = "X";
	if ($birth_type=="2")
		$data[0]['birth_type_twin'] = "X";
	if (($birth_type!="")&&($birth_type!="1")&&($birth_type!="2"))
		$data[0]['birth_type_tr'] = "X";

	# b. IF MULTIPLE DELIVERY, FETUS WAS
	if ($birth_rank == 'first')
		$data[0]['birth_rank_f'] = "X";
	if ($birth_rank == 'second')
		$data[0]['birth_rank_s'] = "X";
	else{
		$data[0]['birth_rank_o'] = "X";

		$data[0]['birth_rank_others'] = $birth_rank;
	}
	...
	if ($delivery_method == '1')
		$data[0]['delivery_method_n'] = "X";
	else{
		$data[0]['delivery_method_o'] = "X";

		$data[0]['delivery_method_info'] = $delivery_method_info;
	}

	# d. BIRTH ORDER
	$data[0]['birth_order'] = $birth_order;
	...
	$data[0]['m_religion'] = $religion_obj['religion_name'];
	...
	if(strlen($m_address) >= 55)
		$data[0]['m_address_shorter'] = $m_address;
	else
		$data[0]['m_address_long'] = $m_address;
	...
	$data[0]['f_religion'] = $religion_obj['religion_name'];
	...
	if (($parent_marriage_date!='0000-00-00') && (!empty($parent_marriage_date))){
		...
		if (($parent_marriage_place)||($parent_marriage_place!='N/A'))
			if($parent_marriage_place != ''){
				$data[0]['parent_marriage_info_tmp'] = date("F d, Y",strtotime($parent_marriage_date))." at ".trim($parent_marriage_place, " ");
			}else {
				$data[0]['parent_marriage_info_tmp'] = date("F d, Y",strtotime($parent_marriage_date))." at ".trim(mb_strtoupper($mun['mun_name'])," ").", ".mb_strtoupper($prov['prov_name']);
			}
			
		else
			$data[0]['parent_marriage_info_tmp'] = date("F d, Y",strtotime($parent_marriage_date));
	}else{
	...
	$data[0]['attendant_address'] = seg_ucwords($attendant_address);

	$data[0]['attendant_date_sign'] = $attendant_date_sign;

	# 23. CORPSE DISPOSAL
	if ($corpse_disposal_tmp=='1')
		$data[0]['corpse_disposal_tmp1'] = "X";
	if ($corpse_disposal_tmp=='2')
		$data[0]['corpse_disposal_tmp2'] = "X";
	if ($corpse_disposal_tmp=='3'){
		$data[0]['corpse_disposal_tmp3'] = "X";

		$corpse_disposal_others = "0";
		$data[0]['corpse_disposal_others'] = $corpse_disposal_others;
	}
	...
	# 25. AUTOPSY
	if ($is_autopsy=='1')
		$data[0]['is_autopsy_yes'] = "X";
	else if ($is_autopsy=='2')
		$data[0]['is_autopsy_no'] = "X";
	...
	if(strlen($informant_address) > 60 && strlen($informant_address) < 80)
		$data[0]['informant_address_short'] = $informant_address;
	elseif(strlen($informant_address) > 79)
		$data[0]['informant_address_shorter'] = $informant_address;
	else $data[0]['informant_address'] = $informant_address;
	...
	if(strlen($encoder_name) > 30)
		$data[0]['encoder_name_shorter'] =  mb_strtoupper($encoder_name);
	elseif(strlen($encoder_name) > 18 && strlen($encoder_name) <= 30)
		$data[0]['encoder_name_short'] =  mb_strtoupper($encoder_name);
	else
