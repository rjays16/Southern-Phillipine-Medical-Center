...
if (isset($_POST['mode'])){
	switch($_POST['mode']) {
		case 'save':
			...
			$HTTP_POST_VARS['birth_weight'] = intval($birth_weight);
								 ...
								 ?>
										...
										?>
															...
															<td width="30%" bgcolor="#FFF7F0" class="style2">City / Municipality <br />
																	<select id="m_residence_mun" name="m_residence_mun" onchange="UpdateInformantAddress(); CheckDropdown(this.value, this.id);">
																		<?php
	...
	<tr>
	 <td height="42" bordercolor="98D228" bgcolor="#F9FFFB" class="style2" id="married_parents">
		...
		<?php
			...
			?>
		 &nbsp;&nbsp; <?= $sDateMarriage ?>
		 <input type="hidden" id="p_residence_mun" name="p_residence_mun" value="NULL">
		 <input type="hidden" id="p_residence_prov" name="p_residence_prov" value="NULL">
		at &nbsp;<input type="text" name="parent_marriage_place" id="parent_marriage_place" size="60"
			value="<?php  if($parent_marriage_place !=""){ echo trim($parent_marriage_place," ");}
						else if($mun['mun_name'] == 'NOT PROVIDED') { echo '';}
						else if($mun['mun_name'] != NULL && $prov['prov_name'] != NULL){ echo trim($mun['mun_name']," ") .', '. $prov['prov_name'];}
						else{ echo trim($parent_marriage_place, "\t"); } ?>" />
