...
<table border=0 cellpadding=0 cellspacing=1>
  <tr class="reg_list_titlebar">
	  ...
	  <?php
		...
		echo $pagen->SortLink($LDSex,'sex',$odir,$flag,$IPBMextend); 
			 ?></b></td>
      <td><b>
	  <?php 
	  	if($oitem=='name_last') $flag=TRUE;
			else $flag=FALSE; 
		echo $pagen->SortLink($LDLastName,'name_last',$odir,$flag,$IPBMextend); 
			 ?></b></td>
      <td><b>
	  <?php 
	  	if($oitem=='name_first') $flag=TRUE;
			else $flag=FALSE; 
		echo $pagen->SortLink($LDFirstName,'name_first',$odir,$flag,$IPBMextend); 
			 ?></b></td>
      <td><b>
	  <?php 
		...
		echo $pagen->SortLink($LDBday,'date_birth',$odir,$flag,$IPBMextend); 
			 ?></b></td>
      <td align='center'><b>
		<?php 
		if($oitem=='brgy_name') $flag=TRUE;
			else $flag=FALSE;
		echo $pagen->SortLink('Barangay','brgy_name',$odir,$flag,$IPBMextend); 		 	
		?></b></td>
      <td align='center'><b>
		<?php 
		if($oitem=='mun_name') $flag=TRUE;
			else $flag=FALSE;
		echo $pagen->SortLink('Muni/City','mun_name',$odir,$flag,$IPBMextend); 		 	
		?></b></td>
      <td align='center'><b>
	  <?php 
...
#		 echo $pagen->SortLink($LDZipCode,'addr_zip',$odir,$flag); 		 	
		...
		echo $pagen->SortLink($LDZipCode,'zipcode',$odir,$flag,$IPBMextend); 		 	
		?></b></td>
      <td align="center"><b>
	  <?php 
	  	if($oitem=='pid') $flag=TRUE;
			else $flag=FALSE; 
		echo $pagen->SortLink($LDRegistryNr,'pid',$odir,$flag,$IPBMextend); 
			 ?></b></td>
      <td><b>
	  <?php 
	  	if($oitem=='date_reg') $flag=TRUE;
			else $flag=FALSE; 
		echo $pagen->SortLink($LDRegDate,'date_reg',$odir,$flag,$IPBMextend); 
			 ?></b></td>
  </tr>
<?php 
	# Load common icons
	...
	while($result=$ergebnis->FetchRow()){
        ...
        }elseif($enc_row['encounter_type']==14){
          $label =  '<img '.createComIcon($root_path,'flag_blue.gif').'>'.
                '<font size=1 color="blue">IPBM-OPD</font>';
        }elseif($enc_row['encounter_type']==13){
...
#			 <td>&nbsp; &nbsp;'.@formatDate2Local($result['date_birth'],$date_format).'</td>
	...
	#    <td align=right>&nbsp; &nbsp;'.$result['addr_zip'].'</td>
		...
		echo '
			<tr><td colspan=8>'.$pagen->makePrevLink($LDPrevious,$IPBMextend).'</td>
			<td align=right>'.$pagen->makeNextLink($LDNext,$IPBMextend).'</td>
			</tr>';
...
{ $IPBMspecialextension = ($isIPBM)?"?from=ipbm":"";
?>

<form method="post" action="<?php echo $thisfile.$IPBMspecialextension; ?>" name="aufnahmeform">
