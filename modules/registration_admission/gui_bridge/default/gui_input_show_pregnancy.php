...
#edited by VAN 02-21-08
#if($pregnancy['delivery_time']) $TP_PTIME=$pregnancy['delivery_time'];

$TP_TIMEhr_OPTIONS='';
for($i = 1; $i <= 12; $i++){
	$TP_TIMEhr_OPTIONS.='<option value='.$i.'';
	if(date("h",strtotime($pregnancy['delivery_time']))==$i) $TP_TIMEhr_OPTIONS.=' selected';
	$TP_TIMEhr_OPTIONS.='>'.date("h",strtotime($i)).'</option>';
}
...
for($i = 0; $i < 60; $i++){
	if ($i<10)
		$i = '0'.$i;
	$TP_TIMEmin_OPTIONS.='<option value='.$i.'';
	if(date("i",strtotime($pregnancy['delivery_time']))==$i) $TP_TIMEmin_OPTIONS.=' selected';
	$TP_TIMEmin_OPTIONS.='>'.$i.'</option>';
}

$meridime = array("AM", "PM");
$TP_TIMEmer_OPTIONS='';
while (list($x,$v) = each($meridime)){
	$TP_TIMEmer_OPTIONS.='<option value='.$v.'';
	if(date("A",strtotime($pregnancy['delivery_time']))==$v) $TP_TIMEmer_OPTIONS.=' selected';
	$TP_TIMEmer_OPTIONS.='>'.$v.'</option>';
}
