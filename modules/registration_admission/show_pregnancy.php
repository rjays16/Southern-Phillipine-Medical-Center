...
#include($root_path.'include/inc_DateDDLGenerator.class.php');
#$dategen = new DateGenerator;	

if(!isset($allow_update)) $allow_update=FALSE;

	#edited by VAN 02-21-08
	$deliver_time = $HTTP_POST_VARS['delivery_hr'].":".$HTTP_POST_VARS['delivery_min'].":00 ".$HTTP_POST_VARS['delivery_mer'];
	
	if ($HTTP_POST_VARS['delivery_hr']){
		$HTTP_POST_VARS['delivery_time'] = date("H:i:s",strtotime($deliver_time));
	}else{
		$HTTP_POST_VARS['delivery_time']=date('H:i:s');
	}
