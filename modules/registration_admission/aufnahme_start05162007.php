							...
							$itemno=$itemname;		
								...
								#------------edited by vanessa 03-27-07--------
									...
									if (($referrer_dr==NULL)||($dr_dept['personell_nr']==NULL)){
										...
										$HTTP_POST_VARS['referrer_dr'] = $dr_dept['personell_nr'];
										$HTTP_POST_VARS['referrer_dept'] = $dr_dept['nr'];
...
*/		
	...
	#			$smarty->assign('referrer_diagnosis','<input name="referrer_diagnosis" type="text" size="60" value="'.ucwords(strtolower(trim($referrer_diagnosis))).'">');   # burn commented: April 20, 2007
				$smarty->assign('referrer_diagnosis','<textarea name="referrer_diagnosis" id="referrer_diagnosis" cols="60" rows="3" wrap="VIRTUAL">'.ucwords(strtolower(trim($referrer_diagnosis))).'</textarea>');   # burn added: April 20, 2007
			}
			if ($dept_belong['dept_nr']==148){   # burn added : May 16, 2007
					# the user is from Admitting section
				$smarty->assign('segERDiagnosis',$LDDiagnosis);
				$smarty->assign('er_diagnosis','<textarea name="er_diagnosis" id="er_diagnosis" cols="60" rows="3" wrap="VIRTUAL">'.ucwords(strtolower(trim($er_diagnosis))).'</textarea>');   # burn added: May 16, 2007
				$smarty->assign('LDDiagnosis',"Referred ".$LDDiagnosis);
				$smarty->assign('referrer_diagnosis','<span style="text-align:justify">'.ucwords(strtolower(trim($referrer_diagnosis))).'</span>');
			}
...
#			$smarty->assign('referrer_recom_therapy','<textarea name="referrer_recom_therapy" id="referrer_recom_therapy" cols="60" rows="3" wrap="VIRTUAL">'.ucwords(strtolower(trim($referrer_recom_therapy))).'</textarea>');   # burn added: April 20, 2007
			...
			if ((($patient_enc['encounter_type']==1)||($patient_enc['encounter_type']==2))&&($update==1)&&($dept_belong['id']=="Admission")){
				$smarty->assign('referrer_dr_name','<input name="referrer_dr_name" onKeyUp="ReferralHandler1()" type="text" size="60" value="">');
				$smarty->assign('referrer_dr','<input name="referrer_dr" type="hidden" size="5" value="">');
			}else{
				...
				if (is_numeric($referrer_dr)){
					$smarty->assign('referrer_dr_name','<input name="referrer_dr_name" type="text" onKeyUp="ReferralHandler1();"  onBlur="GetDr_nr();" size="60" value="'.trim($ref_dr_name).'">');
					$smarty->assign('referrer_dr','<input name="referrer_dr"  type="hidden" size="5"  value="'.$patient_enc['referrer_dr'].'">');
					$smarty->assign('referrer_dept','<input name="referrer_dept" type="hidden" size="5" value="'.$patient_enc['referrer_dept'].'">');
				}else{
					$smarty->assign('referrer_dr_name','<input name="referrer_dr_name" type="text" onKeyUp="ReferralHandler1();" onBlur="GetDr_nr();" size="60" value="'.ucwords(strtolower(trim($referrer_dr))).'">');
					$smarty->assign('referrer_dr','<input name="referrer_dr"  type="hidden" size="5"  value="">');
...
#			$smarty->assign('referrer_notes','<input name="referrer_notes" type="text" size="60" value="'.ucwords(strtolower(trim($referrer_notes))).'">');
			$smarty->assign('referrer_notes','<textarea name="referrer_notes" id="referrer_notes" cols="60" rows="2" wrap="VIRTUAL">'.ucwords(strtolower(trim($referrer_notes))).'</textarea>');   # burn added: May 16, 2007
