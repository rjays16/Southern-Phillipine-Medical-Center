...
if(isset($target)) {
   switch ($target)
	{
	    case 'insurance' :   
#                           $sql="SELECT name AS \"insurance_firm_name2.value\", firm_id AS \"insurance_firm_id.value\", use_frequency FROM care_insurance_firm";
                           $sql="SELECT name AS \"insurance_firm_name.value\", firm_id AS \"insurance_firm_id.value\", use_frequency, hcare_id FROM care_insurance_firm";
									if($mode=='search') {
		...
		case 'ethnic_orig' :    $sql="SELECT name AS \"ethnic_orig_txt.value\" ,nr AS \"ethnic_orig.value\", 0 AS use_frequency FROM care_type_ethnic_orig ";
		                         if($mode=='search') {
											...
											$sql.=" WHERE name REGEXP '[[:<:]]$searchkey' ";
										} else {
										    $sql.=" ORDER BY name DESC";
											 $limitselect=TRUE;
										}
...
function setValue(val) {
	...
	//alert(" array_val[0]= "+ array_val[0] +" \n array_val[1]="+array_val[1]+"\n array_val[2]="+array_val[2]); /* burn added: August 23, 2006 */
	<?php
...
<?php
if($mode=='search')  {    
	 #echo "linecount b4 = ".$linecount;
    if(!$linecount) $linecount=0;
	 #echo "<br>linecounte = ".$linecount;
    #echo '<hr width=80% align=left>'.str_replace("~nr~",$linecount,$LDSearchFoundData).'<p>';
	 #edited by VAN 02-12-08
	 echo '<hr width=80% align=left>'.str_replace("~no.~",$linecount,$LDSearchFoundData).'<p>';
} else {
         ...
         $count=0;
					...
					for($i=0;$i<sizeof($sql_value);$i++)
					{
						...
						while(list($x,$v)=each($sql_value_without_key[$i])){
							#print_r($sql_value_without_key[$i]);
							#echo "<br>x = ".$x;
							#echo "<br>v = ".$v;
