...
function startAJAXSearch(searchID, page,pid) {
		...
		if (searchEL) {
				...
				AJAXTimerID = setTimeout("xajax_populateMedAbstEncRef('"+pid+"','"+searchID+"',"+page+",'"+keyword+"',true,"+isIPBM+")",100);
...
function addProductToList(listID, pid, id, date_prepared, enc_nr, ref_nr, date_admitted, dept, prepared_by,abs_nr,dr) {
...
//alert('sulod listID, id, name, cash, charge, sservice = '+listID+" , "+id, name+" , "+cash+" , "+charge+" , "+sservice);
			...
			if (id) {
				 ...
				 var reagent ='onclick="return overlib(OLiframeContent(\'seg-patient-medical_abstract.php?encounter_nr='+enc_nr+'&referral_nr='+ref_nr+'&showBrowser=1&abs_nr='+abs_nr+IPBMextend+'\', 750, 380, \'flab-list\', 1, \'auto\'), ' +
									'WIDTH, 380, TEXTPADDING, 0, BORDER, 0, STICKY, SCROLL, CLOSECLICK, MODAL, CLOSETEXT, \'<img src=../../images/close.gif border=0 >\', '+
									'CAPTIONPADDING, 4, CAPTION, \'Medical Certificate\', MIDX, 0, MIDY, 0, STATUS, \'Medical Abstract\');">';
					
					var reagentlink = '<td align="center"><a href="javascript:void(0);" '+reagent+'<img src="../../images/cashier_edit.gif" border="0"></a>&nbsp;<a href="javascript:void(0);" onClick="deleteAbst(\''+enc_nr+'\',\''+abs_nr+'\',\''+pid+'\');"><img src="../../images/cashier_delete_small.gif" border="0"></a></td>';
					...
					rowSrc = '<tr><td colspan="8" style="">No encounters with medical abstract...</td></tr>';
			}
...
function NewMedabst(pid){
	IPBMextend = "<?=$IPBMextend?>";

		return overlib(OLiframeContent('med_abst_encounters.php?pid='+pid+IPBMextend, 790, 380, 'historyList', 1, 'auto'),
...
function deleteAbst(encounter_nr, abst_nr, pid){
	
	var answer = confirm("Are you sure you want to delete the medical abstract no. "+(abst_nr)+"?");
	if (answer){
		xajax_deleteAbstract(encounter_nr, abst_nr, pid);
	}
}
// -->
</script>
...
?>
 <table width="98%" cellspacing="2" cellpadding="2" style="margin:0.7%">
				<tbody>
						...
						<tr>
								<td> <input type="image" value="Add New Medical Abstract" src="<?=$root_path?>images/en_create_new_medical_abst.png" onclick="javascript:NewMedabst(<?=$pid?>)">
								</td>
						</tr>
				</tbody>
		</table>
