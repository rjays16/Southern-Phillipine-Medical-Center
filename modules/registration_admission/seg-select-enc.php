...
$user_from = $_GET['var_from']; #added by Christian 11-18-19

if ($getcase || $user_from) 
{
	$case = "Case no:";
	...
	$case_no ='<input id="search_caseno" class="segInput" type="text" style="width:20%; font: bold 12px Arial" align="absmiddle" onBlur="DisabledSearch();" onKeyUp="DisabledSearch(); if ((event.keyCode == 13)&&(isValidSearch(document.getElementById(\'search_caseno\').value))) startAJAXSearch(\'search\',0); return false;;"/>';
}// end bby shandy
...
function startAJAXSearch(searchID, page, e) {
	...
	if (paramGetCase  || (user_from && $('search_caseno').value !='') ) { //Updated by Christian 11-19-19
	...
	}else if ((searchEL && searchEL.value.length >= 3) || user_from) { //Updated by Christian 11-19-19
...
function isValidSearch(key) {
       ...
       var paramGetCase = '<?=$getcase?>';
       var user_from = '<?=$user_from?>'; //Updated by Christian 11-21-19
       if(paramGetCase || user_from) {// -- CASE Number only ---------------------
...
//Added by Christian 11-21-19
function isValidSearchCaseHrn(key){ //independent validity Case number checker when HRN is available
		var s=key.toUpperCase();
			return (
		        /^\d{1,2}\/\d{1,2}\/\d{4}$/.test(s) ||
		        /^\d{1,2}\-\d{1,2}\-\d{4}$/.test(s) ||
...
//Updated by Christian 11-19-19
function DisabledSearch(){
	 var paramGetCase = '<?=$getcase?>';
	 var user_from = '<?=$user_from?>';
	 var skey =$('search').value;
		if (paramGetCase||user_from) {
			...
			var scase_no = $('search_caseno').value;
			    ...
			    else if (user_from && scase_no != '' && skey != ''){
			    	var a=isValidSearch(document.getElementById('search').value);
			    	var c=isValidSearchCaseHrn(document.getElementById('search_caseno').value);
			    	if (a==c)
			    		b=true;
			    	else
			    		b=false;
