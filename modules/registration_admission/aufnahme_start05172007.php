...
*/		}else if(d.current_dept_nr.value==0){		
			...
			return false;
			consulting_dr_nr
		}else if(d.er_diagnosis.value==""){
			alert("Please enter Admitting Diagnosis.");
			d.er_diagnosis.focus();
			return false;
...
//alert("hello 1");	
				...
				xajax_setDoctorsER(0,dept_nr);		// get all OPD doctors; burn added: May 16, 2007
			}else{
//alert("hello 2");	
				xajax_setDoctors(0,0);		// get all OPD doctors
			}
...
//alert("hello 3");					
			...
			//}else{
				xajax_setDoctors(1,0);		// get all IPD doctors
				xajax_setDoctorsER(1,dept_nr);		// get all IPD doctors; burn added: May 16, 2007
			}		
...
echo "LDDoctor3 = '".$LDDoctor3."'<br> \n";
				...
				if ($dept_belong['dept_nr']==148){   # burn added : May 16, 2007
					...
					$smarty->assign('sERDeptInput','<input name="consulting_dept_nr" id="consulting_dept_nr" type="hidden" value="'.$patient_enc['consulting_dept_nr'].'">'.$patient_doc['name_formal']);
...
#					else $smarty->assign('LDDepartment',"$LDClinic/$LDDepartment");   # burn commented : May 16, 2007
		...
		if ($dept_belong['id']!="OPD-Triage"){  	
			...
			if ($dept_belong['dept_nr']==148){   # burn added : May 16, 2007
				...
				$smarty->assign('er_diagnosis','<textarea name="er_diagnosis" id="er_diagnosis" cols="55" rows="3" wrap="VIRTUAL">'.ucwords(strtolower(trim($er_diagnosis))).'</textarea>');   # burn added: May 16, 2007
			}# end of if-stmt "if ($dept_belong['dept_nr']==148)"
...
#			$smarty->assign('referrer_recom_therapy','<textarea name="referrer_recom_therapy" id="referrer_recom_therapy" cols="60" rows="3" wrap="VIRTUAL">'.ucwords(strtolower(trim($referrer_recom_therapy))).'</textarea>');   # burn added: April 20, 2007
	...
	#			$smarty->assign('referrer_notes','<input name="referrer_notes" type="text" size="60" value="'.ucwords(strtolower(trim($referrer_notes))).'">');
			...
			if ((($patient_enc['encounter_type']==1)||($patient_enc['encounter_type']==2))&&($update==1)&&($dept_belong['id']=="Admission")){
				$smarty->assign('referrer_dr_name','<input name="referrer_dr_name" onKeyUp="ReferralHandler1()" type="text" size="73" value=""'.$readonly.'>');
				...
				if (is_numeric($referrer_dr)){
					$smarty->assign('referrer_dr_name','<input name="referrer_dr_name" type="text" onKeyUp="ReferralHandler1();"  onBlur="GetDr_nr();" size="73" value="'.trim($ref_dr_name).'"'.$readonly.'>');
					...
					$smarty->assign('referrer_dr_name','<input name="referrer_dr_name" type="text" onKeyUp="ReferralHandler1();" onBlur="GetDr_nr();" size="73" value="'.ucwords(strtolower(trim($referrer_dr))).'"'.$readonly.'>');
					$smarty->assign('referrer_dr','<input name="referrer_dr"  type="hidden" size="5"  value="">');
