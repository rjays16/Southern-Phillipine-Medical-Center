...
<script type="text/javascript">
	...
	function showRequest(ref, dept, pid) {
		...
		switch($('mode').value.toLowerCase()) {
			...
			break
			default:
				window.location.href = "seg-cashier-main.php<?= URL_APPEND ?>&clear_ck_sid=$clear_ck_sid&mode=edit&ref="+ref+"&dept="+dept+"&pid="+pid;
			break;
		}
	}
...
#echo "<pre>".$cClass->getQuery()."</pre>";
		...
		$fully_paid=TRUE;
			...
			if ($item[0]=='NULL')
				...
				if(!$fully_paid && $row['source_dept']=='OB'){
					$hasManualPayment = $cClass->hasManualPay($row['reference_no']);
					if($hasManualPayment){
						 $fully_paid = TRUE;
					}
				}
