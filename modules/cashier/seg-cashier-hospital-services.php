...
require('./roots.php');
require($root_path."modules/cashier/ajax/hospital-services.common.php");
...
if ($_GET['type'])
	$smarty->assign('sOnLoadJs','onLoad="xajax_populateServiceList(\'search\',\'\',$(\'type\').value,0);$(\'search\').focus()"');
else
...
function startAJAXSearch(searchID, forceSearch, page) {
	...
	if ((searchEL && lastSearch != searchEL.value) || forceSearch) {
		...
		AJAXTimerID = setTimeout("xajax_populateServiceList('"+searchID+"','"+searchEL.value+"',$('type').value,'"+page+"')",200);
...
<script type="text/javascript" src="<?=$root_path?>modules/cashier/js/hospital-services.js?t=<?=time()?>"></script>
