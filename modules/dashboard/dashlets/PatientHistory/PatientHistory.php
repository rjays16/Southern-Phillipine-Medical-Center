...
*/
class PatientHistory extends Dashlet {

	protected static $name 	= 'Previous Cases';
	...
	*/
	public function render($renderParams=null) {
		...
		if ($mode->is(DashletMode::VIEW_MODE))
		{
			...
			return $smarty->fetch($root_path.'modules/dashboard/dashlets/PatientHistory/templates/ListView.tpl');
		}
		elseif ($mode->is(DashletMode::EDIT_MODE))
		{
			...
			$smarty->assign('settings', $preferencesSmarty);
			return $smarty->fetch($root_path.'modules/dashboard/dashlets/PatientHistory/templates/Config.tpl');
		}
		else
		{
