...
#echo "<br>dept_belong = ".$dept_obj->sql."<br>";
#print_r($dept_belong['job_function_title']);
...
<!--
<script type="text/javascript" src="<?=$root_path?>js/dojo/dojo.js"
        djConfig="parseOnLoad: true"></script>
-->		  
...
#commented and transferred by VAN 02-28-08
/*
...
#echo "<br>type a = ".$encounter_type_a;	
...
# Create a new form for data entry################### 
	...
	*/	
	...
	/*
	echo "<br>cond = ";
	print_r($patient_enc_cond);	
	echo "<br>disp = ";
	print_r($patient_enc_disp);	
	echo "<br>res = ";
	print_r($patient_enc_res);	
	*/
	/*if(!empty($HTTP_SESSION_VARS['sess_login_userid']))
		$seg_user_name = $HTTP_SESSION_VARS['sess_login_userid'];
	else
...
*/
	...
	#echo "<br>gui_show_medocs : encounter_type, enc nr, enctype = ".$encounter_type." - ".$encounter_class_nr." - ".$encounter_type_a;
	#echo "<br>gui_show_medocs : is_discharged = ".$is_discharged;
	#added by VAN 02-27-08
	#edited by VAN 03-27-08
	#if($userDeptInfo['dept_nr'] != 151){
	#only ER and Medical Records can discharged a patient
	if(($userDeptInfo['dept_nr'] == 149)||($userDeptInfo['dept_nr'] == 151)){
		if (($userDeptInfo['dept_nr'] == 151))
			...
			$smarty->assign('sCheckDischarge','<input type="checkbox" id="isdischarge" name="isdischarge" value="1" onClick="unhideObject();"> <strong>To be Discharged?</strong>');	
	}
	...
	if($userDeptInfo['dept_nr'] == 151){
		...
		//echo "type = ".$encounter_type_a." - ".$encounter_type;
			...
			*/
			...
			if ($encounter_type==2){
				$smarty->assign('sDocLabel','Consulting'); //discharged department
				#$smarty->assign('sSetDeptDischarged',false); //discharged department
			}else{
				$smarty->assign('sDocLabel','Attending'); //discharged department
				#$smarty->assign('sSetDeptDischarged',true); //discharged department	
			}			
			...
			if ((stristr($dept_belong['job_function_title'], 'head') === FALSE)){
				#echo "not head";
				#echo "enctype = ".$encounter_type;
				#one time discharge only.. Only head can edit or update any info
				if ($is_discharged)
	...
	//User is from Admitting section department	
		...
		if($encounter_type ==1 && $encounter_class_nr == 1){
			...
			$smarty->assign('txtAreaDiagnosis','<textarea name="aux_notes" id="aux_notes" cols="75" rows="3" wrap="physical" readonly="readonly">'.trim($aux_notes_d).'</textarea>');
		...
		*/
			...
			*/
			...
			$setHidden = false; //unhide image save
			
			$smarty->assign('sSetConsult',true); //show consulting doctors & departments.
			#$smarty->assign('sSetResult',false); //Show Result row and Disposition row
			#$smarty->assign('sSetResult',true); //Show Result row and Disposition row van
			#$smarty->assign('sSetCon',false); //Hide Condition row
			#$smarty->assign('sSetCon',true); //Hide Condition row van
			$smarty->assign('sSetDeptDiagnosis',true); //show Select Doctors and Departments for diagnosis
			$smarty->assign('sSetDeptTherapy',true); //show # 2 Select Doctors and Departments for therapy
			...
			#edited by VAN 02-27-08
			#echo "type, type_a = ".$encounter_type.' - '.$encounter_type_a;
			#if ($encounter_type==2 || $encounter_type==4 || $encounter_type_a==2 || $encounter_type_a==4){
			if ($encounter_type_a==2){
	...
	if($encounter_type_a!=2){
		...
		if(is_object($rowDisp)){
			...
			$count=0;
			#edited by VAN 02-27-08
			if(($userDeptInfo['dept_nr'] == 149)||($userDeptInfo['dept_nr'] == 148)){
				#added by VAN 02-28-08
		...
		#echo "<br>rowDispA = ".$rowDispA;
		#echo "<br>rowDispB = ".$rowDispB;
...
#	$smarty->assign('sFormatTime','onblur="setFormatTime()"');   # burn commented : June 6, 2007
	...
	#echo "enc_Info['er_opd_admitting_dept_name'] = ".$enc_Info['er_opd_admitting_dept_name'];
	#print_r($enc_Info);
...
if ($mode=='show'||$mode=='details'){
	...
	/*	$smarty->assign('sTailScripts','<script language="javascript">xajax_populateCode("'.$encounter_nr.'","'.$encounter_type.'","icd");xajax_populateCode("'.$encounter_nr.'","'.$encounter_type.'","icp");</script>');*/
		...
		*/
		#if (!$discharged){
		if ((!$discharged)||($userDeptInfo['dept_nr'] == 151)){
			$smarty->assign('sHideNewRecLink',true);
			$smarty->assign('sNewLinkIcon','<img '.createComIcon($root_path,'bul_arrowgrnlrg.gif','0','absmiddle').'>');
			$lnk="<a href=".$thisfile.URL_APPEND."&pid=".$HTTP_SESSION_VARS['sess_pid']."&encounter_nr=".$HTTP_SESSION_VARS['sess_en']."&target=".$target."&tabs=".$tabs."&mode=new&type_nr=".$type_nr."&is_discharged=".$enc_obj->Is_Discharged($encounter_nr)."&encounter_type=".$encounter_type."&encounter_type_a=".$encounter_type_a."&encounter_class_nr=".$encounter_class_nr.">Edit Record </a>";
			$smarty->assign('sNewRecLink','<span id="enterNewRecord">'.$lnk.'</span>');
...
if(($mode!='show'&&!$nolist) ||($mode=='show'&&$nolist&&$rows>1)){
	...
	#$smarty->assign('sListRecLink','<a href="'.$thisfile.URL_APPEND.'&pid='.$HTTP_SESSION_VARS['sess_pid'].'&encounter_nr='.$HTTP_SESSION_VARS['sess_en'].'&target='.$target.'&mode=show&type_nr='.$type_nr.'&encounter_class_nr = '.$encounter_class_nr.'&encounter_type = '.$encounter_type.'">'.$LDShowDocList.'</a>');
