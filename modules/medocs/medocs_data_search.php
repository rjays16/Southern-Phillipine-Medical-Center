...
#added ic encounter type '6' by art 02/25/2014
if($allow_ipbmMedocs_user && $isIPBM){
	$sql_type = 'AND (enc.encounter_type IN ('.IPBMIPD_enc.','.IPBMOPD_enc.'))';
}else{
	if (($allow_accessER)&&($allow_accessOPD)&&($allow_accessIPD)){
    $sql_type = 'AND (enc.encounter_type IN (1,2,3,4,6,'.IPBMIPD_enc.','.IPBMOPD_enc.'))';
	}elseif (($allow_accessER)&&($allow_accessOPD)){
    $sql_type = 'AND (enc.encounter_type IN (1,2,6,'.IPBMOPD_enc.'))';
	}elseif (($allow_accessER)&&($allow_accessIPD)){
    $sql_type = 'AND (enc.encounter_type IN (1,3,4,6,'.IPBMIPD_enc.'))';
	}elseif (($allow_accessOPD)&&($allow_accessIPD)){
    $sql_type = 'AND (enc.encounter_type IN (2,3,4,6,'.IPBMIPD_enc.','.IPBMOPD_enc.'))';    
	}elseif ($allow_accessOPD){
	$sql_type = 'AND (enc.encounter_type IN (2,'.IPBMOPD_enc.'))';
	}elseif ($allow_accessIPD){
	$sql_type = 'AND (enc.encounter_type IN (3,4,6,'.IPBMIPD_enc.'))';
	...
	if($mode=='search') $HTTP_SESSION_VARS['sess_searchkey']=$searchkey;
		...
		if(empty($GLOBAL_CONFIG['pagin_patient_search_max_block_rows'])) $pagen->setMaxCount(MAX_BLOCK_ROWS); # Last resort, use the default defined at the start of this page
			...
			if(is_numeric($suchwort))
			{
				...
				$suchbuffer=$suchwort;
				//Added OR FUNCTION (suchwort) to fix pagination Error Borj 2014-21-03  
				if ((isset($_POST['option_pid']) && $_POST['option_pid']) || isset($suchwort)){
			...
			$enctypecond = "enc.encounter_type<3 OR enc.encounter_type=".IPBMOPD_enc." OR enc.encounter_type=".IPBMIPD_enc;
			...
			$sql='SELECT enc.encounter_nr,
								...
								IF('.$enctypecond.',enc.encounter_date,enc.admission_dt) AS date,
								reg.pid,
								reg.fromtemp,
								reg.name_last, 
								reg.name_first, 
		...
		if ($isTemp){
			...
			elseif (is_numeric($key)){
				if (isset($_POST['option_pid']) && $_POST['option_pid']){
					...
					if ($person2['fromtemp']){
						if($allow_ipbmMedocs_user && $isIPBM){
							$sql4 = " OR (reg.pid = '".$key."' AND enc.encounter_type IN (".IPBMIPD_enc.",".IPBMOPD_enc.") AND reg.fromtemp=1) ";
						}else{
...
#echo "zeile : <br>"; print_r($zeile);  echo "<br> \n";
						...
						header('location:show_medocs.php'.URL_APPEND.'&from=such&pid='.$zeile['pid'].'&encounter_nr='.$zeile['encounter_nr'].'&target=entry&tabs='.$tabs.'&encounter_type='.$zeile['encounter_type'].'&encounter_class_nr='.$zeile['encounter_class_nr'].'fromtemp='.$zeile['fromtemp'].$IPBMextend);
...
if($isIPBM)
	$smarty->assign('sToolbarTitle',"Medical History");
else
...
<?php
                  ...
                  echo "</td>";
						...
						#edited by art 02/27/2014
						elseif($zeile['encounter_type']==6) 
							echo ' <img '.createComIcon($root_path,'redflag.gif').'> <font size=1 color="blue">IC</font>';
						elseif($zeile['encounter_type'] == IPBMIPD_enc)
							echo ' <img '.createComIcon($root_path,'redflag.gif').'> <font size=1 color="green">IPBM - IPD</font>';
						elseif($zeile['encounter_type'] == IPBMOPD_enc)
							echo ' <img '.createComIcon($root_path,'redflag.gif').'> <font size=1 color="blue">IPBM - OPD</font>';
						else	
						...
						if (empty($zeile['date']))
							#edited by art 02/27/2014
							if($zeile['encounter_type']==6){
								$encdate = formatDate2Local($zeile['encounter_date'],$date_format,1);
							}else{
...
#						echo "&nbsp;".formatDate2Local($zeile['date_birth'],$date_format);
                  ...
                  echo "</td>";	
						...
						<td>&nbsp;
							<a href=show_medocs.php'.URL_APPEND.'&from=such&pid='.$zeile['pid'].'&encounter_nr='.$zeile['encounter_nr'].'&target=entry&tabs='.$tabs.'&encounter_type='.$zeile['encounter_type'].'&encounter_class_nr='.$zeile['encounter_class_nr'].'&fromtemp='.$zeile['fromtemp'].$IPBMextend.'>
