...
if (in_array(SegRequest::POC_REQUEST, $request_types))
{
	$blueprints[SegRequest::POC_REQUEST] = Array(
		"coreTable" => "seg_lingap_entries_poc ld",
		"joins" => Array(
			"INNER JOIN seg_lingap_entries l ON l.id=ld.entry_id",
			"INNER JOIN care_person p ON p.pid=l.pid",
			"INNER JOIN seg_lab_services s ON s.service_code=ld.service_code",
		...
		"fields" => Array(
			"id"		=> "l.id",
			"costCenter" 	=> SegRequest::POC_REQUEST,
			"date" 		=> "l.entry_date",
			"pid" 		=> "l.pid",
			"controlNo" 	=> "l.control_nr",
			"ssNo" 		=> "l.ss_nr",
...
if (in_array(SegRequest::DIALYSIS_REQUEST, $request_types)) {
    $blueprints[SegRequest::DIALYSIS_REQUEST] = Array(
        "coreTable" => "seg_lingap_entries_dialysis d",
        "joins" => Array(
        	"INNER JOIN seg_lingap_entries l ON l.id = d.entry_id",
            "INNER JOIN care_person p ON p.pid = l.pid",
