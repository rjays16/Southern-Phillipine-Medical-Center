...
function cancelEntry($ssId) {
	global $db;
	$objResponse = new xajaxResponse();

	$referralId = $db->GetOne("SELECT id FROM seg_lingap_entries WHERE ss_nr=".$db->qstr($ssId));
	if (!$referralId) {
	...
	$referral = new SegLingapReferral($referralId);

	$db->StartTrans();
	if ($referral->cancel()) {
		$objResponse->call('refreshLists');
		$objResponse->alert('Referral successfully deleted...');
	}
	else {
		$db->FailTrans();
		$objResponse->call('cancelDelete',$ssId);
		$objResponse->alert('An error has occurred! Please contact your administrator!');
	}
	$db->CompleteTrans();
	return $objResponse;
