...
function startAJAXSearch(searchID, page) {
	...
	//if (searchEL && lastSearch != searchEL.value) {
		...
		$("ajax-loading").style.display = "";
		//AJAXTimerID = setTimeout("xajax_populateProductList('"+searchID+"','"+searchEL.value+"')",200);
		AJAXTimerID = setTimeout("xajax_populateProductList('"+searchID+"','"+searchEL.value+"','"+areas.value+"',"+page+")",100);
		lastSearch = searchEL.value;
	//}
}
	...
	<table width="98%" cellspacing="2" cellpadding="2" style="margin:0.7%">
		<tbody>
			<tr>
				<td style="font:bold 12px Arial; background-color:#e5e5e5; color: #2d2d2d" >
					<div style="padding:4px 2px; padding-left:10px; ">
						Hospital Service Area <select id="serv_areas" name="serv_areas" onChange="clearList('product-list'); addProductToList('product-list',0); enableSearch();">
			...
			<tr>
				<td style="font:bold 12px Arial; background-color:#e5e5e5; color: #2d2d2d" >
					<div style="padding:4px 2px; padding-left:10px; ">
						Search Item <input id="search" class="segInput" type="text" readonly="1" style="width:60%; margin-left:10px; font: bold 12px Arial" align="absmiddle" onkeyup="if (this.value.length >= 3) startAJAXSearch(this.id,0)" onKeyPress="checkEnter(event,this.id)" />
