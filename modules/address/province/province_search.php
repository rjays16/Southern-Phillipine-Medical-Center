...
$breakfile='province_manage.php'.URL_APPEND;
...
if(empty($oitem)) $oitem='prov_name';
...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&!empty($searchkey)){
	...
	//$address=$address_prov->searchActiveCityTown($searchkey);
	$address=$address_prov->searchLimitAddress($searchkey,$GLOBAL_CONFIG['pagin_address_search_max_block_rows'],$pgx,$oitem,$odir,TRUE);
	# Get the resulting record count
	$linecount=$address_prov->LastRecordCount();
	$pagen->setTotalBlockCount($linecount);
	# Count total available data
	if(isset($totalcount)&&$totalcount){
		$pagen->setTotalDataCount($totalcount);
	}else{
		$totalcount=$address_prov->searchCountAddress($searchkey);
		$pagen->setTotalDataCount($totalcount);
	}
...
# Title in toolbar
 $smarty->assign('sToolbarTitle',"$segProvince :: $LDSearch");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$segProvince :: $LDSearch");
...
<!--  The search mask  -->
	<table border=0 cellpadding=10 bgcolor="<?php echo $entry_border_bgcolor ?>">
     <tr>
       <td>
	   <?php 
	   		$searchprompt=$segProvinceSearchPrompt;		//replaced $LDSearchPrompt for clarity, 10-30-2007, fdp---
		...
		<tr class="wardlisttitlerow">
			<td><b>
				<?php 
                    ...
                    echo $pagen->SortLink("Province Code",'code',$odir,$flag);
                ?></b>
            </td>
			<td><b>
				<?php
					if($oitem=='prov_name') $flag=TRUE;
						else $flag=FALSE; 
					echo $pagen->SortLink($segProvinceName,'prov_name',$odir,$flag); 
				?></b>
			</td>
			<td><b>
				<?php 
					if($oitem=='region_name') $flag=TRUE;
						else $flag=FALSE; 
					echo $pagen->SortLink($segRegionName,'region_name',$odir,$flag); 
				?></b>
			</td>	
		</tr> 
<?php
...
* has 2 fields with same name. Added  $addr['3'] is the
* exact value.
*/
	$toggle=0;
...
?>
		<tr class="<?php echo $bgc ?>">
            <td><?php echo $prov_code; //$addr['code']; ?></td>
			<td>
				<a href="province_info.php<?php echo URL_APPEND.'&retpath=search&prov_nr='.$addr['prov_nr']; ?>">
					<?php echo $addr['prov_name']; ?>
				</a>
			</td>
			<td><?php echo $addr['region_name']; ?></td>
		</tr> 
<?php
	}# end of while loop
	echo '
