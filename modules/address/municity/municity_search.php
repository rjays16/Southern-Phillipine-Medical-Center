...
$breakfile='municity_manage.php'.URL_APPEND;
...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&!empty($searchkey)){
	...
	//$address=$address_municity->searchActiveCityTown($searchkey);
	$address=$address_municity->searchLimitAddress($searchkey,$GLOBAL_CONFIG['pagin_address_search_max_block_rows'],$pgx,$oitem,$odir,TRUE);
    //echo print_r($address);
	# Get the resulting record count
	$linecount=$address_municity->LastRecordCount();
	$pagen->setTotalBlockCount($linecount);
	# Count total available data
	if(isset($totalcount)&&$totalcount){
		$pagen->setTotalDataCount($totalcount);
	}else{
		$totalcount=$address_municity->searchCountAddress($searchkey);
		$pagen->setTotalDataCount($totalcount);
	}
...
# Title in toolbar
 $smarty->assign('sToolbarTitle',"$segMuniCity :: $LDSearch");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$segMuniCity :: $LDSearch");
...
<!--  The search mask  -->
	<table border=0 cellpadding=10 bgcolor="<?php echo $entry_border_bgcolor ?>">
     <tr>
       <td>
	   <?php 
	   		$searchprompt=$segMuniCitySearchPrompt;		//replaced $LDSearchPrompt for clarity, 10-30-2007, fdp---
		...
		<tr class="wardlisttitlerow">
			<td><b>
				<?php 
                    ...
                    echo $pagen->SortLink("Municipality/City Code",'code',$odir,$flag);
                ?></b>
            </td>
			<td><b>
				<?php
...
<?php
	...
	while($addr=$address->FetchRow()){
        ...
        $municity_code = $addr['4'];
		if($toggle) $bgc='wardlistrow2';
			else $bgc='wardlistrow1';
		$toggle=!$toggle;
?>
		<tr class="<?php echo $bgc ?>">
            <td><?php echo $municity_code; //$addr['code']; ?></td>
			<td>
				<a href="municity_info.php<?php echo URL_APPEND.'&retpath=search&mun_nr='.$addr['mun_nr']; ?>">
					<?php echo $addr['mun_name']; ?>
				</a>
			</td>
			<td><?php echo $addr['prov_name']; ?></td>
		</tr> 
<?php
	}# end of while loop
	echo '
