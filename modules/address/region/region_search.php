...
$breakfile='region_manage.php'.URL_APPEND;
...
if(empty($oitem)) $oitem='region_name';
...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&!empty($searchkey)){
	...
	$address=$address_obj->searchLimitAddress($searchkey,$GLOBAL_CONFIG['pagin_address_search_max_block_rows'],$pgx,$oitem,$odir);
	...
	if(isset($totalcount)&&$totalcount){
		...
		$totalcount=$address_obj->searchCountAddress($searchkey);
		$pagen->setTotalDataCount($totalcount);
	}
...
# Title in toolbar
 $smarty->assign('sToolbarTitle',"$segRegion :: $LDSearch");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$segRegion :: $LDSearch");
...
<!--  The search mask  -->
	<table border=0 cellpadding=10 bgcolor="<?php echo $entry_border_bgcolor ?>">
     <tr>
       <td>
	   <?php 
	   		$searchprompt=$segRegionSearchPrompt;	//replaced $LDSearchPrompt for clarity, 10-30-2007, fdp---
		...
		<tr class="wardlisttitlerow">
			<td><b>
				<?php 
                    ...
                    echo $pagen->SortLink("Region Code",'code',$odir,$flag);
                ?></b>
            </td>
			<td><b>
				<?php
					...
					echo $pagen->SortLink($segRegionShortName,'region_name',$odir,$flag); 
				?></b>
			</td>
			<td><b>
				<?php 
					if($oitem=='region_desc') $flag=TRUE;
						else $flag=FALSE; 
					echo $pagen->SortLink($segRegionName,'region_desc',$odir,$flag); 
				?></b>
			</td>	
		</tr> 
<?php
...
?>
		<tr class="<?php echo $bgc ?>">
            <td><?php echo $addr['code']; ?></td>
			<td>
				<a href="region_info.php<?php echo URL_APPEND.'&retpath=search&region_nr='.$addr['region_nr']; ?>">
					<?php echo $addr['region_name']; ?>
				</a>
			</td>
			<td><?php echo $addr['region_desc']; ?></td>
		</tr> 
<?php
	}# end of while loop
	echo '
