...
if ($result !== FALSE) {
	...
	while ($row = $result->FetchRow()) {
		...
		if($mode=="cashier_search") {
			...
			$delete_btn = '<button class="segButton" style="cursor: pointer;" onclick="deleteAgency(\''.$row['company_id'].'\'); return false;" title="Delete agency"><img src="../../gui/img/common/default/cancel.png"/>Delete</button>';
			$details_btn = '<button class="segButton" style="cursor: pointer;" onclick="openDetailsTray(\''.$row['company_id'].'\');return false;" title="Update agency details"><img src="../../gui/img/common/default/page_go.png"/>Details</button>';
