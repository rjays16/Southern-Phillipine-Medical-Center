...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL && searchEL.value.length >= 3) {
		...
		AJAXTimerID = setTimeout("xajax_populatePersonList('"+searchID+"','"+searchText+"',"+page+","+searchLastname+","+includeEnc+",0)",100);
...
function startAJAXSearch2(keyword, page) {
		...
		$("ajax-loading").style.display = "";
		$("person-list-body").style.display = "none";
		AJAXTimerID = setTimeout("xajax_populatePersonList('search','"+keyword+"',"+page+","+searchLastname+","+includeEnc+",0)",100);
		lastSearch = keyword;
	}
...
{
	return overlib(
		OLiframeContent('../../modules/registration_admission/patient_register.php?<?URL_APPEND?>&ptype=ic&from_ic=1&checkintern=1',
									700, 400, 'fGroupTray', 0, 'auto'),
									WIDTH,700, TEXTPADDING,0, BORDER,0,
										STICKY, SCROLL, CLOSECLICK, MODAL,
										CLOSETEXT, '<img src=<?=$root_path?>/images/close_red.gif border=0>',
								 CAPTIONPADDING,2, CAPTION,'Register New Patient',
								 MIDX,0, MIDY,0,
								 STATUS,'Register New Patient');
}

function loadDependent(){
		var keyword = document.getElementById('dep_pid').value;
		startAJAXSearch2(keyword, 0);
...
?>
				...
				<td style="font:bold 12px Arial; background-color:#e5e5e5; color: #2d2d2d" >
					<table width="95%" border="0" cellpadding="0" cellspacing="0" style="font:bold 12px Arial; color:#2d2d2d; margin:1%">
						<tr>
							...
							<td align="right">
									<button class="segButton" id="btnRegister" name="btnRegister" style="cursor:pointer;" onclick="openRegister();return false;"><img src="../../gui/img/common/default/user_add.png" border="0">Register Patient</button>
							</td>
						</tr>
						<!-- <tr>
							<td></td>
	...
	<input type="hidden" name="agency_id" id="agency_id" value="<?=$_GET["agency_id"]?>">
