...
class IC_Billing_Statement_Report extends RepGen {
	...
	function IC_Billing_Statement_Report($comp_id, $bill_nr) {
		global $db;                                  
	...
	function FetchData()
	{
		...
		if($result)
		{
			...
			{
				$employee_id = $row['encounter_nr'];
				$cutoff = $row['cutoff_date'];
				$patient_name = $row['fullname'];
				
				$enc_sql = "SELECT encounter_nr FROM seg_industrial_transaction WHERE refno=".$db->qstr($employee_id);

				$enc_result = $db->Execute($enc_sql);
				$enc_row = $enc_result->FetchRow();
				$enc_nr = $enc_row['encounter_nr'];
				...
				$charges_result = $objIC->getPatientExaminationsTotals($enc_nr);//added by Nick 05-19-2014
				$data = $charges_result->FetchRow();
				...
				DATE("m-d-Y", strtotime($cutoff)),strtoupper($patient_name),         
		...
		else
		{
			...
			$result = $objIC->checkDiscountBillNr($this->bill_nr, $this->comp_id);
			...
			if($result != FALSE)
			{
				...
				if($count != 0)
				{
					$result = $objIC->getDiscount($this->bill_nr, $this->comp_id);
...
$rep = new IC_Billing_Statement_Report($comp_id, $bill_nr);
$rep->AliasNbPages();
$rep->FetchData();
$rep->Report();
?>
