<?php
function populateAppliedOpsList($senc_nr, $sElem, $searchkey, $page, $b_drchrg, $dr_nr=0, $b_all=0){
		global $db;
...
//		$total_srv = $srv->countAppliedOP($senc_nr,$searchkey,$maxRows,$offset,(strcmp($b_drchrg, 'true') == 0 ? 1 : 0), $dr_nr);
		...
		$ergebnis=$srv->SearchAppliedOP($senc_nr,$searchkey,$maxRows,$offset,(strcmp($b_drchrg, 'true') == 0 ? 1 : 0), $dr_nr, $b_all);

		$total = $srv->countOPForApplication();  // fix for Bugzilla bug 68
...
//            }
						...
						$objResponse->addScriptCall("addAppliedOPtoList","procedure-list",trim($result["ops_code"]),trim($description_short),trim($result["description"]),
																				(is_null($result["op_date"])) ? '00-00-0000' : strftime("%m-%d-%Y", strtotime($result["op_date"])),
																				$result["rvu"],$result["multiplier"],$bselected,$result["entry_no"],$result["refno"], $result["group_code"], (strcmp($b_drchrg, 'true') == 0 ? 1 : 0));
				}#end of while
		} #end of if
