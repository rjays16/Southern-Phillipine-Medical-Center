...
#end art
$define_nbs_2 = Config::model()->findByPk('NBS2_PACKAGE_ID');
define('NBS2_PACKAGE_ID', $define_nbs_2);
define(ICU_TYPE, 8);
define('OUTPATIENT', 2);
...
class SOA extends FPDF {
    ...
    function __construct(){
        ...
        $this->editedfinalDiagnosis = stripcslashes(strtoupper($this->Billing->getEditedDiagnosis($this->encounter_nr)));
           if(!$this->finalDiagnosis){
                $this->finalDiagnosis = $this->editedfinalDiagnosis;
           }
        $this->otherDiagnosis = stripcslashes(strtoupper($this->Billing->getOtherDiagnosis($this->encounter_nr)));
        $this->editedotherDiagnosis = stripcslashes(strtoupper($this->Billing->getEditedOtherDiagnosis($this->encounter_nr)));
        if(!$this->otherDiagnosis){
              $this->otherDiagnosis = $this->editedotherDiagnosis;
        }
    ...
    function Header(){
        ...
        if ($this->Bill->billInfo['accommodation_type']) {
            ...
            if ($this->Bill->billInfo['opd_type'] == '0')
                ...
                $result = $db->GetOne("SELECT opd_name FROM seg_opdarea WHERE id = ".$this->Bill->billInfo['opd_type']." ORDER BY opd_code ASC");
            
                $reportTitle .= " - ".strtoupper(str_replace('-', '', $result));
        } else {
    ...
    function Footer(){
        $revison_details = explode("~",Config::model()->findByPk('soa2_revision_effectivity'));
        $this->SetY(-15);
        $this->SetFont('Arial', 'B', 8);
        $this->setX(10);
        $this->Cell(1, 4, "SPMC-F-BIL-11", "", 0, 'L');
        $this->setX(70);
        $this->Cell(1, 4, "Effectivity: ".$revison_details[0], "", 0, 'L');
        $this->setX(130);
        $this->Cell(1, 4, "Rev. ".$revison_details[1], "", 0, 'L');
        $this->SetFont('Arial', 'I', 8);
        $this->setXY(100, -18);
        $this->Cell(0, 10, 'Page ' . $this->PageNo() . ' of {nb}', 0, 0, 'R');
    }

    function addBillInfo($caption,$value,$isLabel=true,$w=array(30,20),$b=false,$exceed=0){



        if($caption != "First Case Rate")
            $w = (
                (isset($_GET['isPatientCopy']) && !$_GET['isPatientCopy']) ||
                    ($_GET['isPatientCopy'] && $caption == "Second Case Rate") ||
                    ($_GET['isPatientCopy'] && $caption == "Type of Hearing Test")
                ) ? $w : array(18,50);
        ...
        if($caption == 'Dept' && $exceed && $_GET['isPatientCopy'])
            $this->SetFont("Times", "", "7");

        $this->Cell($w[1], 4, $colon . $value, 0, 0, 'L');
        $this->Cell(65, 30, "", 0, 0, 'L');

        if(strpos($caption, 'Time of A') !== false && $exceed && !$_GET['isPatientCopy'])
    ...
    function setBillInfo(){
        ...
        $dataEhr = $ehr->billing_getRepetitivSession($data);
        $result = $dataEhr->status;
        $taken = array();
        $arr_start = array();
        $arr_end = array();
        $first = $this->Bill->encounterInfo['first'];
        $second = $this->Bill->encounterInfo['second'];
        $counterFirst = 0;
        $counterSecond = 0;

        foreach ($result as $key => $reps) {
            array_push($taken ,date('m/d/Y', strtotime($reps->session_start_date)));
            ...
            array_push(
                    $arr_end,
                    date('m/d/Y', strtotime($reps->session_end_date)).' '.date(
                            'h:i a',
                            strtotime($reps->session_end_time)
                    )
            );

            if($reps->rvs_code == $first) {
                $counterFirst++;
            }

            if($reps->rvs_code == $second) {
                $counterSecond++;
            }
        }

        $startFormat = min($arr_start);
        $endFormat = max($arr_start);

        $start = date('M d, Y h:i a', strtotime($startFormat));
        $end = date('M d, Y h:i a', strtotime($endFormat));

        $encounterType = $this->getEncounterInfo();
        $encType = $encounterType['encounter_type'];
        $phic = $encounterType['insurance']['insurance_nr'];
        ...
        if($encType == OUTPATIENT) {
            if($counterFirst > 1) {
                if ($phic) {
                    ...
                    $dateStart = $this->Bill->encounterInfo['encounter_date'];
                    $dateEnd = $_GET['billdate_display'];
                }
        ...
        $valueSpacing = 42;
        if(strlen($this->Bill->encounterInfo['dept_name']) > 27)
            $valueSpacing = 30;
        
        $this->SetY(-1);
        $this->addBillInfo('Case #', $this->encounter_nr);
        $this->addBillInfo('Bill Ref. #', $this->bill_nr,true,array($valueSpacing,20),false);
        $this->Ln(4);
        $this->addBillInfo('HRN', $this->Bill->encounterInfo['pid']);
        $billDate = (isset($_GET['isPatientCopy']) && !$_GET['isPatientCopy']) ? "Bill Date" : "Date";
        
        $this->addBillInfo($billDate, date('M d, Y', strtotime($this->Bill->billInfo['bill_dte'])),true,array($valueSpacing,20),false);
                $this->Ln(4);

        $exceed = 0;
        if(strlen($this->Bill->encounterInfo['dept_name']) > 32)
            $exceed = 1;

        $patientName = $this->Bill->encounterInfo['name_last'] . ', ' . $this->Bill->encounterInfo['name_first'] . ' ' . $this->Bill->encounterInfo['name_middle'];
        $this->addBillInfo('Name', strtoupper($patientName),true,array(30,20),false);
        $this->addBillInfo('Dept', $this->Bill->encounterInfo['dept_name'],true,array($valueSpacing,20),false,$exceed);
        $this->Ln(4);
        $street_name = trim($this->Bill->encounterInfo['street_name']);
        ...
        if (($baranggay == "NOT PROVIDED" || $baranggay == NULL) && $municipality == "NOT PROVIDED" && $zipcode == "" && $province == "NOT PROVIDED") {
            $baranggay = $street_name;
            if(strlen($street_name) >= 38) {
                $this->SetFont("Times", "", "7.2");
            }
        ...
        // if(!$this->isDialysis)
        $dateAdmission = (isset($_GET['isPatientCopy']) && !$_GET['isPatientCopy']) ? "Date & Time of Admission" : "Admitted";

        if(strlen("$municipality $zipcode, $province") > 41)
            $exceed = 1;

        $this->addBillInfo($dateAdmission, date('M d, Y h:i a', strtotime($dateStart)),true,array(42,20),false,$exceed);
        $this->addBillInfo($dateAdmission, date('M d, Y h:i a', strtotime($this->Bill->encounterInfo['encounter_date'])),true,array(42,20),false,$exceed);
        $this->Ln(4);
        ...
        $this->addBillInfo('', strtoupper("$municipality $zipcode $comma $province"));
        
        // $this->SetFont("Times", "B", "10");
        if(isset($_GET['deathdate']) && $_GET['deathdate'] === ""){
            if(isset($_GET['isPatientCopy']) && !$_GET['isPatientCopy']){
                $this->addBillInfo('Date & Time of Discharge', date('M d, Y h:i a', strtotime($dateEnd)),true,array(42,20),false);
            }else{
                $this->SetFont("Times", "B", "10");
                $this->addBillInfo('', $this->Bill->encounterInfo['isphic'] ? 'PHIC' : "No PHIC", false, array(42,20),true);
                $this->SetFont("Times", "", "10");
            }
        ...
        if ($this->Bill->encounterInfo['isMedicoLegal']) {
            ...
            $this->SetFont("Times", "B", "10");
                $this->addBillInfo('', $this->Bill->encounterInfo['isphic'] ? 'PHIC' : "No PHIC", false, array(42,20),true);
                $this->Ln(4);
                if(isset($_GET['isPatientCopy']) && !$_GET['isPatientCopy'] && (isset($this->Bill->encounterInfo['deathdate']) && $this->Bill->encounterInfo['deathdate'])){
                    $this->addBillInfo('Death Date', date('M d, Y h:i a', strtotime($this->Bill->encounterInfo['deathdate'])));
                }else
                    $this->addBillInfo("", "",false);
                
                $this->SetFont("Times", "B", "10");
                $this->addBillInfo('', 'Medico Legal', false,array(42,20));
                $this->SetFont("Times", "", "10");
            }else{
                $this->SetFont("Times", "B", "10");
                $this->addBillInfo('', 'Medico Legal', false,array(42,20));
            ...
            if(isset($_GET['isPatientCopy']) && !$_GET['isPatientCopy']){
                $this->Ln(4);
                if(isset($this->Bill->encounterInfo['deathdate']) && $this->Bill->encounterInfo['deathdate']){
                    $this->SetFont("Times", "B", "10");
                    $this->addBillInfo('Death Date', date('M d, Y h:i a', strtotime($this->Bill->encounterInfo['deathdate'])));
                    $this->SetFont("Times", "", "10");
                }else{
        ...
        if ( (isset($this->Bill->encounterInfo['deathdate']) && $this->Bill->encounterInfo['deathdate']) && $this->Bill->encounterInfo['deathdate'] && $this->isDialysis) {
            if(isset($_GET['isPatientCopy']) && $_GET['isPatientCopy']){
                $this->Ln(4);
                $this->SetFont("Times", "B", "10");
                $this->addBillInfo('Death Date', date('M d, Y h:i a', strtotime($this->Bill->encounterInfo['deathdate'])));
            ...
            if($encType == OUTPATIENT) {
                if($counterFirst > 1 || $counterSecond > 1) {
                    if ($phic) {
                        ...
                        usort($taken, "date_sort");
                        
                        $separated = implode("; ", $taken);

                        if($result) {
                            if($_GET['isPatientCopy'] =='0'){
                                $this->SetFont("Times", "", "10");
                                $this->Cell(30, 4, 'Sessions Taken',0 ,0 , 'L');
                                $this->SetFont("Times", "B", "10");
                                $this->Multicell(150, 4,": ".$separated);
                                $this->SetFont("Times", "", "10");
                            }else {
                                $this->SetFont("Times", "", "10");
                                $this->Cell(30, 4, 'Sessions Taken',0 ,0 , 'L');
    ...
    function addGroup($title,$value=null){
        ...
        if(isset($value)){
            ...
            // if($this->CLAIMED_DISCOUNT_HCI!=$this->TOTAL_DISCOUNT_HCI){
                ...
                // }
            // }
            if($accommodation_type=='1'&&!is_null($accommodation_mandatory_excess)){
    ...
    function addDetail($title,$value=null,$b=false,$wrap=false,$indent=0,$titleAlign='L'){
        ...
        if(isset($value)){
            ...
            if(strlen($title) > 50 && strlen($title) < 55)
            ...
            // if($this->CLAIMED_DISCOUNT_HCI!=$this->TOTAL_DISCOUNT_HCI){
                ...
                // }
            // }
            if($accommodation_type=='1'&&(!is_null($accommodation_mandatory_excess) && (!$this->Bill->encounterInfo['isphic']))){
                $DISCOUNT=$value-($accommodation_mandatory_excess*$accommodation_days);
            }else{
                $DISCOUNT=$value*$this->Bill->encounterInfo['applied_discount'];
            }
    ...
    function addSubTotal($title,$totalCharge=null,$totalDiscount=null,$totalCoverageFirst=null,$totalCoverageSecond=null,$totalExcess=null,$border='T',$titleAlign='R',$indent=0,$b=array(0,0,0,0,0),$wrap=false){
        ...
        $this->Cell($this->widths[1]+2, 4, isset($totalCharge) ? number_format($totalCharge, 2) : '', $border, 0, 'R');
        $this->Cell(2, 4, "", 0, 0, 'C');
        ...
        $this->Cell(($this->widths[3]/2)-2, 4, isset($totalCoverageFirst) ? number_format($totalCoverageFirst, 2) : '', $border, 0, 'R');
    ...
    function showAccommodation(){
        ...
        foreach ($this->Bill->accommodation as $key => $acc) {
            ...
            $per_hour = $acc->isRoomRatePerHour();
            $hrs_label = "hr";
            if ($acc->n_hrs > 0 && $per_hour) {
                $days_total = 0;
                $hours_total = 0;
                
                if ($acc->n_hrs > 1) $hrs_label .= "s";
                
                $andHrs = " & $acc->n_hrs ".$hrs_label;
                if($acc->n_days)
                    $days_total = $acc->room_rate * $acc->n_days;
                
                $rate_per_hour = number_format(($acc->room_rate/24), 2);
                $hours_total = $acc->n_hrs * $rate_per_hour;
                $accomComputation = $days_total + $hours_total;
            }
            else{
                $andHrs = " ";
                $accomComputation = $acc->n_days * $acc->room_rate;
            }
            $days_label = "day";
            if ($acc->n_days > 1) {
                $days_label .= "s";
            ...
            $this->addDetail($accommodation_details, $accomComputation);
            //end gelie
        }
            // echo "<pre>" . print_r($this->Bill->accommodation,true) . "</pre>";
    ...
    function showXlo(){
        $LabTotal=0.00;$RadTotal=0.00;$RegSupTotal=0.00;$ConSupTotal=0.00;$OthTotal=0.00;
        if ($this->isDetailed) {
            ...
            if (count($this->Bill->xlo['laboratories'])) {
                foreach ($this->Bill->xlo['laboratories'] as $key => $xlo) {
                    ...
                    $LabTotal+=($xlo['qty'] * $xlo['serv_charge']);
                }
            }
            $this->addGroupTotal('Laboratories', $LabTotal,$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,($LabTotal-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            ...
            if (count($this->Bill->xlo['radiologies'])) {
                foreach ($this->Bill->xlo['radiologies'] as $key => $xlo) {
                    ...
                    $price = number_format($xlo['serv_charge'], 2);
                    $this->addDetail($xlo['qty'] . " @ " . $price, $xlo['qty'] * $xlo['serv_charge']);
                    $RadTotal+=($xlo['qty'] * $xlo['serv_charge']);
                }
            }
            $this->addGroupTotal('Radiologies', $RadTotal,$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,($RadTotal-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            $this->addDetail('Supplies', null, true);
            $this->addDetail('Regular Supplies', null, true);
            if (count($this->Bill->xlo['supplies'])) {
                foreach ($this->Bill->xlo['supplies'] as $key => $xlo) {
                    $this->addDetail($xlo['service_desc'], null, false, true);
                    $price = number_format($xlo['serv_charge'], 2);
                    $this->addDetail($xlo['qty'] . " @ " . $price, $xlo['qty'] * $xlo['serv_charge']);
                    $RegSupTotal+=($xlo['qty'] * $xlo['serv_charge']);
                }
            }
            $this->addGroupTotal('Regular Supplies', $RegSupTotal,$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,($RegSupTotal-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            $this->addDetail('Consigned Supplies', null, true);
            if (count($this->Bill->xlo['supplies_consigned'])) {
                foreach ($this->Bill->xlo['supplies_consigned'] as $key => $xlo) {
                    $this->addDetail($xlo['service_desc'], null, false, true);
                    $price = number_format($xlo['serv_charge'], 2);
                    $this->addDetail($xlo['qty'] . " @ " . $price, $xlo['qty'] * $xlo['serv_charge']);
                    $ConSupTotal+=($xlo['qty'] * $xlo['serv_charge']);
                }
            }
            $this->addGroupTotal('Consigned Supplies', $ConSupTotal,$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,($ConSupTotal-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            if (count($this->Bill->xlo['supplies_consigned']) || count($this->Bill->xlo['supplies'])) {
                $this->addGroupTotal('Sub-Total(Supplies)', ($RegSupTotal+$ConSupTotal),$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,(($RegSupTotal+$ConSupTotal)-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
            }
            ...
            if (count($this->Bill->xlo['others'])) {
                foreach ($this->Bill->xlo['others'] as $key => $xlo) {
                    ...
                    $price = number_format($xlo['serv_charge'], 2);
                    $this->addDetail($xlo['qty'] . " @ " . $price, $xlo['qty'] * $xlo['serv_charge']);
                    $OthTotal+=($xlo['qty'] * $xlo['serv_charge']);
                }
            }
            $this->addGroupTotal('Others', $OthTotal,$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,($OthTotal-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            $this->addGroupTotal('Sub-Total(X-Ray, Lab, & Others)', ($LabTotal+$RadTotal+$RegSupTotal+$ConSupTotal+$OthTotal),$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,(($LabTotal+$RadTotal+$RegSupTotal+$ConSupTotal+$OthTotal)-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
        } else {
            foreach ($this->Bill->xlo['laboratories'] as $key => $xlo) {
                $LabTotal+=($xlo['qty'] * $xlo['serv_charge']);
            }
            $this->addGroup('Laboratories', $LabTotal);
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            foreach ($this->Bill->xlo['radiologies'] as $key => $xlo) {
            ...
            $this->addGroup('Radiologies', $RadTotal);
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            foreach ($this->Bill->xlo['supplies'] as $key => $xlo) {
            ...
            $this->addGroup('Regular Supplies', $RegSupTotal);
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            foreach ($this->Bill->xlo['supplies_consigned'] as $key => $xlo) {
            ...
            $this->addGroup('Consigned Supplies', $ConSupTotal);
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            foreach ($this->Bill->xlo['others'] as $key => $xlo) {
    ...
    function showVent(){
        ...
        if ($this->isDetailed) {
            $this->addGroup('Ventilator Use');
            if(count($this->Bill->vent)){
                foreach ($this->Bill->vent as $key => $vent) {
                    $this->addDetail($vent['service_desc']);
                    $this->addDetail($vent['quant'] . " @ " . $vent['serv_price_charge'], $vent['quant'] * $vent['serv_price_charge']);
                    $VentTotal+=($vent['quant'] * $vent['serv_price_charge']);
                }
            }
             $this->addGroupTotal('Sub-Total(Ventilator Use)', ($VentTotal),$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,(($VentTotal)-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
        } else {
            foreach ($this->Bill->vent as $key => $vent) {
                $VentTotal+=($vent['quant'] * $vent['serv_price_charge']);
            }
            $this->addGroup('Ventilator Use', $VentTotal);
        }
        $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
        $this->GROUP_CASERATE_TOTAL_FIRST=0;
        $this->GROUP_CASERATE_TOTAL_SECOND=0;
    ...
    function showMeds(){
        $Med_Reg_Total=0.00;$Med_Con_Total=0.00;
        if ($this->isDetailed) {
            $this->addGroup('Drugs and Medicines');
            if(count($this->Bill->meds)){
                $this->addDetail('Regular Medicines', null, true);
                foreach ($this->Bill->meds as $key => $med) {
                    $this->addDetail($med['artikelname']);
                    $this->addDetail($med['qty'] . " @ " . $med['price'], $med['qty'] * $med['price']);
                    $Med_Reg_Total+=($med['qty'] * $med['price']);
                }
            }
            $this->addGroupTotal('Regular Medicines', $Med_Reg_Total,$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,($Med_Reg_Total-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            if(count($this->Bill->meds_consigned)){
                $this->addDetail('Consigned Medicines', null, true);
                foreach ($this->Bill->meds_consigned as $key => $med) {
                    $this->addDetail($med['artikelname']);
                    $this->addDetail($med['qty'] . " @ " . $med['price'], $med['qty'] * $med['price']);
                    $Med_Con_Total+=($med['qty'] * $med['price']);
                }
            }
            $this->addGroupTotal('Consigned Medicines', $Med_Con_Total,$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,($Med_Con_Total-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            $this->addGroupTotal('Sub-Total(Drugs & Medicines)', ($Med_Con_Total+$Med_Reg_Total),$this->GROUP_CASERATE_TOTAL_DISCOUNT,$this->GROUP_CASERATE_TOTAL_FIRST,$this->GROUP_CASERATE_TOTAL_SECOND,(($Med_Con_Total+$Med_Reg_Total)-($this->GROUP_CASERATE_TOTAL_FIRST+$this->GROUP_CASERATE_TOTAL_SECOND+$this->GROUP_CASERATE_TOTAL_DISCOUNT)));
        } else {
            foreach ($this->Bill->meds as $key => $med) {
                $Med_Reg_Total+=($med['qty'] * $med['price']);
            }
            $this->addGroup('Regular Medicines', $Med_Reg_Total);
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
            foreach ($this->Bill->meds_consigned as $key => $med) {
            ...
            $this->addGroup('Consigned Medicines', $Med_Con_Total);
            $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
            $this->GROUP_CASERATE_TOTAL_FIRST=0;
            $this->GROUP_CASERATE_TOTAL_SECOND=0;
        }
    ...
    function addDoctorRow($doctor,&$roleTotal){
        ...
        }else if($doctor['caserate']==FIRST_CASE_NUM){
    ...
    function showDoctors(){
        ...
        $this->addDetail('Professional Fees');
         if (count($this->Bill->doctors['drProf_fee'])) {
            $roleTotal = array();
            // $this->addDetail('DOCTORS FROM READERS FEE(OB GYNE MODULE)', null, false, false, 2);
            foreach ($this->Bill->doctors['drProf_fee'] as $key => $doctor) {
                $this->addDoctorRow($doctor, $roleTotal);
            }
            $this->Bill->billInfo['totalDoctorsCharge'] += $roleTotal['totalCharge'];
            if ($this->isDetailed)
                $this->addSubTotal('Sub Total(Prof. Fee)', $roleTotal['totalCharge'], $this->Bill->billInfo['total_d1_discount'], $this->GROUP_CASERATE_TOTAL_FIRST, $this->GROUP_CASERATE_TOTAL_SECOND, $roleTotal['totalExcess'], 'T');
        }
        $this->GROUP_CASERATE_TOTAL_DISCOUNT=0;
        $this->GROUP_CASERATE_TOTAL_FIRST=0;
        $this->GROUP_CASERATE_TOTAL_SECOND=0;
...
//        $subtractor = 0;
    ...
    function showCreditCollectionSettlements()
    {
    ...
    function getBillInfo()
    {
        ...
        $this->Bill->meds_consigned = $this->getMeds_consigned();
        $this->Bill->vent = $this->getVent();
    ...
    function getAccommodation()
    {
        ...
        $dateAdmission = $this->Billing->getAdmissionDate();
        
        if($dateAdmission > $accom_effectivity->value)
            $this->Billing->greater_accom_effec = 1;

        $accommodationList = array();
        $getDeath = $this->getDeathDate();
        $this->Billing->death_date = $getDeath;

        if(!$this->Bill->billInfo['is_final'])  
            $this->Billing->bill_dte = strftime("%Y-%m-%d %H:%M:%S");
        
        $accommodations = $this->Billing->getAccomodationList()->GetRows();
        // echo "<pre>" . print_r($accommodations,true) . "</pre>";die;
        if ($getDeath!=''){
            $tempbill_dte = $getDeath;
        }elseif(strcmp($bill_dte, "0000-00-00 00:00:00") != 0){
                $tempbill_dte = $this->Billing->bill_dte;
        }else{
            $tempbill_dte = strftime("%Y-%m-%d %H:%M:%S");
        }

        if($this->Billing->greater_accom_effec){
            $details = $this->Billing->getExtractedAccommodationList($accommodations);
            ...
            foreach ($details as $i => $accommodation) {
                ...
                if ($accommodation['source'] == 'AD') {
                    ...
                    if(strtotime($accommodation['date_to']) > strtotime($this->tempbill_dte)){
                        $accommodation['date_to'] = strftime("%Y-%m-%d", strtotime($this->tempbill_dte));
    ...
    function getXlo()
    {
        $xlos = $this->Billing->getXLOList_v2()->GetRows();
        $laboratories = array();
        $radiologies = array();
        $supplies = array();
        $others = array();
        foreach ($xlos as $i => $xlo) {
            switch ($xlo['source']) {
                ...
                case ($xlo['source'] == 'SU' || $xlo['source'] == 'MS'):
                    $supplies[] = $xlo;
                    break;
                case ($xlo['source'] == 'SU_CON' || $xlo['source'] == 'MS_CON'):
                    $supplies_consigned[] = $xlo;
                    break;
                case 'OA':
                    $others[] = $xlo;
                    break;
        ...
        $result = array(
            ...
            'supplies' => $supplies,
            'supplies_consigned' => $supplies_consigned,
            'others' => $others
        );
    ...
    function getMeds()
    {
        $meds = $this->Billing->getMedsList_v2()->GetRows();
        return $meds;
    }

    function getMeds_consigned()
    {
        $meds = $this->Billing->getMedsList_Consigned()->GetRows();
        return $meds;
    }

    function getVent()
    {
        $ventss =  $this->Billing->getVentilatorList()->GetRows();
        return $ventss;
    }
    ...
    function getDoctors()
    {
        ...
        foreach ($benefits as $x => $benefit) {
            ...
            foreach ($doctors as $y => $doctor) {
                if ($benefit->dr_nr == $doctor->dr_nr && $benefit->role_area == $doctor->role_area && (!$doctor->from_ob)) {
                    $doctorsList[] = array(
                        'dr_nr' => $doctor->dr_nr,
                        ...
                        'from_ob'=> $doctor->from_ob
                    );
                }
                if($doctor->entry_no != $entry_no && $benefit->dr_nr == $doctor->dr_nr && $doctor->from_ob){
                      $doctorsList[] = array(
                        'dr_nr' => $doctor->dr_nr,
                        'name' => strtoupper("$doctor->dr_first $doctor->dr_mid $doctor->dr_last"),
                        'role_nr' => $doctor->role_nr,
    ...
    */
    ...
    {
        global $db;
        if ($id == NBS_PACKAGE_ID || $id == NBS2_PACKAGE_ID) {
    ...
    function getInsuranceMemberInfo()
    {
        global $db;
