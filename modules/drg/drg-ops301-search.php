			...
			if ($linecount>0) 
				{ 
					...
					while($zeile=$ergebnis->FetchRow())
					{
						...
						//	if(stristr($strbuf,"...")) //comment by mark on June 14, 2007
									echo "isparent";
							if((stristr($strbuf,"-"))&& (strlen($strbuf)== '3'))
							{
								$isparent=1;
								$iscolor=1;
								echo "isparent";
							}
						//	elseif(stristr($strbuf,"."))  // comment by mark June 14, 2007 
							elseif(stristr($strbuf,"-"))	
							{
									//$parentcode=substr($strbuf,0,strpos($strbuf,".")); //comment by mark June 14, 2007
									#op code  =  1-20 parentcode
									$parentcode = substr($strbuf,1,strpos($strbuf,"-"));
									echo $parentcode;
