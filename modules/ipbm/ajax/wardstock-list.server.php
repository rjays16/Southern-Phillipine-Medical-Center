    ...
    if($target=="recent"){
        ...
        if ($seldate!="") {
            switch(strtolower($seldate)) {
                ...
                break;
                case "specificdate":
                    $search_title = "Paid/Urgent Requests On " . date("F j, Y",strtotime($args["specificdate"]));
                    $dDate = date("Y-m-d",strtotime($args["specificdate"]));                
                    $filters['DATE'] = $dDate;
                break;
                case "between":
                    $search_title = "Paid/Urgent Requests From " . date("F j, Y",strtotime($args["between1"])) . " To " . date("F j, Y",strtotime($_REQUEST["between2"]));
        ...
        if ($selward!="") {
            $filters["WARD"] = $selward;
        }
        
  
   
    $offset = $page_num * $max_rows;
	$sortColumns = array('stock_date','stock_nr','ward_name','','encoder','area_full');
	$sort = array();
