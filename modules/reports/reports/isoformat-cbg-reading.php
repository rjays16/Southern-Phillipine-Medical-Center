...
$params->put("rprt_title", "BLOOD GLUCOSE RESULT SHEET");
...
$strSQL = "SELECT DATE_FORMAT(reading_dt, '%c/%d/%y') reading_dt, DATE_FORMAT(reading_dt, '%l:%i %p') reading_tm, reading_level, readby_name "
...
if ($result) {
    while ($row = $result->FetchRow()) {
        $data[] = array('post_dt' => $row['reading_dt'],
                        'post_tm' => $row['reading_tm'],
                        'reading_level' => doubleval($row['reading_level']),
                        'reader' => is_null($row['readby_name']) ? "" : strtoupper($row['readby_name'])
            );        
    }
}
