...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&!empty($searchkey)){
	...
	$refCode=$person_obj->searchLimitActiveEthnic($searchkey,$GLOBAL_CONFIG['pagin_icpm_search_max_block_rows'],$pgx,$oitem,$odir);
	...
	if(isset($totalcount)&&$totalcount){
		...
		$totalcount=$person_obj->searchCountActiveEthnic($searchkey);
		$pagen->setTotalDataCount($totalcount);
	}
...
# Title in toolbar
 $smarty->assign('sToolbarTitle',"Ethnic Group :: $LDSearch");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Ethnic Group :: $LDSearch");
...
<!--  The search mask  -->
	...
	<table border=0 cellpadding=10 bgcolor="<?php echo $entry_border_bgcolor ?>">
     <tr>
       <td>
	   <?php 
		 		$searchprompt="Please enter an ethnic search key";
...
?>
 ...
 <table border=0 cellpadding=2 cellspacing=1 id="ethnic_list">
...
<?php
	...
	while($refcode=$refCode->FetchRow()){
		...
		$toggle=!$toggle;
?>
  <tr  class="<?php echo $bgc ?>" id="row<?=$refcode['nr'];?>">
    ...
    <td width="30%"><a href="ethnic_info.php<?php echo URL_APPEND.'&retpath=search&ethnic_nr='.$refcode['nr']; ?>"><?php echo $refcode['name']; ?></a></td>
  </tr> 
<?php
	}
	echo '
