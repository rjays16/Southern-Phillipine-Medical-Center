...
$breakfile='edv-system_manage.php'.URL_APPEND.'&target=religion';
...
if(empty($oitem)) $oitem='religion_name';
...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&!empty($searchkey)){
	...
	$refCode=$person_obj->searchLimitActiveReligion($searchkey,$GLOBAL_CONFIG['pagin_icpm_search_max_block_rows'],$pgx,$oitem,$odir);
	...
	if(isset($totalcount)&&$totalcount){
		...
		$totalcount=$person_obj->searchCountActiveReligion($searchkey);
...
# Title in toolbar
 $smarty->assign('sToolbarTitle',"Religion :: $LDSearch");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Religion :: $LDSearch");
...
<!--  The search mask  -->
	...
	<table border=0 cellpadding=10 bgcolor="<?php echo $entry_border_bgcolor ?>">
     <tr>
       <td>
	   <?php 
		 		$searchprompt="Please enter a religion search key";
...
?>
 ...
 <table border=0 cellpadding=2 cellspacing=1 id="religion_list">
  <tr class="wardlisttitlerow">
      <td><b>Delete</b></td>
			<td><b>
	  <?php echo $pagen->makeSortLink("Religion",'religion_name',$oitem,$odir);  ?></b>
	</td>
  </tr> 
<?php
	$toggle=0;
	while($refcode=$refCode->FetchRow()){
		...
		$toggle=!$toggle;
?>
  <tr  class="<?php echo $bgc ?>" id="row<?=$refcode['religion_nr'];?>">
		 <td class=pblock  bgColor="#eeeeee" align="center" valign="middle" width="5%">
 			<img name="delete<?=$refcode['religion_nr'];?>" id="delete<?=$refcode['religion_nr'];?>" src="../../images/btn_delitem.gif" style="cursor:pointer" border="0" onClick="deleteReligion('<?=$refcode['religion_nr'];?>','<?=$refcode['religion_name'];?>');"/>
		 </td>
    <td width="30%"><a href="religion_info.php<?php echo URL_APPEND.'&retpath=search&religion_nr='.$refcode['religion_nr']; ?>"><?php echo $refcode['religion_name']; ?></a></td>
  </tr> 
<?php
	}
	echo '
...
<form action="religion_new.php" method="post">
