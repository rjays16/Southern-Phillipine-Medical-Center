...
$thisfile='edv_system_measurement_unit_add.php';
if($from=='set') $returnfile='edv_system_measurement_unit_set.php'.URL_APPEND.'&from=add';
...
if(($mode=='save')&&$short_name&&$long_name&&$info){
    ...
    if($unit_id){
	   $sql="UPDATE $dbtable SET unit_name='$unit_name',
							unit_desc='$unit_desc',							
	   ...
	   if($ergebnis=$core->Transact($sql))
       {
		 if($db->Affected_Rows())
		 {
		   ...
		   $db->Execute($sql);*/
		   $new_measurement_unit_ok=1;
		   $saved_msg=$LDMeasurementUnitUpdated;
			 //$saved_msg="Data was saved...";
		 }
		   else
		   {
		     $new_measurement_unit_ok=0;
		   }
		}
		else 
			echo "<p> $sql <p>$LDDbNoSave";
			//echo "<p> $sql <p>Data not saved...";
	}
	else
	{
		$info_exist=0;
	   ...
	   $sql="SELECT unit_id FROM $dbtable WHERE unit_name='$unit_name' AND unit_desc $sql_LIKE '$unit_desc'";
			...
			if($ergebnis=$core->Transact($sql))
       		{
				...
				{
					$new_currency_ok=1;
				 	$saved_msg=$LDAddedNewMeasurementUnit;
...
if(($mode=='edit')&&$item_no)
{
    ...
    $sql="SELECT unit_name,unit_desc FROM $dbtable WHERE unit_id='$unit_id'";
	if($ergebnis=$db->Execute($sql))
	{
	  if($ergebnis->RecordCount())
	  {
	     $c_result=$ergebnis->FetchRow();
		 $unit_name=$c_result['unit_name'];
		 $unit_desc=$c_result['unit_desc'];
      }else{
	  	$unit_id='';
	  }
	}else{
...
<?php if($unit_id) echo $LDUpdateMeasurementUnit; else echo $LDAddMeasurementUnit ?> </FONT><FONT class="prompt"><p>
<?php
...
<?php if($unit_id) $save_button='update.gif'; else $save_button='savedisc.gif'; ?>
...
<img <?php echo createComIcon($root_path,'bul_arrowgrnsm.gif','0') ?>> 
<?php echo $LDClk2SetMeasurementUnit; ?>
</a>
