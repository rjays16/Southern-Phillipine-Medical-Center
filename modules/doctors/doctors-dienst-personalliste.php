...
#global $dept_current;
#$dept_current = $_REQUEST['dept_current'];
#echo "<br>dept_current = $dept_current <br>";
...
#echo"doctors-dienst-personalliste.php : cfg : <br> \n"; print_r($cfg); echo " <br> \n";
//echo"doctors-dienst-personalliste.php : dept_nr = '".$dept_nr."' <br> \n";
...
$append='&retpath='.$retpath.'&ipath='.$ipath.'&dept_nr='.$dept_nr;   # burn added: Spet 29, 2006
#echo "append : $append";
...
if($mode=='search'||$mode=='paginate'){
	...
	#$search_result=&$pers_obj->searchLimitPersonellBasicInfo($searchkey,$pagen->MaxCount(),$pgx,$oitem,$odir);
       # NOTE: last parameter is 1, meaning doctors
   # echo "b4 searchLimitPersonellBasicInfoByJobType job type= <br>";
	$search_result=&$pers_obj->searchLimitPersonellBasicInfoByJobType($searchkey,$pagen->MaxCount(),$pgx,$oitem,$odir,1);   # burn modified: Sept 28, 2006
	#echo $pers_obj->sql;
...
<!--     <td  align=center  class="v13" colspan=2><font><nobr>&nbsp;<?php echo $LDDateOfBirth; ?></nobr></td>
 -->    <td  align=center  class="v13" colspan=2><nobr>&nbsp;<?php echo $LDFunction; ?></nobr></td>
	...
	?></nobr></td>
    <td  class="v13" colspan=3><nobr>&nbsp;<?php echo ucwords(strtolower($row['name_last'])); ?></nobr></td>
    <td  class="v13" colspan=2><nobr><fon >&nbsp;<?php echo ucwords(strtolower($row['name_first'])); ?></nobr></td>
<!--     <td  class="v13" colspan=2><font>&nbsp;<?php echo formatDate2Local($row['date_birth'],$date_format); ?></td>
 -->    <td  class="v13" colspan=2><nobr>&nbsp;<?php echo $row['job_function_title']; ?></nobr></td>
	...
	<td>
			<a href="javascript:deleteItem('<?php echo $row['nr']; ?>')" title="<?php echo $LDDelete; ?>">
...
<?php
					...
					while($row=$search_result->FetchRow())
					{
						...
						<td>&nbsp;
							<a href="doctors-list-add.php'.URL_APPEND.'&nr='.$row['nr'].'&dept_nr='.$dept_nr.'&job_fxn='.$row['job_function_title'].'&mode='.$mode_status.'&retpath='.$retpath.'&ipath='.$ipath.'" title="'.$LDAddDoctorToList.'">
...
*/
			echo $v['name_formal'];
			if (($v['admit_inpatient'] == 1)&&($v['admit_outpatient'] == 1))
				echo " (IPD/OPD)";
			if (($v['admit_inpatient'] == 1)&&($v['admit_outpatient'] == 0))
				echo " (IPD)";
			if (($v['admit_inpatient'] == 0)&&($v['admit_outpatient'] == 1))
				echo " (OPD)";
		}
		echo '</option>';
	}?>
