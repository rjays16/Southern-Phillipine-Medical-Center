 ...
 */
 ...
 $smarty->assign('sToolbarTitle',"Clinics :: Services Manager");
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Clinics :: Services Manager");
...
function deleteService(code){
	var answer = confirm("Are you sure you want to delete the service with a code of "+(code.toUpperCase())+"?");
		if (answer){
...
function startAJAXSearch(searchID, page) {
	...
	var aLabServ = 0;//$("parameterselect").value;
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateLabServiceList('"+aLabServ+"','"+searchID+"','"+keyword+"',"+page+",'"+labArea+"')",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("ServiceList-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateLabServiceList('"+aLabServ+"','search','"+keyword+"',"+page+",'"+labArea+"')",50);
	lastSearch = keyword;
}
...
function refreshWindow(key,page){
	...
	startAJAXSearch('search', 0);
	//startAJAXSearch(key.value, page.value);
    //startAJAXSearch('', page.value);
}
...
//alert('sulod listID, id, name, cash, charge, sservice = '+listID+" , "+id, name+" , "+cash+" , "+charge+" , "+sservice);
		...
		if (id) {
			...
			*/
			...
			rowSrc = '<tr><td colspan="9" style="">No such service exists...</td></tr>';
		}
...
function AddItem(code,mode){
	...
	var grpcode = 0;//document.forms["paramselect"].parameterselect.value;	
	var label;
	...
	return overlib(
         OLiframeContent('seg-lab-services-edit.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&nr='+escape(code)+"&grpcode="+grpcode+"&mode="+mode+"&labarea=<?php echo "$lab_area"?>", 700, 450, 'fGroupTray', 1, 'auto'),
...
<table id="ServiceList" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
	<thead>
		<tr>
			...
			<th width="10%" align="left">Code #</th>
			<th width="10%" align="left">Cash</th>
	...
	<tbody id="ServiceList-body">
		<tr>
			<td colspan="9">No such service exists...</td>
		</tr>
	</tbody>
</table>
