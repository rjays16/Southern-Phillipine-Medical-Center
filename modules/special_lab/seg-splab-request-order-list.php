...
#Added by Gervie 11/09/2015
require_once $root_path . 'include/care_api_classes/class_acl.php';
$acl = new Acl($_SESSION['sess_temp_userid']);

$CanWriteEcgResult = $acl->checkPermissionRaw(array('_a_2_splabecgresult'));
$CanWriteEEGResult = $acl->checkPermissionRaw(array('_a_2_splabEEGresult'));
...
function OpenEcgResult(obj, refno){

	var canWriteResult = '<?=$CanWriteEcgResult?>';

	if(canWriteResult == 1) {
		overlib(
			OLiframeContent('<?=$root_path?>modules/special_lab/seg-splab-ecg-result.php?refno=' + refno,
				600, 500, 'fGroupTray', 0, 'auto'),
			WIDTH, 600, TEXTPADDING, 0, BORDER, 0,
			STICKY, SCROLL, CLOSECLICK, MODAL,
			CLOSETEXT, '<img src=<?=$root_path?>/images/close_red.gif border=0 >',
			CAPTIONPADDING, 2, CAPTION, "Encode ECG Result",
			MIDX, 0, MIDY, 0,
			STATUS, "Encode ECG Result");
	}
	else{
		alert('No access permission in writing ECG result.');
	}

}
//added by gelie 11-28-2015
function openEEGResult(obj, refno, code){

	var canWriteResult = '<?=$CanWriteEEGResult?>';

	if(canWriteResult == 1) {
		overlib(
			OLiframeContent('<?=$root_path?>modules/special_lab/seg-splab-eeg-result.php?refno=' + refno + '&service_code=' + code,
				600, 500, 'fGroupTray', 0, 'auto'),
			WIDTH, 600, TEXTPADDING, 0, BORDER, 0,
			STICKY, SCROLL, CLOSECLICK, MODAL,
			CLOSETEXT, '<img src=<?=$root_path?>/images/close_red.gif border=0 >',
			CAPTIONPADDING, 2, CAPTION, "Encode EEG Result",
			MIDX, 0, MIDY, 0,
			STATUS, "Encode EEG Result");
	}
	else{
		alert('No access permission in writing EEG result.');
	}
...
//													'CAPTIONPADDING, 4, CAPTION, \'Special Laboratory Request\', MIDX, 0, MIDY, 0, STATUS, \'Special Laboratory Request\');">';
							...
							if(service == 'ECG'){
								donerow = '<td align="center" style="font-size:11px"><img  name="served' + rowid + '" id="served' + rowid + '" src="../../gui/img/common/default/form_pen.gif" width="20px" border="0" style="cursor:pointer" onClick="OpenEcgResult(this,\'' + refno + '\');"></td>';
							}
							else if(code.indexOf("EEG") >= 0){
								donerow = '<td align="center" style="font-size:11px"><img  name="served' + rowid + '" id="served' + rowid + '" src="../../gui/img/common/default/form_pen.gif" width="20px" border="0" style="cursor:pointer" onClick="openEEGResult(this,\'' + refno + '\', \'' + code + '\');"></td>';
							}
							else {
				...
				<tr>
						...
						<?php }else{?>
								<!--<th width="3%" align="center">Reagents</th><th width="4%" align="center">Lab Result</th>-->
								 <th width="3%" align="center">Served</th>
						<?php } ?>
				</tr>
		</thead>
		<?php
