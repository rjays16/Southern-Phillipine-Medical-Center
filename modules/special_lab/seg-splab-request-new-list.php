...
function jsRequest(sub_dept_nr, No, refno, name, req_date,
                   urgency, labstatus, paid, encounter_nr,
                   pid, age, sex, location, enctype, or_no,
                   is_cash, request, source_req, is_printed) {

    var dTable,dTBody,dRows,rowSrc,sid,lang;
    var patType;
    ...
    if (dTable=document.getElementById('Ttab'+sub_dept_nr)) {
        ...
        if (refno) {
                ...
                if (paid!=0){
                    ...
                    if (typeof request != 'undefined') {
                        if (parseInt(request.allowDelete) == 0) {
                            ...
                            mode = '<img data-warning="{warning}" name="delete{refno}" id="delete{refno}" src="../../images/delete.gif" style="cursor:pointer" border="0" onClick="deleteRequest(\'{refno}\'); refreshWindow(key,pagekey); "/>';
                            mode = mode.replace(/\{refno\}/g,refno);
                            mode = mode.replace('{warning}',request.warning);
                }
            }
            ...
            editlink2 ='onclick="return overlib(OLiframeContent(\'seg-splab-reports.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&user_origin=splab&popUp='+1+'&ref='+refno+'&encounter_nr='+encounter_nr+'&pid='+pid+'\', 800, 440, \'flab-list\', 1, \'auto\'), ' +
                'WIDTH, 800, TEXTPADDING, 0, BORDER, 0, STICKY, SCROLL, CLOSECLICK, MODAL, CLOSETEXT, \'<img src=../../images/close.gif border=0 onClick=refreshWindow(key,pagekey);>\', ' +
            'CAPTIONPADDING, 4, CAPTION, \'Special Laboratory Reports\', MIDX, 0, MIDY, 0, STATUS, \'Special Laboratory Reports\');">';
            ...
            '<td align="center" style="font-size:11px">'+pid+'</td>'+
            '<td align="center" style="font-size:11px">'+age+'</td>'+
            '<td align="center" >'+sex_img+'</td>'+
            '<td align="center" style="font-size:11px">'+enctype+'</td>'+
            '<td align="justify" style="font-size:11px">'+location+'</td>'+
            ...
            '<td align="center">'+mode+'</td>'+
                //borj
            '<td align="center"><a href="javascript:void(0);" '+editlink2+'<img src="../../images/timeplan.gif" border="0" title="Reports"></a></td>'+
            '</tr>';
        } else {
...
function jsSortHandler(items,oitem,dir,sub_dept_nr){
    ...
    xajax_PopulateRequests('T'+tab, key, sub_dept_nr, pgx, thisfile, rpath, mode,oitem,odir);
}
...
//-------------- Commented Out by Gervie 08/29/2015 ----------------
                ...
                <ul>
                    ...
                    <div id="tbContainer" dojoType="TabContainer" style="width:auto; height:30.5em; ">
                        <div dojoType="ContentPane" widgetId="tab0" label="All" style="display:none; overflow:auto;" >
                            ...
                            <div dojoType="ContentPane" widgetId="tab2" label="OPD" style="display:none; overflow:auto" <?php if($patient_type == "2")echo"selected=\"true\""; ?>>
                            ...
                            <div dojoType="ContentPane" widgetId="tab4" label="ER" style="display:none; overflow:auto" <?php if($patient_type == "1")echo"selected=\"true\""; ?>>
                            ...
                            <div dojoType="ContentPane" widgetId="tab7" label="IPBM" style="display:none; overflow:auto" >
...
<table id="RequestList" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <thead>
        ...
        <tr>
            ...
            <!--borj-->
<!--
            <th width="3%" align="center">Reports</th>
        </tr>
    </thead>
    <?php
            if (empty($rows))
