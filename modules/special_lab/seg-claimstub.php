...
*/
	...
	function Header() {
		...
		if ($labserv_details["request_dept"])
            ...
            if($labserv_details['request_doctor']){
                $docDept = $pers_obj->get_Dept_name($labserv_details['request_doctor']);
                $docName = $pers_obj->get_Person_name($labserv_details['request_doctor']);

                $reqDoc = 'DR. ' . $docName['dr_name'];
                $reqDept = $docDept['name_formal'];
            }
            else{
                $reqDoc = "N/A";
                $reqDept = "N/A";
        ...
        if($person['encounter_type'] == 1) {
            ...
            $this->SetFont("Arial", "B", "9");
            $this->Cell(60, 4, $reqDoc, $borderNo, $newLineNo, 'L');
        }
        ...
        if($person['encounter_type'] == 1) {
            $this->SetFont("Arial", "", "8");
            $this->Cell(25, 4, 'DEPARTMENT : ', $borderNo, $newLineNo, 'L');
            $this->SetFont("Arial", "B", "9");
            $this->Cell(60, 4, $reqDept, $borderNo, $newLineNo, 'L');
        }
