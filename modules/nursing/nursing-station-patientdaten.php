...
function rx(){
	...
	if ($edit) return 'onClick="javascript:pullRosebar(this)"></a>';
		...
		*/
		echo '<input 
		type="button" onClick="javascript:window.location.href=\''.$root_path.'modules/laboratory/labor_datalist_noedit.php'.URL_REDIRECT_APPEND.'&station='.$station.'&pn='.$pn.'&pid='.$pid.'&user_origin='.$user_origin.'&edit='.$edit.'\'" value="'.$LDLabReports.'"><input 
		type="button" onClick="javascript:enlargewin();window.location.href=\''.$root_path.'modules/fotolab/fotos-start.php'.URL_REDIRECT_APPEND.'&pn='.$pn.'&station='.$station.'&fileroot='.$fr.'&edit='.$edit.'\'" value="'.$LDPhotos.'">';
		echo '<input type="button" onClick="js_onClick()" value="Attending Doctor\'s" >';
