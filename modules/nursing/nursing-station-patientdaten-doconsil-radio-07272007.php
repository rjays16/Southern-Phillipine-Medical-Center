...
<!-- 
   ...
   -->
	...
	<tr bgcolor="<?php echo $bgc1 ?>">
		...
		<textarea name="clinical_info" cols=80 rows=2 wrap="physical"><?php if($edit_form || $read_form) echo stripslashes($stored_request['clinical_info']) ?></textarea>
		</td>
	</tr>	
		...
		<td colspan=2><div class=fva2_ml10><?php echo $LDReqTest ?>:<br>
		 ...
		 <td colspan=2><div class=fva2_ml10><?php echo $LDReqTest ?>:<br>
		   ...
		   if(is_object($all_radGrp)){
			$stmp = '';
		   	echo '<select name="raddept_nr" id="raddept_nr" onChange="jsGetServiceGroup(paramselect)">
		   ...
		   ?>
		        <select name="paramselect" id="paramselect" onChange="jsGetRadioService(paramselect, 1);"></select>
							...
							<thead class="">
								<td colspan="4">
									<table width="100%" cellspacing="2" cellpadding="2" border="0">
										<tr>
											<td width="*" class="reg_header"><?=$serviceSelected?></td>
											<td width="1%" align="right" style="padding:2px;2px;font-weight:normal" class="reg_header">
												<span class="reglink" onclink="toggleDisplay2('grpBodyDT');">Show/Hide</span>
											</td>
										</tr>
									</table>
								</td>
							</thead>
							<thead id="grpBodyDT" class="reg_list_titlebar" style="height:0;overflow:visible;font-weight:bold;padding:4px;" id="srcRowsHeader">
								<td width="1"><input id="chk_all" name="chk_all" type="checkbox" disabled></td>
								<td width="15%" nowrap>Code</td>
								<td width="60%" nowrap>Description</td>
								<td width="15%" nowrap>Price</td>
							</thead>
						 	<tbody id="grpBody" style="height:0;overflow:visible"></tbody>							
				   		</table>
				   	</div>
