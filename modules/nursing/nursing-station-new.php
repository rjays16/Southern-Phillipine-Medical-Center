...
function preSet(){
	...
	if (d.accomodation_type_temp[0].checked){
		...
		//alert('payward');
		d.accomodation_type_temp[0].disabled = false;
	}
...
function statchange(room_nr, hide) {
    var option = hide != 1? 'Change status to Active' : 'Are you sure you want to hide this room?';
    var ward_nr = $J('#ward_nr').val();
    var title_str = hide != 1? 'Change status of room # ' : 'Hide room # ';
    $J("#dialog-changestatus").html(option);
    $J("#dialog-changestatus").dialog({
        resizable: false,
        modal: true,
        title: title_str+" "+room_nr,
        height: 150,
        width: 300,
        buttons: {
            "Yes": function () {
                if (!hide) {
                	xajax_updateRoomStatus(room_nr,ward_nr,0);
                }else{
                	xajax_updateRoomStatus(room_nr,ward_nr,1);
                }
                $J(this).dialog('close');
...
$count=0;
#$result = $ward_obj->getRoomsData($ward_nr); commented by art 07/17/2014
$result = $ward_obj->getAllRoomsData($ward_nr);  #added by art 07/17/2014
#echo "sql = ".$ward_obj->sql;
if ($result){
	...
	foreach ($rows as $i=>$row) {
		if ($row) {
			...
			if ($row['status'] != ''){
				$deleteImg = 'src="../../gui/img/common/default/update.gif" border=0 alt="change status" title="change status" style="cursor:pointer"';
				$haspatient = 0;
			}elseif($haspatient){
			...
			*/
			...
			#added by art 07/17/2014
            $status = $row['status'] != '' ? '- <span style="color:red"><strong>'.$row['status'].'</strong></span>' : ''; 
            #end art
            $dis = $row['status'] != '' ? 'disabled':'';
			$selectType = '<select '.$dis.' id="type_nr2'.$row['room_nr'].'" name="type_nr2[]" onchange="setTypeNrArray(this.value,\''.$row['room_nr'].'\');">
							<option value="0">Select Room Type</option>
										'.$option.'
						 </select> ';

#edited by art 07/27/2014
/*			$src .=
					'<tr class="wardlistrow'.$alt.'" id="row'.$row['room_nr'].'" style="font-weight:bold;padding:0px">
						<td align="center"><font face="verdana,arial" size="2" >
							...
							<input type="hidden" name="rooms[]" id="rooms'.$row['room_nr'].'" value="'.$row['room_nr'].'">
							'.$row['room_nr'].'
							'.$status. #added by art 07/17/2014
							'</font>
						</td>
						<td align="center">
							<input type="text" name="beds[]" id="beds'.$row['room_nr'].'" onBlur="trimString(this); checkOccupants('.$row['room_nr'].', this.value, \'P\');" size="8" maxlength="3" value="'.$row['nr_of_beds'].'" '.$disabled.'>
						</td>
						<td>
							<input type="text" name="infos[]" id="infos'.$row['room_nr'].'" onBlur="trimString(this);" size=50 maxlength=100 value="'.$row['info'].'" '.$disabled.'>
						</td>
						<td> '.$selectType.'
            ...
            if($row['status']!=''){
                $src .= '<td align="center">
							<img name="changestatus'.$row['room_nr'].'" id="changestatus'.$row['room_nr'].'" '.$deleteImg.' onclick="statchange('.$row['room_nr'].')">
						</td>
					</tr>
				';
            }else{
				...
				if ($row['status'] == '') {
					...
					'<tr class="wardlistrow'.$alt.'" id="row'.$row['room_nr'].'" style="font-weight:bold;padding:0px">
						...
						<td align="center">
							<img name="delete'.$row['room_nr'].'" id="delete'.$row['room_nr'].'" src="../../gui/img/common/default/eye.gif" border=0 onClick="statchange('.$row['room_nr'].',1);" alt="hide" title="hide" style="cursor:pointer">
						</td>
						<td align="center">
							<img name="delete'.$row['room_nr'].'" id="delete'.$row['room_nr'].'" '.$deleteImg.'>
						</td>
				...
				';
					...
					'<tr class="wardlistrow'.$alt.'" id="row'.$row['room_nr'].'" style="font-weight:bold;padding:0px">
						...
						<td> '.$selectType.'
							...
							<td align="center">
							<img name="changestatus'.$row['room_nr'].'" id="changestatus'.$row['room_nr'].'" '.$deleteImg.' onclick="statchange('.$row['room_nr'].',0)">
						</td>
						<td align="center">
						</td>
					</tr>
            ...
            $src .='<div id="dialog-changestatus"></div>';
		}
		#end art
	}
}else{
