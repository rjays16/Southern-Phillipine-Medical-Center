...
function getPatientAdmissionInfo(pn){
	//alert("pn = "+pn);
	urlholder="<?php echo $root_path ?>modules/registration_admission/aufnahme_daten_zeigen.php<?php echo URL_REDIRECT_APPEND ?>&from=such&encounter_nr="+pn+"&target=search&fromnurse=1";	
	adminwin<?php echo $sid ?>=window.open(urlholder,"adminwin<?php echo $sid ?>","width=850,height=570,menubar=no,resizable=yes,scrollbars=yes");
}
...
#			$smarty->assign('sBed',strtoupper(chr($j+96)));   # burn commented : September 27, 2007
			...
			if($edit){
				if(($is_patient)&&!empty($bed['encounter_nr'])){
					...
					#$sAstart = $sAstart.'javascript:getPatientAdmissionInfo(\''.$bed['encounter_nr'].'\')" title="'.$LDClk2Show.'">';
					$smarty->assign('sAdmitDataIcon','<a href="javascript:getPatientAdmissionInfo(\''.$bed['encounter_nr'].'\')" title="'.$LDAdmissionData.' : '.$LDClk2Show.'"><img '.createComIcon($root_path,'pdata.gif','0','',TRUE).' alt="'.$LDAdmissionData.' : '.$LDClk2Show.'"></a>');
					...
					$sBuffer = '<a href="javascript:getrem(\''.$bed['encounter_nr'].'\')" title="'.$LDNoticeRW.'"><img ';
					...
					$smarty->assign('sTransferIcon','<a href="javascript:Transfer(\''.$bed['encounter_nr'].'\')" title="'.$LDTransferPatient.'"><img '.createComIcon($root_path,'xchange.gif','0','',TRUE).' alt="'.$LDTransferPatient.'"></a>');
					$smarty->assign('sDischargeIcon','<a href="javascript:release(\''.$bed['room_nr'].'\',\''.$bed['bed_nr'].'\',\''.$bed['encounter_nr'].'\')" title="'.$LDReleasePatient.'"><img '.createComIcon($root_path,'bestell.gif','0','',TRUE).' alt="'.$LDReleasePatient.'"></a>');
				}
			}
	...
	if($waitlist_count&&$edit){
		while($waitpatient=$waitlist->FetchRow()){
			...
			$TP_WLIST_BLOCK.='&nbsp;'.ucwords(strtolower($waitpatient['name_last'])).', '.ucwords(strtolower($waitpatient['name_first'])).' '.formatDate2Local($waitpatient['date_birth'],$date_format).'</nobr></a><br>';
		}
	}else{
		$TP_WLIST_BLOCK='&nbsp;';
	}
