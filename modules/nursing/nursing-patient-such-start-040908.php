...
if($mode=='such'||$mode=='paginate')
{
	...
	#edited by by VAN 01-28-08
	#if(!$arch) $cond.=" AND e.is_discharged IN ('',0) AND p.pid=e.pid ";
	if(!$arch) $cond.=" AND p.pid=e.pid ";
	...
	$gbuf="l.location_nr,p.name_last, p.name_first,p.date_birth,
					...
					w.name,w.roomprefix,
					l.date_from,
					r.location_nr, l.date_from AS ward_date";
	*/
...
//		else $cond.=' AND p.pid=e.pid GROUP BY r.location_nr';
	...
	if($dblink_ok){			
		...
		#added by VAN 01-28-08
		if(is_numeric($srcword)){
			$encounter_nr = (int)$srcword;
		}else{
			$sqlenc = "SELECT p.*, e.encounter_nr from care_person AS p
					  INNER JOIN care_encounter AS e on p.pid=e.pid
					  WHERE p.name_last $sql_LIKE '".$srcword."%' OR p.name_first 
					  $sql_LIKE '".$srcword."%' OR p.date_birth $sql_LIKE '".$srcword."%'";
		#echo "sql = ".$sqlenc."<br>";			  
		
		$resultenc = $db->Execute($sqlenc);			  
		$rowenc = $resultenc->FetchRow();
		$encounter_nr = $rowenc['encounter_nr'];
		}
		
		#echo "<br>enc = ".$encounter_nr;
		
		#print_r($rowsubquery);
		#echo "enc = ".$rowenc['encounter_nr'];
		...
		*/
		...
		$sqlselect="SELECT DISTINCT e.current_room_nr AS room_nr, p.name_last, 
						p.name_first,p.date_birth, e.encounter_nr,
						e.is_discharged, e.discharge_date, e.discharge_time, 
						e.encounter_class_nr,e.in_ward, 
						(SELECT w.name FROM care_ward AS w WHERE e.current_ward_nr=w.nr) AS ward_name,
						(SELECT w.roomprefix FROM care_ward AS w WHERE e.current_ward_nr=w.nr) AS roomprefix, 
						e.current_ward_nr AS ward_nr, 
						(SELECT l.date_from FROM care_encounter_location AS l 
						 WHERE encounter_nr=".$encounter_nr." ORDER BY date_from DESC LIMIT 1) AS ward_date";
