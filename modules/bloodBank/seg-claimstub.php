...
*/
	...
	function Header() {
		...
		}elseif ($person['encounter_type']==2 || $person['encounter_type']==self::IPBMOPD){
			if($person['encounter_type']==self::IPBMOPD) $enctype = "IPBM (OPD)";
			else $enctype = "OUTPATIENT (OPD)";
			$dept = $dept_obj->getDeptAllInfo($person['current_dept_nr']);
			$location = strtoupper(strtolower(stripslashes($dept['name_formal'])));
		}elseif (($person['encounter_type']==3)||($person['encounter_type']==4)||($person['encounter_type']==self::IPBMIPD)){
			if($person['encounter_type']==self::IPBMIPD) $enctype = "IPBM (IPD)";
			else $enctype = "INPATIENT";
