...
function addPerson(listID, refno, encounter_nr, name, req_date, urgency, labstatus,
				   paid, repeat, pid, age, sex, location, enctype, or_no,
				   is_cash, withsample, is_hact, details_rec, request, enc_nr) {
	...
	if (list) {
		...
		if (refno) {
			...
			editlink ='onclick="return overlib(OLiframeContent(\'seg-blood-request-new.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&user_origin=blood&popUp='+1+'&repeat='+repeat+'&ref='+refno+'&pid='+pid+'&encounter_nr='+encounter_nr+'\', 1050, 500, \'flab-list\', 1, \'auto\'), ' + 'WIDTH, 1050, TEXTPADDING, 0, BORDER, 0, STICKY, SCROLL, CLOSECLICK, MODAL, CLOSETEXT, \'<img src=../../images/close.gif border=0 onClick=refreshWindow(key,pagekey);>\', '+
