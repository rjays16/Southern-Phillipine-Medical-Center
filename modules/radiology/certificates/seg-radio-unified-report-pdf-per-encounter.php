...
include("roots.php");
include_once($root_path."/classes/fpdf/fpdf_diagnostic.php");
...
// end carriane
// var_dump($_GET['batch_nr_grp']);die;
...
$radio_obj_get_batch = new SegRadio;


$batch = $radio_obj_get_batch->getAllRadiologyBatchPerEncounter($pid, $encounter_nr);
if ($batch) {
	while ($rows_first = $batch->FetchRow()) {
		$batch_nr = $rows_first['refno'];
...
if ($batch_nr){
		if (!($radioResultObj = $radio_obj->getAllInfoUnifiedBatchRequestListInEncounter($batch_nr))){
...
*/
	...
	echo '<p class="warn">Sorry but the page cannot be displayed! <br> No Radiology Results!</p>';
		exit();
}
$name = "RAD-[".$_GET['encounter_nr']."].pdf";
$pdf->Output($name);
