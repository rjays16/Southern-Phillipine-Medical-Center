...
//$db->debug=1;
$breakfile="radiolog.php".URL_APPEND.$obgnyedpt;
 ...
 */
 ...
 $smarty->assign('sToolbarTitle',($_GET['ob']=='OB' ? "OB-GYN :: Finding's Code Manager" : "Radiology :: Finding's Code Manager"));
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',($_GET['ob']=='OB' ? "OB-GYN :: Finding's Code Manager" : "Radiology :: Finding's Code Manager"));
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioFindingsList('"+searchID+"','"+searchEL.value+"',"+page+",'"+ob+"')",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("radiogrouplistTable-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateRadioFindingsList('search','"+keyword+"',"+page+",'"+ob+"')",50);
	lastSearch = keyword;
}
...
function addRadioFindings(listID, id,code, desc, impression) {
	...
	if (list) {
		...
		if (id) {
			...
			// alert(ob);
			if(ob){
				rowSrc = '<tr><td colspan="7">No OB-GYN finding\'s code availables at this time...</td></tr>';

			}else{
				rowSrc = '<tr><td colspan="7">No radiology finding\'s code availables at this time...</td></tr>';
			}
					}
...
function deleteRadioFindings(id){
	...
	var statement = "";
	if(ob){
	 statement = "Are you sure you want to delete the OB-GYN finding's code?";
	}else{
		 statement = "Are you sure you want to delete the radiology finding's code?";
	}
	var answer = confirm(statement);
		if (answer){
...
function AddItem(id,mode){
	...
	var ob = $('obgyne').value;
	if(ob){
		label = "Add Ultrasound findings code";
	}
	else{
		label = "Add Radiology findings code";
	}
	// 
	return overlib(
					OLiframeContent('seg-radio-services-findings.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&id='+id+'&mode='+mode+"&ob=<?=$_GET[
						'ob']?>", 650, 340, 'fGroupTray', 1, 'auto'),
									...
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="refreshWindow(key,pagekey);">',
										 CAPTIONPADDING,4, CAPTION,label,
										 MIDX,0, MIDY,0, 
		...
		<div align="center" style="display:">
			<table width="100%" cellpadding="4">
				...
				<tr>
					...
					<td>
						<input id="search" name="search" class="segInput" type="text" size="30" style="background-color:#e2eaf3; border-width:thin; font:bold 13px Arial" align="absmiddle" onkeyup="if (this.value.length >= 1) startAJAXSearch(this.id,0)" />
