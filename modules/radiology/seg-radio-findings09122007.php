...
<form name="form_test_findings" id="form_test_findings" method="post" action="<?php echo $thisfile ?>" onSubmit="return checkFindingsForm()">
<!--
	...
	<table border=0 bgcolor="#000000" cellpadding=1 cellspacing=0>
		...
		<tr>
			<td colspan="*" align="left" bgcolor="#ffffff" >
				<input type="image" name="saveButton" id="saveButton" <?php echo createLDImgSrc($root_path,'savedisc.gif','0') ?>  title="<?php echo $LDSaveEntry ?>" onClick="$('save').value=1;"> 
			</td>
		</tr>
		<tr>
			<td>	
...
<?php
	...
	foreach($findings_array as $key_finding => $value_finding){
		if($toggle) { $class_style='wardlistrow2'; $toggle=0; } 
		else { $class_style='wardlistrow1'; $toggle=1; }
		$count_find++;
#		$personell_obj->getPersonellInfo($doctor_in_charge_array[$key_finding]);
		$report_doc_name = "";
...
?>	
										<tr class="<?= $class_style ?>" style="font-weight:bold;padding:0px">
											<td align="center">
												<b><?php echo $count_find;  ?></b>
											</td>
											<td>
												<b><?php echo $report_doc_name; ?></b>
											</td>
											<td>
												<b><?php echo $value_finding;  ?></b>
											</td>
											<td>
												<b><?php echo $radio_impression_array[$key_finding];   ?></b>
											</td>
											<td>
												<b>
												<?php 
													if (	(trim($findings_date_array[$key_finding])!='0000-00-00')  
															&& (trim($findings_date_array[$key_finding])!=""))
														echo formatDate2Local($findings_date_array[$key_finding],$date_format);  
												?></b>
											</td>
<?php
		if ($radioRequestInfo['status']!='done'){
?>
											<td align="center">
<!--
												<input type="button" name="edit" id="edit" value="Edit" onClick="popEditFinding(<?= $batch_nr ?>,<?= $key_finding ?>);">
			<input type='image' <?= createLDImgSrc($root_path,'edit_sm.gif','0') ?> title='Edit finding'name="edit" id="edit" onClick="popEditFinding(<?= $batch_nr ?>,<?= $key_finding ?>);">
-->			

<a href="javascript:void(0);"
	onclick="return overlib(
		OLiframeContent('seg-radio-findings-edit.php<?= URL_APPEND ?>&batch_nr=<?=$batch_nr?>&findings_nr=<?=$key_finding?>', 500, 475, 'if1', 1, 'auto'),
	...
	onmouseout="nd();">
	<img name="edit<?=$key_finding?>" id="edit<?=$key_finding?>" <?= createLDImgSrc($root_path,'edit_icon_06.gif','0') ?>>
</a>

											</td>
											<td align="center">
<!--
												<input type="button" name="delete" id="delete" value="Delete" onClick="deleteFinding(<?= $batch_nr ?>,<?= $key_finding ?>);">
-->
<img name="delete<?=$key_finding?>" id="delete<?=$key_finding?>" <?= createLDImgSrc($root_path,'trash_06.gif','0') ?> onClick="deleteFinding(<?= $batch_nr ?>,<?= $key_finding ?>);">
											</td>
<?php
		}# end of if-stmt 'if ($radioRequestInfo['status']!='done')'
?>
...
?>
<?php 
   $temp = unserialize($radioRequestInfo['findings']);
	if (empty($temp) || !$radioRequestInfo['findings']){
#	if (empty($radioRequestInfo['findings']) || !$radioRequestInfo['findings']){
?>
										<tr>
...
<a href="javascript:void(0);"
	onclick="return overlib(
		OLiframeContent('seg-radio-findings-edit.php<?= URL_APPEND ?>&batch_nr=<?=$batch_nr?>&findings_nr=<?=$count_find?>', 500, 475, 'if1', 1, 'auto'),
			WIDTH,500, TEXTPADDING,0, BORDER,0, 
			STICKY, SCROLL, CLOSECLICK, MODAL, DRAGGABLE,
			CLOSETEXT, '<img src=<?= $root_path ?>images/x.gif border=0>',
...
<?php 
			...
			# burn added : August 27, 2007
			if ($count_find){
?>
	<input type="image" name="referralButton" id="referralButton" <?php echo createLDImgSrc($root_path,'referral.gif','0') ?>  alt="Referral" onClick="$('save').value=1; referralHandler();">
<?php
			}# end of if-stmt 'if ($count_find)'
		}# end of if-stmt 'if ($radioRequestInfo['status']!='done')'
?>
								</div>
							</td>
...
?>
	<input type="image" name="saveButton" id="saveButton" <?php echo createLDImgSrc($root_path,'savedisc.gif','0') ?>  title="<?php echo $LDSaveEntry ?>" onClick="$('save').value=1;"> 
	<input type="image" name="saveDoneButton" id="saveDoneButton" <?php echo createLDImgSrc($root_path,'save_done_02.gif','0') ?>  alt="Save&Done" onClick="$('save').value=1; saveAndDone();">
<?php
		}else{# end of if-stmt 'if ($radioRequestInfo['status']!='done')'
?>
	<input type="button" name="print" id="print" value="Print Report" onClick="printRadioReport();">
<?php
...
?>
	...
	<input type="hidden" name="count_find" id="count_find" value="<?= $count_find ?>">
...
//			alert("false : $F('save') = '"+$F('save')+"'");
	...
	function saveAndDone(){
		if (checkFindingsForm()){
			$('mode').value = 'save';
			$('status').value = 'done';
			alert('Save and done sucessfully!');
			fSubmit('form_test_findings');
		}
	}
	...
	function deleteFinding(batch_nr,nr){
		...
		if (answer){
			$('mode').value = 'delete';
			$('finding_nr').value = nr;
		//delete this try-finally stmt if the MODAL pop-up is working properly :=)
			fSubmit('form_test_findings');			
		}
	}
/*	
		...
		if (list) {
			...
			if (dBody) {
				...
				if (details) {
					...
					src = "<tr><td colspan=\"7\">Findings list is currently empty...</td></tr>";	
				}
				dBody.innerHTML += src;
				return true;
			}
...
/*
					src = 
						'<tr class="wardlistrow'+alt+'" id="row'+details.no+'" style="font-weight:bold;padding:0px"> '+
						'	<td align="center"><b> '+(parseInt(details.no)+1)+'</b></td> '+
						'	<td><b> '+details.docName+' </b></td> '+
						'	<td><b> '+details.finding+' </b></td> '+
						'	<td><b> '+details.r_impression+' </b></td> '+
						'	<td><b> '+details.f_date+' </b></td> ';
					src +=
						'	<td align="center">
<a href="javascript:void(0);"
	onclick="return overlib(
