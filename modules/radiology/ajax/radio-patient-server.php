...
*/
	$tr  = "<thead>";
	$tr .= "<tr><th colspan=\"11\" id=\"mainHead".$sub_dept_nr."\"></th></tr>";
	$tr .= "<tr>";
#	$tr .= "<th width=\"1%\">";
...
#	$tr .= "</th>";
	...
	$tr .= makeSortLink('Borrower','', $oitem, $odir, $sub_dept_nr,'12%');
	$tr .= makeSortLink('Date Borrowed','', $oitem, $odir, $sub_dept_nr,'12%');
	$tr .= makeSortLink('Date Returned','', $oitem, $odir, $sub_dept_nr,'12%');
...
#$objResponse->addAlert("populatePatientRecords : ergebnis = ".$ergebnis);
		...
		#$objResponse->addAlert("populatePatientRecords : ergebnis = ");
#			if ($temp==0){
...
$msg ="\n date_format = '".$date_format."'".
		...
		#$objResponse->addAlert($row['batch_nr']);
		$rs = $objRadio->getBorrowedInfo($row['batch_nr']);
		if ($rs)
			$row2= $rs->FetchRow();
		#$objResponse->addAlert(print_r($row2));

		if ($row['is_borrowed']=='done'){
			$date_borrowed = "";
			$date_returned = "";
			$borrower = "";
		}else{
			$date_borrowed = date("m/d/Y",strtotime($row2['date_borrowed'])).' '.date("h:i A",strtotime($row2['time_borrowed']));
			$borrower = $row2['borrower'];

			if ($row['is_borrowed']=='borrowed')
				$date_returned = 'Still Borrowed';
			else
				$date_returned = date("m/d/Y",strtotime($row2['date_returned'])).' '.date("h:i A",strtotime($row2['time_returned']));
		}
#$objResponse->addAlert("populatePatientRecords: $msg");
			$objResponse->addScriptCall("jsRadioRequest",$sub_dept_nr,$my_count,$row['batch_nr'],
														...
														$date_request,$date_service,$available, $borrower, $date_borrowed, $date_returned);
			$my_count++;
		}//end while loop
	//end if (ergebnis)
	}else{
...
#		$objResponse->addAlert("setDoctors : pers_obj->sql = '".$pers_obj->sql."'");
		...
		if ($rs) {
			...
			while ($result=$rs->FetchRow()) {
				...
				$doctor_name = mb_strtoupper($result["name_last"]).", ".mb_strtoupper($result["name_first"])." ".mb_strtoupper($result["name_2"]).", MD";
...
#$objResponse->addAlert("saveRadioBorrow :: data : \n".print_r($data,true));
		...
		if ($borrow_nr=$objRadio->createBorrowEntry($batch_nr, $borrower_id, $date_borrowed, $time_borrowed,
									 $releaser_id, addslashes($releaser_fullname), addslashes($remarks))){
