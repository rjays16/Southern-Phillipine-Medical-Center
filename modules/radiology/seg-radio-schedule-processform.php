...
<?php 
 ...
 while($row=$rs->FetchRow()){
 	$sFilmSized .= '
					<tr>
						<td width="2%"><input type="checkbox" id="size'.$i.'" name="size'.$i.'" value="'.$row['id'].'" onClick="enableNoSize('.$i.');"; /></td>
						<td style="font-weight:bold">'.$row['size'].'</td>
						<td style="font-weight:bold"><input type="text" id="no_film_used'.$i.'" readonly size="5" name="no_film_used'.$i.'" value="'.$rowres['no_film_used'].'"></td>
					</tr>';
	$i++;				
 }
 
 $smarty->assign('sFilmSized',$sFilmSized);
 #--------------------------
...
<input type="hidden" name="filmcnt" id="filmcnt" value="<?=$filmcnt?>" />
...
$smarty->assign('sMainBlockIncludeFile','radiology/schedule_form_processframe.tpl');
 /**
 * show Template
 */
