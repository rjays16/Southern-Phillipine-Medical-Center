...
function startAJAXSearch(searchID, page) {
	...
	var dept_OB1 = $('OBgyne').value;
  	var ob_gyne1 = "209";
	var aRadioDept = dept_OB1 =="OB" ? ob_gyne1 : $("dept_nr").value;
  	 if(dept_OB1 =="OB"){
    		$("dept_nr").value = ob_gyne1;
   	}
   	var group_code = dept_OB1 =="OB" ? $("sub_service_group").value : "";
	var keyword;
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioServiceList('"+ptype+"','"+area_type+"','"+is_cash+"','"+discountid+"','"+discount+"','"+is_senior+"','"+is_walkin+"','"+aRadioDept+"','"+source_req+"','"+isStat+"','"+is_charge2comp+"','"+compID+"','"+searchID+"','"+keyword+"',"+page+",'"+group_code+"')",100);
...
function prepareAdd_Package(id,name,cash,charge,sservice,in_pacs,pacs_code,group,net_price,ctscan,mri) {
...
function prepareAdd_NotPackage(id,ctscan,mri) {
	...
	//if (checkRequestDetails(id)){
		...
		details.in_pacs = $('in_pacs'+id).value;
		details.pacs_code = $('pacs_code'+id).value;
		...
		if(from_dept=='OB'){
			...
			details.net_pf = $('net_pf'+id).value;
		}
		details.fromdept = from_dept;
		var list = window.parent.document.getElementById('order-list');
...
function addProductToList(listID, id, name, grp_code, dept_name, cash, charge, sservice, in_pacs, pacs_code,net_price,pf,net_pf,available) {
		...
		if (id) {
				...
				if(from_dept=='OB'){
					rowSrc = "<tr>"+
									...
									'	<input id="net_pf'+id+'" type="hidden" value="'+net_pf+'"/>'+
									'	<input id="in_pacs'+id+'" type="hidden" value="'+in_pacs+'"/>'+
									'	<input id="pacs_code'+id+'" type="hidden" value="'+pacs_code+'"/>'+
									'</td>'+
