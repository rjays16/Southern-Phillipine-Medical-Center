...
function editService(nr,rowno){
	...
	*/
	...
	var ww=500;
	var wh=500;
	urlholder="<?php echo $root_path ?>modules/radiology/seg-radio-services-edit031508.php?sid=<?php echo "$sid&lang=$lang" ?>&nr="+escape(nr)+"&deptnr="+deptnr+"&grpcode="+grpcode+"&row="+rowno;
...
function ajxClearOptions() {
	...
	function fetchServList(ms) {
		...
		if ((aLabServ_grpcode!="none")&&(serv_code)){
			...
			//timeoutHandle=setTimeout("xajax_psrv('"+aLabServ_grpcode+"','a',0)",ms);
		}	
	}
