 ...
 */
 ...
 $smarty->assign('sToolbarTitle',($_GET['ob']=='OB' ? "OB-GYN Ultrasound ::  Group Manager" : "Radiology :: Group Manager"));
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',($_GET['ob']=='OB' ? "OB-GYN Ultrasound ::  Group Manager" : "Radiology :: Group Manager"));
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioGroupList('"+searchID+"','"+searchEL.value+"',"+$F('dept_nr')+","+page+",'"+raddept+"')",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("radiogrouplistTable-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateRadioGroupList('search','"+keyword+"',"+$F('dept_nr')+","+page+",'"+raddept+"')",50);
	lastSearch = keyword;
}
	...
	if (list) {
	   dBody=list.getElementsByTagName("tbody")[0];
		...
		if (id) {
			...
			if(raddept=='OB'){
			rowSrc = '<tr><td colspan="7">No OB-GYN groups available at this time...</td></tr>';
			}else{
...
function deleteRadioGroup(id, dept_nr){
		...
		var answer = confirm("Are you sure you want to delete the "+text+" Group?");
		if (answer){
			xajax_deleteRadioGroup(id, dept_nr,tagging);
		}
}
...
function AddItem(id,mode,dept_nr){
	...
	if(raddept=='OB'){
			text = " OB-GYN Service Group";
	}else{
			text = " radiological service group";
	}
	if(mode=='save')
		caption = "Add";
	else
	...
	return overlib(
          OLiframeContent('seg-radio-services-groups.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&ob=<?=$_GET['ob']?>&id='+escape(id)+'&mode='+mode+'&dept_nr='+dept_nr, 350, 200, 'fGroupTray', 1, 'auto'),
						         ...
						         CAPTIONPADDING,4, CAPTION,caption+ text,
						         MIDX,0, MIDY,0, 
						         STATUS,caption+ text);							
}
		...
		<div align="center" style="display:">
			<table width="100%" cellpadding="4">
				<tr>
					...
					if($obgynedept=='OB'){
					?>
						<td width="28%"><span><strong>OB-GYN Section</strong></span></td>
					<?php
					}else{ $dept_obj->ob_parent_nr='209';
					?>
