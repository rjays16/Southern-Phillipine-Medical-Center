...
#$breakfile=$root_path.'modules/laboratory/labor_test_request_pass.php'.URL_APPEND.'&target=radio_undone&user_origin=radio&dept_nr=158';   # burn added : November 16, 200
$breakfile=$root_path.'modules/laboratory/labor_test_request_pass.php'.URL_APPEND.'&target='.$target.'&user_origin=radio&dept_nr=158';   # burn added : November 16, 200
...
#		}else{# end of if-stmt 'if ($radioRequestInfo['status']!='done')'
	$onClick="
		onClick=\" var sub_dept_nr_temp = $('sub_dept_nr').value; return overlib(
			OLiframeContent('seg-radio-findings-select-batchNr.php".URL_APPEND."&batch_nr=".$batch_nr."&pid=".$pid."&refno=".$refno."&sub_dept_nr='+sub_dept_nr_temp, 575, 475, 'if1', 1, 'auto'),
				WIDTH,575, TEXTPADDING,0, BORDER,0, 
				STICKY, SCROLL, CLOSECLICK, MODAL, DRAGGABLE,
				CLOSETEXT, '<img src=".$root_path."images/x.gif border=0>',
				CAPTIONPADDING,4, CAPTION,'Print Results', MIDX,0, MIDY,0, 
