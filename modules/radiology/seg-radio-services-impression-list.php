 ...
 */
 ...
 $smarty->assign('sToolbarTitle',($_GET['ob']=='OB' ? "OB-GYN :: Impression's Code Manager" : "Radiology :: Impression's Code Manager"));
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',($_GET['ob']=='OB' ? "OB-GYN :: Impression's Code Manager" : "Radiology :: Impression's Code Manager"));
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioImpressionList('"+searchID+"','"+searchEL.value+"',"+page+",'"+ob+"')",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("radiogrouplistTable-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateRadioImpressionList('search','"+keyword+"',"+page+",'"+ob+"')",50);
	lastSearch = keyword;
}
...
function addRadioFindings(listID, id,code, desc) {
	...
	if (list) {
		...
		if (id) {
			...
			if(ob){
				rowSrc = '<tr><td colspan="7">No OB-GYN impression\'s code available at this time...</td></tr>';
			}else{
...
function deleteRadioFindings(id){
	...
	var statement = "";
	if(ob){
		statement= "Are you sure you want to delete the OB-GYN impression's code?";
	}else{
		statement ="Are you sure you want to delete the radiology impression's code?";
	}
...
function AddItem(id,mode){
	...
	var ob =$('obgyne').value;
	if(ob){
		label = 'Add impressions code';
	}
	else{
		label = 'Add radiological impressions code';
	}
	return overlib(
					OLiframeContent('seg-radio-services-impression.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&ob=<?=$_GET['ob']?>&id='+id+'&mode='+mode, 650, 340, 'fGroupTray', 1, 'auto'),
