...
echo"radiologists-dienstplan-planen.php : _GET : <br> \n"; print_r($_GET); echo" <br> \n";
...
function test_print($item2, $key) 
{
 ...
 echo "radiologists-dienstplan-planen.php : Before ...: <br>";
 ...
 echo "<br>";
   ...
   if ($this->PRequest){
      ...
      if (!empty($this->DOCDuty['count'])){
	     echo "radiologists-dienstplan-planen.php : this->DOCDuty[count] is not empty <br> <br>";
            /* DOC exists */
         $rowRequest="";
		 $i=0;
         $encoder = $HTTP_SESSION_VARS['sess_user_name'];
		 echo "radiologists-dienstplan-planen.php : encoder = ".$encoder." <br> ";
...
echo "<!--";
?>
<form name="dienstplanDoctorScheduler" action="<?= $thisfile ?>" method="post">
...
#	  $doctorInfo="";
      ...
      if ($this->doctorInfo = $sked_obj->getDoctorName($rowRequest['batch_nr'])){
	     ...
	     echo ' <input type="hidden" name="docID_p'.$my_count.'" value="'.$temp_row['personell_nr'].'"> 
              ...
              <a href="javascript:popselectRadiologists(\''.$my_count.'\',\'p\')"> 
	          <button onclick="javascript:popselectRadiologists(\''.$my_count.'\',\'p\')"><img '.createComIcon($root_path,'patdata.gif','0').' alt="'.$LDClk2Plan.'"></button></a> ';
      }
      else{
...
for ($i=1,$n=0,$wd=$firstday;$i<=$maxdays;$i++,$n++,$wd++)
{
	...
	$smarty->assign('sPopWin1','<a href="javascript:popselectRadiologists(\''.$n.'\',\'a\')">
	<button onclick="javascript:popselectRadiologists(\''.$n.'\',\'a\')"><img '.createComIcon($root_path,'patdata.gif','0').' alt="'.$LDClk2Plan.'"></button></a>');
	...
	$smarty->assign('sPopWin2','<a href="javascript:popselectRadiologists(\''.$n.'\',\'r\')">
	<button onclick="javascript:popselectRadiologists(\''.$n.'\',\'r\')"><img '.createComIcon($root_path,'patdata.gif','0').' alt="'.$LDClk2Plan.'"></button></a>');
	if($wd==6) $wd=-1;
	...
	ob_start();
		...
		# $sTemp = $sTemp.ob_get_contents();   # burn commented : July 18, 2007
	ob_end_clean();
}
/*
