...
//commented & edited by VAN 03-19-08
/*
function emptyTray() {
...
//	alert("refreshTotal : subTotal.innerHTML='"+subTotal.innerHTML+"' \nnetTotal.innerHTML='"+netTotal.innerHTML+"'");
	*/
	...
	if ($('repeat').checked){
		...
		totalamt = parseInt(subTotal.innerHTML) + parseInt(discountTotal.innerHTML);
...
//	$('orderaddress').readOnly=!iscash;
	...
	*/
	...
	if (($F('hasPaid')==1)||($F('view_from')=='ssview')||($('repeat').checked)){	
		...
		$('request_date_trigger').style.cursor='default';
...
// commented by VAN 01-15-08
//function initialRequestList(serv_code,grp_code,name,c_info,r_doc,r_doc_name,n_house,cash,charge,hasPaid,sservice) {
function initialRequestList(serv_code,grp_code,name,c_info,r_doc,r_doc_name,n_house,cash,charge,hasPaid,sservice,parent_batch,head,remarks) {
...
//alert("batch, head, remark = "+parent_batch+" - "+head+" - "+remarks);
