...
$breakfile="radiolog.php".URL_APPEND.$obgynedpt;
 ...
 */
 ...
 $smarty->assign('sToolbarTitle',($_GET['ob']=='OB' ? "OB-GYN Ultrasound ::Services Manager" : "Radiology ::Services Manager"));
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',($_GET['ob']=='OB' ? "OB-GYN Ultrasound ::Services Manager" : "Radiology ::Services Manager"));
...
function deleteService(code){
	...
	var answer = confirm("Are you sure you want to delete the "+text+" Service with a code of "+(code.toUpperCase())+"?");
		if (answer){
			xajax_deleteService(code,tagging);
		}
}
...
function jsGetServiceGroup(d){
		var obgnye = $('OBgyne').value;
	 xajax_getServiceGroup($F('dept_nr'),obgnye);
}
...
function addProductToList(listID, id, name, cash, charge, sservice, available, is_ER, is_IC,pf,is_pf) {
	var list=$(listID), dRows, dBody, rowSrc;
	var i;
	var classified, mode, editlink;
	var rad_dept = "<?php echo $_GET['ob']?>";
...
//alert('sulod listID, id, name, cash, charge, sservice = '+listID+" , "+id, name+" , "+cash+" , "+charge+" , "+sservice);
		...
		if (id) {
			...
			if (is_pf==1)
				 available_label_pf = '<td style="color:#007" align="center">Yes</td>';
			else
				 available_label_pf = '<td style="color:#FF0000" align="center">No</td>';
            ...
            if (is_IC==1)
                 inIC_label = '<td style="color:#007" align="center">Yes</td>';
            else
                 inIC_label = '<td style="color:#FF0000" align="center">No</td>';
            // end Gervie   
            if(rad_dept=='OB'){
			...
			if(rad_dept=='OB'){
					rowSrc = '<tr><td colspan="11" style="">No such OB-GYN Service exists...</td></tr>';
			}else{
					rowSrc = '<tr><td colspan="11" style="">No such radiological service exists...</td></tr>';
			}
...
function AddItem(code,mode){
	...
	return overlib(
				 OLiframeContent('seg-radio-services-edit.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&nr='+escape(code)+"&grpcode="+grpcode+"&mode="+mode+"&deptnr="+deptnr+"&ob="+ob, 500, 450, 'fGroupTray', 1, 'auto'),
									...
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="refreshWindow(key,pagekey);">',
										 CAPTIONPADDING,4, CAPTION,label+label_dept+' service',
										 MIDX,0, MIDY,0,
	...
	<div id="tabFpanel">
		<div align="center" style="display:">
			<table width="100%" cellpadding="4" border="0">
							...
							if(!$OBgynedept){
								...
								if(is_object($all_meds)){
									while($deptrow=$all_meds->FetchRow()){
											...
											else echo 'Obstetrics and Gynecology';
											echo '</option>';
											}
				...
				<tr>
					<?php if($_GET['ob']){?>
					<td width="28%"><span><strong>OB-GYN Service Section</strong></span></td>
					<?php } else{?>
				...
				<tr>
					...
					<td>
						...
						<input type="image" id="search_img" name="search_img" src="<?= $root_path ?>images/his_searchbtn.gif" onclick="startAJAXSearch('search',0);return false;" align="absmiddle" disabled="if(this.value!=0) false" /><br />
					</td>
				</tr>
			</table>
		</div>
...
<table id="ServiceList" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
	<thead>
		...
		<tr>
			...
			<?php if($_GET['ob']){?>
				<th width="15%" align="left">PF</th>
				<th width="9%" align="left">Discountable PF</th>
			<?php } ?>
			<th width="9%" align="center">Discountable</th>
			<th width="5%" align="center">Available</th>
			<?php if(!$_GET['ob']){?>
            <th width="5%" align="center">Is ER</th>
            <th width="5%" align="center">Is IC</th>
            	<?php } ?>
	...
	<tbody id="ServiceList-body">
		<tr>
			<?php if($_GET['ob']){?>
				<td colspan="11">No such OB-GYN Service exists...</td>
			<?php }else{
				?>
				<td colspan="11">No such radiological service exists...</td>
		<?php } ?>
		</tr>
	</tbody>
</table>
