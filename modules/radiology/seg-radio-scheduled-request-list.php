...
define('OB_DEPT_TAB',tab209);

if($_GET['ob']){
	$obgy = "&ob=OB";
}

$breakfile = "radiolog.php".URL_APPEND.$obgy;
 ...
 */
 ...
 $smarty->assign('sToolbarTitle',($_GET['ob']=='OB'?  "OB-GYN Ultrasound :: Scheduled Request List " : "Radiology :: Scheduled Request List"));
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',($_GET['ob']=='OB'?  "OB-GYN Ultrasound :: Scheduled Request List " : "Radiology :: Scheduled Request List"));
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
       ...
       AJAXTimerID = setTimeout("xajax_populateScheduledList('"+searchID+"','T"+tab+"','"+keyword+"',"+page+",'"+obgyne.value+"')",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("T"+tab).style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateScheduledList('search','T"+tab+"','"+keyword+"',"+page+",'"+obgyne.value+"')",100);
    lastSearch = keyword;
}
		...
		if (refno) {
            ...
            if (is_served==1)
                ...
                if (disabled_icon==1){
                    ...
                    serve = 'onclick="return overlib(OLiframeContent(\'seg-done-radtech-list.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&code='+code+'&rad_dept='+obgyne+'&is_served=1&refno='+refno+'&batch_nr='+batch_nr+'\', 400, 200, \'flab-list\', 1, \'auto\'), ' +
			...
			<table width="100%" cellpadding="4">
				<tr>
					<td width="30%" align="center">
						<?php  if ($obgyne!='OB'){?>
						<span>Enter the search key (HRN, Batch No., Family Name or Scheduled Date).
							<?php  } else{  ?>
								<span>Enter the search key (HRN, Family Name , Case no. or Scheduled Date).
							<?php } ?>
...
<input type="hidden" id="obdept" name="obdept" value="<?=OB_DEPT_TAB?>">
