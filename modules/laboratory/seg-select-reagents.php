...
$cat = "pharma";
$title="Laboratory::Select reagent";
...
function startAJAXSearch(searchID, page) { 	
	...
	if (searchEL && searchEL.value.length >= 3) {
		...
		AJAXTimerID = setTimeout("xajax_populateReagentList('"+searchID+"','"+searchText+"',"+page+")",100);
	}
}
...
<script type="text/javascript" src="<?=$root_path?>modules/laboratory/js/reagent-search-gui.js?t=<?=time()?>"></script>
			...
			<tr>
				<td>
					<div style="display:block; border:1px solid #8cadc0; width:100%; background-color:#e5e5e5">
						<table border="0" id="labreagentslistTable" class="jedList" cellpadding="0" cellspacing="0" width="100%">
							<thead>
								<tr class="nav">
									<th colspan="9">
								...
								<tr>
									<th width="20%">Code</th>
									<th width="*">Reagents</th>
									<th width="30%">Other Name</th>
									<th width="5%">&nbsp;</th>
									<th width="1%">&nbsp;</th>
								</tr>
							</thead>
							<tbody id="labreagentslistTable-body">
								<tr>
									<td colspan="9">No such reagent exists...</td>
								</tr>
							</tbody>
						</table>
