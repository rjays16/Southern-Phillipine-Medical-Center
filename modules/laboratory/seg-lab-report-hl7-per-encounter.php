...
require('./roots.php');
require($root_path.'classes/fpdf/fpdf_diagnostic.php');
...
require_once($root_path.'include/care_api_classes/seg_hl7/seg_class_hl7_per_encounter.php');

define('FONT_INCREMENT', 1);

class lab_pdf_per_encounter extends FPDF {
...
var $pid,$lis,$sql,$encounter_nr;
    ...
    function Results(){
        ...
        $hl7fxnObj = new seg_HL7_per_encounter();

        

        $hrn = $this->pid;
        $encounter_nr = $this->encounter_nr;
        $encounter_detail = $hl7fxnObj->getEncounterDateDetail($encounter_nr);
        $detail = $encounter_detail->FetchRow();
        $dateAndTime = $hl7fxnObj->getAllLisOrderNumberByEncounter($hrn, $encounter_nr, $detail['encounter_date'], $detail['discharge_date'], $detail['discharge_time']);
        // var_dump($detail['discharge_date']);die;
        if($dateAndTime && $detail['discharge_date'] &&  $detail['discharge_time']){
        // if($dateAndTime ){
            // var_dump($dateAndTime);die;
        while($row_lis_order_no=$dateAndTime->FetchRow()){
            ...
            $t_details = array();
            $t_info = array();
        $rs = $hl7fxnObj->getAllResultByOrder($hrn, $row_lis_order_no['lis_order_no']);
        if (is_object($rs)){    
            ...
            $msg = "";
            if ($dateAndTime == "0") {
                 $msg = "Sorry but the page cannot be displayed! <br> No Laboratory Results";
            }else{
                  $msg = "Sorry but the page cannot be displayed! <br> Please Discharge and Final Bill the patient first";
            }
             echo "<html><head></head><body>'$msg'</body></html>";
             die();
        }
                    ...
                    foreach ($t_group as $key2 => $t_name) {
                        ...
                        if(!is_array($t_name)){
                            ...
                            foreach ($t_name as $key3 => $t_details) {
                                ...
                                $this->Cell(20,4,$t_details[3], 0, 0,'L');
    ...
    function Signatory($medtech,$pathologist,$cur_group){
        $this->Ln();
        $this->Ln();
        $this->SetFont('Courier','B',6+FONT_INCREMENT);
        $this->Cell(10,4,'', 0, 0,'C');//indent
        $this->Cell(80,4,$medtech, 0, 0,'C');//name
        $this->Cell(55,4,"", 0, 0,'C');//separate
        $this->Cell(10,4,$pathologist, 0, 1,'C');//name
        $this->SetFont('Courier','',7+FONT_INCREMENT);
        $this->Cell(10,4,'', 0, 0,'C');//indent
    ...
    function lab_pdf_per_encounter($pid,$encounter_nr){
    ...
    function outputFile($mode=false){
        $this->AliasNbPages();
        $this->Results();
        $name = "LAB-[".$this->encounter_nr."].pdf";
        if($mode){
...
if(isset($_GET['pid']) && isset($_GET['encounter_nr'])){
    // var_dump(1);die;
    $fpdf = new lab_pdf_per_encounter($_GET['pid'],$_GET['encounter_nr']);
    $fpdf->outputFile();
}