...
{
	urlholder="<?php echo $root_path ?>modules/laboratory/seg-lab-services-edit.php?sid=<?php echo "$sid&lang=$lang" ?>&nr="+nr+"&row="+rowno;
...
$sTemp = '<select id="parameterselect" name="parameterselect" size="1" onChange="jsGetLabService(paramselect,1);jsGetServiceGroup(paramselect);">
	       <option value="0">Select a Department first</option>';
/*foreach ($groups as $gr) {  #VAS
...
#$sTemp = $sTemp.'</select><font size=1><img '.createComIcon($root_path,'redpfeil_l.gif','0','',TRUE).'> Service Group</font>';
$sTemp = $sTemp.'</select><font size=1><img '.createComIcon($root_path,'redpfeil_l.gif','0','',TRUE).'> Laboratory Service Group</font>';
