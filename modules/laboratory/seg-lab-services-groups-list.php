 ...
 */
 ...
 $smarty->assign('sToolbarTitle',"$LDLab :: Laboratory Group Manager");
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$LDLab :: Laboratory Group Manager");
...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateLabGroupList('"+searchID+"','"+searchEL.value+"',"+page+")",50);
...
function startAJAXSearch2(keyword, page) {
	...
	$("ajax-loading").style.display = "";
	$("labgrouplistTable-body").style.display = "none";
	AJAXTimerID = setTimeout("xajax_populateLabGroupList('search','"+keyword+"',"+page+")",50);
	lastSearch = keyword;
}
...
function addLabGroup(listID, id, groupName, groupOtherName) {
	var list=$(listID), dRows, dBody, rowSrc;
	var i;
	
	//alert('listID, id, groupId, groupName = '+listID+" , "+id+" , "+groupId+" , "+groupName);
	   ...
	   dBody=list.getElementsByTagName("tbody")[0];
		...
		if (id) {
			...
			rowSrc = '<tr class="wardlistrow'+alt+'" id="row'+addslashes(id)+'">'+
								'<td width="1%">&nbsp;</td>'+
								'<td width="*">'+groupName+'</td>'+
								'<td width="20%">'+id+'</td>'+
								'<td width="28%">'+groupOtherName+'</td>'+
             				'<td width="5%" align="center">'+info+'</td>'+
								'<td width="5%" align="center"><img name="delete'+addslashes(id)+'" id="delete'+id+'" src="../../images/delete.gif" style="cursor:pointer" border="0" onClick="deleteLabGroup(\''+addslashes(id)+'\');"/></td>'+
								'<td width="1%">&nbsp;</td>'+
								'</tr>';				
		}
		else {
			rowSrc = '<tr><td colspan="8">No laboratory groups available at this time...</td></tr>';
		}
...
//alert('id = '+id);
		...
		return overlib(
          OLiframeContent('seg-lab-services-groups.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&id='+escape(id)+'&mode='+mode, 350, 200, 'fGroupTray', 1, 'auto'),
						         ...
						         CAPTIONPADDING,4, CAPTION,caption+' laboratory service section',
						         MIDX,0, MIDY,0, 
						         STATUS,caption+' laboratory service section');							
		/*
		return overlib(
          OLiframeContent('seg-lab-services-groups.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&id='+id+'&mode='+mode, 305, 139, 'fGroupTray', 1, 'auto'),
          						WIDTH,305, TEXTPADDING,0, BORDER,0, 
									STICKY, SCROLL, CLOSECLICK, MODAL, 
			...
			<table width="100%" cellpadding="4">
				<tr>
					<td width="30%" align="center">
						<span>Enter search keyword: e.g. Section Name, all data (just type: * or space)</span>
						<br><br>
...
<table id="labgrouplistTable" class="segList" width="100%" border="0" cellpadding="0" cellspacing="0">
	<thead>
		<tr>
			...
			<th width="*" align="left">Section Name</th>
			<th width="20%" align="left">Section Code</th>
