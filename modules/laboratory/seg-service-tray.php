...
function startAJAXSearch(searchID, page) {
	...
	var grpcode = $('grpcode').value;
	var is_param = $('is_param').value;

	if (searchEL) {
		searchEL.style.color = "#0000ff";
		if (AJAXTimerID) clearTimeout(AJAXTimerID);
		$("ajax-loading").style.display = "";
		AJAXTimerID = setTimeout("xajax_populateLabService('"+searchID+"','"+grpcode+"','"+is_param+"','"+searchEL.value+"',"+page+")",100);
...
<script type="text/javascript" src="<?=$root_path?>modules/laboratory/js/lab-service-tray-gui.js?t=<?=time()?>"></script>
...
?>
			...
			<tr>
				<td style="font:bold 12px Arial; background-color:#e5e5e5; color: #2d2d2d" >
					<div style="padding:4px 2px; padding-left:10px; ">
						Search Lab Test <input id="search" name="search" class="segInput" type="text" style="width:60%; margin-left:10px; font: bold 12px Arial" align="absmiddle" onkeyup="if (this.value.length >= 3) startAJAXSearch(this.id,0)" onKeyPress="checkEnter(event,this.id)"/>
				...
				<td>
					<div style="display:block; border:1px solid #8cadc0; overflow-y:hidden; height:335px; width:100%; background-color:#e5e5e5">
						<table id="product-list" class="segList" cellpadding="1" cellspacing="1" width="100%">
							...
							<tbody>
								<tr>
									<td colspan="9" style="font-weight:normal">No such laboratory service exists</td>
								</tr>
							</tbody>
						</table>
	...
	<input type="hidden" name="is_param" id="is_param" value="<?=$is_param?>" />
