...
//function nrow(grp, code, name, cash, charge, cashC1, cashC2, cashC3, flash) {
function nrow(grp, code, name, cash, charge, flash) {	
	var dRowSrc,
		cashprc, chargeprc; //, cashprcC1, cashprcC2, cashprcC3;
	//alert("code = "+code);
	if (code) {
		if (isNaN(cash))
			cashprc="N/A";
		...
		dRowSrc = '<tr id="srvRow{{ROWNO}}" height="24">' +
					...
					'<td align="center"><a href="" id="srvDel{{ROWNO}}" onclick="if(confirm(\'Do you wish to remove this service?\')) { xajax_dsrv({{ROWNO}},\''+code+'\'); closechild();} return false;" style="text-decoration:underline">Delete</a></td>'+
					'</tr>';
	}
	else {		
		//alert("grp = "+grp);
		if (grp == 0){
			...
			dRowSrc = '<tr id="srvRow{{ROWNO}}">' +
				'<td colspan="9">No services are available for this group...</td>' +
				'</tr>';
		}
	}
...
//function xrow(rowno, code, name, cash, charge, cashC1, cashC2, cashC3) {
...
//function validate(code, name, cash, charge, cashC1, cashC2, cashC3) {
function validate(code, name, cash, charge) {	
	...
	//alert("echarge ="+echarge.value);
	//alert("ecash = "+ ecash.value);
	...
	*/
	if($F('parameterselect')==0){
		alert("Please select a laboratory service group.");
	...
	*/
	...
	if ((isNaN(ecash.value))||(ecash.value=="")) {	
		alert("Enter the price(cash)...");
		ecash.focus();
		return false;
	}
	if ((isNaN(echarge.value))||(echarge.value=="")) {
	...
	//alert("xajax_nsrv");
	xajax_nsrv(ecode.value, ename.value, ecash.value, echarge.value, group_id);
	//xajax_nsrv(ecode.value, ename.value, ecash.value, echarge.value, group_id, ecashC1.value, ecashC2.value, ecashC3.value);
}