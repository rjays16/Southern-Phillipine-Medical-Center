...
function populate_checklist(val, id) {
 ...
 //alert("ptype, ref, cash, id, dis, senior, walkin = "+ptype+"','"+ref_source+"','"+is_cash+"','"+discountid+"','"+discount+"','"+is_senior+"','"+is_walkin);
	xajax_populate_lab_checklist(val,area_type, id,ptype,ref_source,is_cash,discountid,discount,is_senior,is_walkin,source_req,isStat,is_charge2comp,compID);
...
function setAllDeptDoc(){
	...
	var isIPBM = "<?=$isIPBM?>"; // added by carriane 10/24/17
	...
	// updated by carriane 10/24/17; added isIPBM
				...
				<td class="segPanel">
					<table width="100%" style="font:bold 12px Arial; background-color:#e5e5e5;">
						...
						<tr>
							...
							<td align="left">
									<textarea name="clinical_info" id="clinical_info" cols=30 rows=2 wrap="physical" <?= ($encounter_type == '3' || $encounter_type == '4' ? 'readonly' : '')?>><?=$impression?></textarea>
							</td>
						</tr>
						<tr id="section">
										...
										<!--<option value="0">All Laboratory Service Section</option>-->
											<?php
                                                    ...
                                                    if($_GET['ref_source'] == 'LB'){
                                                        ...
                                                        $cond = "group_code NOT IN ('POC', 'B', 'SPL', 'IC', 'SPC','CATH','ECHO')";
                                                    }
                                                    else{
                                                        echo '<option value="0">All Laboratory Service Section</option>';
                                                        $cond = "group_code IN ('SPL','SPC','CATH','ECHO')";
                                                    }
