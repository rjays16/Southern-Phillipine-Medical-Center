...
if($saved||$row['test_date']||$std_date){
   ...
   $smarty->assign('sExamDate','<input name="test_date" id="date_text" type="text" size="14" value="'.formatDate2Local(date('Y-m-d'),$date_format).'" onBlur="IsValidDate(this,\''.$date_format.'\')")  onKeyUp="setDate(this,\''.$date_format.'\',\''.$lang.'\')">');
  // $smarty->assign('sMiniCalendar',"<a href=\"javascript:show_calendar('datain.test_date','$date_format')\"><img ".createComIcon($root_path,'show-calendar.gif','0','absmiddle')."></a>");
	 $smarty->assign('sMiniCalendar',"<img ".createComIcon($root_path,'show-calendar.gif','0','absmiddle')." id=\"date_trigger\" style=\"cursor:pointer\">".$calendarSetup);
}
