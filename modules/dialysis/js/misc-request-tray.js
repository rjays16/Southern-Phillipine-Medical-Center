...
function prepareAdd(id) {
	...
	details.discount = $('discount'+id).value;
	details.calculate= 1;
	details.doreplace = 1;
	details.limit= -1;
	details.src = 'other';
	details.ref = '0000000000'; 
	details.misc_type = $("acctype"+id).value;
	details.disable = 0;
...
function addServiceToList(listID, details) {
	...
	if (list) {
		...
		if (typeof(details)=='object') {
			...
			if(details.lock==1 && $('misc_type').value == 50)
				lock_indicator = '<img src="../../images/btn_nonsocialized.gif" />&nbsp;';
			else
				lock_indicator = '';
			...
			rowSrc = "<tr "+eclass+">"+
									'<td>'+
										lock_indicator+//added by Nick, 3/13/2014
										'<input type="hidden" id="id'+id+'" value="'+id+'">'+
										'<span style="font:bold 11px Arial;color:#660000">'+lpad(id,6,'0')+'</span>'+
									'</td>'+
									'<td>'+
										'<input type="hidden" id="acctype'+id+'" value="'+details.account_type+'">'+
