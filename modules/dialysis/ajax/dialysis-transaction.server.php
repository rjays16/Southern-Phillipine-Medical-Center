...
function getDoctors($nr="",$pid,$target) {
	global $db;
	$objResponse = new xajaxResponse();
	$dialysis_obj = new SegDialysis();

	$result = $dialysis_obj->getDialysisPersonell(1, "doctor");
	// $objResponse->alert($target);
	$getPrevDoc = $dialysis_obj->getPreviousDoctor($pid);
	if($target=='newrequest'){
		$options.="<option value='0'>-Select requesting doctor-</option>";
	}
	if($result){
		if($getPrevDoc){
			while($data1=$getPrevDoc->FetchRow())
			{
			$options.="<option value='".$data1["id"]."' selected='selected'>".$data1['doctor_name']."</option>";
			}
		}
...
function ajxsetIsPrinted($enc_number, $val)
{
	$objResponse = new xajaxResponse();
	$dialysis_obj = new SegDialysis;

	if(!$dialysis_obj->setIsReleased($enc_number,$val))
		$objResponse->alert("Error updating record");

	return $objResponse;
}
function ajxgetIsPrinted($enc_number)
{
	$objResponse = new xajaxResponse();
	$dialysis_obj = new SegDialysis;
	$isReleased = 0;
	$result = $dialysis_obj->getIsReleased($enc_number);
	if($result)
	{
	...
	if($enc_number=='0'){
		$objResponse->call('hideIsPrinted');
	}
	else{
		$objResponse->call('getIsPrinted',$isReleased);
	}
	return $objResponse;
...
// Added function by Matsuu 01072017
function getPreviousRequest($pid){
	global $db;
	$objResponse = new xajaxResponse();
	$dialysis_obj = new SegDialysis();
	$PrevTrans = $dialysis_obj->getPHICTransaction($pid);
	$phquantityd = $PrevTrans['ph_type'];
	$phamountd = $PrevTrans['ph_amount'];
	$nphquantityd = $PrevTrans['nph_type'];
	$nphamountd = $PrevTrans['nph_amount'];
	$hdfquantityd = $PrevTrans['hdf_type'];
	$hdfamountPh = $PrevTrans['ph_hdf'];
	$hdfAmountNph = $PrevTrans['nph_hdf'];
	...
	$objResponse->assign("PHquantity","value",$phquantityd);
	$objResponse->assign("PHamount","value",$phamountd);
	$objResponse->assign("NPHquantity","value",$nphquantityd);
	$objResponse->assign("NPHamount","value",$nphamountd);
	$objResponse->assign("HDFQty","value",$hdfquantityd);
	$objResponse->assign("HDFAmount","value",$hdfamountPh);
	$objResponse->assign("HDFAmountNPH","value",$hdfAmountNph);

	if($subsidyValue != '0.00') {
		if(isset($phamountd)) {
			$objResponse->assign("PH_subsidize_amount", "value", $subsidyValue);
			$objResponse->assign("PH_subsidize_class", "value", $subsidyClass);
		}

		if(isset($nphamountd)) {
			$objResponse->assign("NPH_subsidize_amount", "value", $subsidyNValue);
			$objResponse->assign("NPH_subsidize_class", "value", $subsidyNClass);
		}
	}

	return $objResponse;
}
// Ended function by Matsuu 01072017
function getPreviousDiag($pid,$encounter_nr){
	global $db;

	$objResponse = new xajaxResponse();
	$dialysis_obj  = new SegDialysis();
	$icdIcp = new Icd_Icp();
	$getIcd_icp = $dialysis_obj->getPreviousDialysisEnc($pid,$encounter_nr);
	$ergebnis = $icdIcp->searchIcd($getIcd_icp['encounter_nr']);
	// $objResponse->alert($icdIcp->sql);
	if ($ergebnis) {
			while($result=$ergebnis->FetchRow()) {
				$data = array(
								...
								'modify_id'=>$result['modify_id'],
								'modify_time'=>$result['modify_time'],
								'create_id'=>$result['create_id'],
								'create_time'=>$result['create_time']
							);
			$insres = $db->Replace('seg_encounter_diagnosis',$data,array('encounter_nr','code'),$autoquote = true);
...
function setVisitNo($pid)
{
	...
	$sql = "SELECT count(refno) FROM seg_dialysis_transaction_X WHERE pid=".$db->qstr($pid);
...
function showEncounterByPid($pid)
{
	global $db;
	$objResponse = new xajaxResponse();

	if($pid==""){
		$objResponse->assign("dp_encounter","innerHTML","<option value='0'>-No Encounter-</option>");
	}else{
		#Modified by Aleeya
		#updated by KENTOOT 09-01-2014
		$sql = "SELECT encounter_nr, is_discharged, encounter_date,consulting_dr_nr FROM care_encounter WHERE encounter_type='5' && pid=".$db->qstr($pid) . "ORDER BY encounter_nr DESC LIMIT 20;";
		$result = $db->Execute($sql);

		if($result!=FALSE){
			$options.="<option value='0'>-Select an Encounter-</option>";
			while($data=$result->FetchRow())
			{
				if(trim($data['consulting_dr_nr']) != '')
					$is_dr = 'data-is_dr=1';
				$encounter_date=date("m/d/Y", strtotime($data["encounter_date"]));
				$options.="<option $is_dr value='".$data["encounter_nr"]."'>".$data["encounter_nr"].' - '.$encounter_date. "<input type='text' id='discharge_id' name='discharge_id' value='".$data['is_discharged']."'></option>";
			}
			$objResponse->assign("dp_encounter","innerHTML",$options);
		}else{
			$objResponse->assign("dp_encounter","innerHTML", "<option value='0'>-No Encounter-</option>");
		}
	}
...
function populatePersonList($sElem,$searchkey,$page,$include_firstname,$include_encounter=TRUE,$include_walkin=FALSE,$exclude_mgh=FALSE,$from_dialysis="")
{
	...
	if ($ergebnis) {
		...
		while($result=$ergebnis->FetchRow()) {
			...
			$details->previousDiagnosis = SegDialysis::getPreviousDiagnosis($result["pid"]);//added by Nick 8-1-2015
		...
		// $objResponse->alert("misc\n".$misc_obj->sql);
		 if($res!==FALSE){
			 ...
			 while($row=$res->FetchRow())
			 {
				 ...
				 {
					 case 'cmap':
							$request_flag = '<img src="../../images/flag_cmap.gif" title="Item charged to MAP"/>';
							$req_flag=true;
							break;
					 case 'lingap':
...
function populateOtherRequests($encounter_nr,$pid)
{
	global $db;
	$objResponse = new xajaxResponse();
	$lab_obj = new SegLab();
		...
		while($ref = $result->FetchRow())
		{
			 ...
			 if($res!==FALSE){
					...
					while($row=$res->FetchRow())
					{
						...
						$objResponse->call("printRequestlist", "other_requests", "lab-list".$ref["refno"], $data);
					}
...
# Added Aleeya
function savePatientDetails($transaction_nr, $pid, $new_machine_nr, Array $dialyzer, $isNew , $isUpdate, $selectedEncounter, $isPhic)
{
    global $db;
    $objResponse = new xajaxResponse();
    ...
    if ($isNew == 'true') {
        $isNew = true;
        $type = (strtoupper($dialyzer['name']) == 'HIGH FLUX DIALYZER') ? 'high' : 'low';
        $objDialysis->addNewDialyzer($dialyzer['serialNo'], $dialyzer['serviceCode'], $type);
    }else{
        $isNew = false;
    }

    $splitDate = explode('/', $dialyzer['date']);
    $dialyzer['hours'] %= 12;
    $dialyzer['hours'] += $dialyzer['meridian'] === 'PM' ? 12 : 0;

    $tnDate = date('Y-m-d H:i:s', mktime($dialyzer['hours'], $dialyzer['mins'], 0, $splitDate[0], $splitDate[1], $splitDate[2]));
    if(!$isUpdate) {
        $isTransactionSuccess = $objDialysis->insertTransaction($transaction_nr, $pid, $new_machine_nr, $dialyzer['serialNo'], $tnDate);
        $transactionLog[] = ValidateTransaction($isTransactionSuccess,'Error saving transaction',$db,$objResponse);

        $isDialyzerUpdateSuccess = $objDialysis->updateDialyzerReuse($transaction_nr,$dialyzer['reusex']);
        $transactionLog[] = ValidateTransaction($isDialyzerUpdateSuccess,'Error updating dialyzer',$db,$objResponse);

        $isBillTypeUpdateSuccess = $objDialysis->updateBillType($transaction_nr, $isPhic);
        $transactionLog[] = ValidateTransaction($isBillTypeUpdateSuccess,'Error updating bill type',$db,$objResponse);

        if($isPhic == 'true'){
            $isUpdateBillingSuccess = $objDialysis->updateBilling($pid,$selectedEncounter,$tnDate,'save');
            $transactionLog[] = ValidateTransaction($isUpdateBillingSuccess,'Error updating billing',$db,$objResponse);
        }
    } else {
        $isUpdateHemoSuccess = $objDialysis->setHemoOpDate($selectedEncounter,$tnDate,$dialyzer['entry_no']);
        $transactionLog[] = ValidateTransaction($isUpdateHemoSuccess,'Error updating procedure date',$db,$objResponse);

        $isSavePatientDetailsSuccess = $objDialysis->savePatientDetails($transaction_nr, $new_machine_nr, $dialyzer['serialNo'], $tnDate);
        $transactionLog[] = ValidateTransaction($isSavePatientDetailsSuccess,'Error saving transaction',$db,$objResponse);

        $isDialyzerUpdateSuccess = $objDialysis->updateDialyzerReuse($transaction_nr, $dialyzer['reusex']);
        $transactionLog[] = ValidateTransaction($isDialyzerUpdateSuccess,'Error updating dialyzer',$db,$objResponse);

        if($isPhic == 'true'){
            $isUpdateBillingSuccess = $objDialysis->updateBilling($pid,$selectedEncounter,$tnDate,'update');
            $transactionLog[] = ValidateTransaction($isUpdateBillingSuccess,'Error updating billing',$db,$objResponse);
        }
    }

    if(in_array(false,$transactionLog)){
        $db->FailTrans();
    }else{
        $db->CompleteTrans();
    }

    $objResponse->call('closeWindow');

    return $objResponse;
}

function ValidateTransaction($isSuccess,$error,&$db,&$objResponse){
    if(!$isSuccess){
        $db->FailTrans();
...
function getAllMachines($machine_nr){
	global $db;
	$objResponse = new xajaxResponse();
	$objdialysis = new SegDialysis();
	$machine = $objdialysis->getAllMachineNumbers($machine_nr);

	if($machine){	
		while($data=$machine->FetchRow())
		{
			$machine_date = str_replace( " ", "-", $data['machine_nr'] );
			$options.= "<li onclick=getMachineNumber('".$machine_date."') >".$data['machine_nr'] ."</li>";
		}
			
			$objResponse->assign("searchList","innerHTML",$options);
	}
	
	return $objResponse;
}

function addNewDialyzer($encounter, $service_code, $request_date, $pid, $dialyzer_serial, $transaction_nr){
	global $db;
	$objResponse = new xajaxResponse();
	$objdialysis = new SegDialysis();

	$addDialyzer = $objdialysis->addNewDialyzer($encounter, $service_code, $request_date, $pid, $dialyzer_serial, $transaction_nr);
	if($addDialyzer!==FALSE)
	{
		$objResponse->call("closeWindow", $addDialyzer);
	}else {
		$objResponse->alert("Error:".$dialysis_obj->getErrorMsg()."\nSQL:".$dialysis_obj->sql);
	}
	return $objResponse;
...
function validateDialyzer($dialyzer_serial_nr){
	global $db;
	$objResponse = new xajaxResponse();
	$objdialysis = new SegDialysis();

	$validateDialyzerSerial = $objdialysis->validateDialyzer($dialyzer_serial_nr);

	if($validateDialyzerSerial == NULL){
		$objResponse->call("addDialyzerInfo");
	}else{
		$objResponse->call("invalidDialyzer", $validateDialyzerSerial);
	}
	return $objResponse;
}
#End jayson
...
#added by Keith 03/06/2014
#disable Dialysis Encounter
...
// 	$result = $cc->DisableDialysisEncounter($enc);
// 	if($result)
// 	{
// 		$objResponse->call("disabledButtons");
// 		$objResponse->call("refreshHistory");
// 	}
// 	else{
// 		$objResponse->alert("Error:".$dialysis_obj->getErrorMsg()."\nSQL:".$dialysis_obj->sql);
...
// 	$result = $cc->EnableDialysisEncounter($enc);
// 	if($result)
// 	{
// 		$objResponse->call("enabledButtons");
// 		$objResponse->call("refreshHistory");
...
//function getDischargeFlag($encounter)
//{
//	$objResponse = new xajaxResponse();
//	$dialysis_obj = new SegDialysis;
//
//	$dischargeFlag = $dialysis_obj->GetEncounterDischargeFlag($encounter);
//
//	if ($dischargeFlag) {
//		$isDischargedFlag=0;
//		while ($data = $dischargeFlag->FetchRow()) {
//			$isDischargedFlag = $data['is_discharged'];
//		}
//		if ($isDischargedFlag == '1')
//			$objResponse->call("disabledButtons");
//		else
//			$objResponse->call("enabledButtons");
//	} else
...
//TODO Refactor
function getDischargeFlag($encounterNr){
	$xajax = new xajaxResponse();
	if(SegDialysis::hasFinalBill($encounterNr)){
		$xajax->call("disabledButtons");
		$xajax->assign('viewBillButton','title','Has Final Bill');
	}else{
		$xajax->call("enabledButtons");
		...
		if(!$Billing_permission){
			$xajax->assign('viewBillButton','disabled','disabled');
			$xajax->assign('viewBillButton','title','No Permission');
		}else{
			$xajax->assign('viewBillButton','disabled','');
		}
...
function getUnpaidPrebill_ajx($enc,$current_nr){
	global $db;
	#$db->debug=true;
	$objResponse = new xajaxResponse();
	$dialysis_obj = new SegDialysis;
	$unpaid = $dialysis_obj->getUnpaidBills($enc);
	if ($unpaid) {
		$prebillno = '<select id="bill_nr" name="bill_nr" onchange="selectBillnr();"><option value="0"> -select- </option>';
	
		while($bill_nr = $unpaid->FetchRow()){
			$selected = $current_nr == $bill_nr['bill_nr']? 'selected' : '';
			$prebillno .= '<option '.$selected.' value="'.$bill_nr['bill_nr'].'">'.$bill_nr['bill_nr'].'</option>';
		}
		$prebillno .= '</select>';
	}else{
		$prebillno = '<select><option > -no more bills!- </option></select>';	
	}
	$objResponse->assign("prebillno","innerHTML",$prebillno);
	return $objResponse;

}

function getPrebillPayments_ajx($bill_nr){
	global $db;
	$objResponse = new xajaxResponse();
	$dialysis_obj = new SegDialysis;
	$amount_req = $dialysis_obj->getBillNrDetails($bill_nr);
	$total_payed = $dialysis_obj->getTotalAmountPayed($bill_nr);

	$amountWithHdf = $amount_req['amount']+$amount_req['hdf_amount'];
	$amountWithHdf = number_format($amountWithHdf, 2, '.', ',');

	
	$objResponse->assign("total","innerHTML",$bill_nr ? $amountWithHdf : '');
	if ($bill_nr != 0) {
		$str = '<table id="" class="table-hover table" cellspacing="0" width="100%">
			        <thead>
			            <tr> 
			                <th><i class="icon-search icon-plus" onclick="appendRow();"></i></th>
			                <th>Account Type</th>
			                <th>Amount</th>
			                <th>Control No.</th>
			                <th>Remarks</th>
			            </tr>
			        </thead><tbody id="paytbody">';

		$payments = $dialysis_obj->getPrebillPayments($bill_nr);
		if ($payments) {
			while ($row = $payments->FetchRow()) {
				$pay_type_list = $dialysis_obj->getPayType();
				$sel ='';
				$pay_type = '<select name="pay_type[]" id="pay_type" class="input-small"><option value=""> -select- </option>';
								while ($list = $pay_type_list->FetchRow()){
									$sel = ($row['pay_type'] == $list['pay_id'] ? 'selected':'');
									$pay_type .= '<option '.$sel.' value="'.$list['pay_id'].'">'.$list['name'].'</option>';
								}
				$pay_type .= '</select>';
				$str .= '<tr class="'.$row['ref_no'].'">
				 			<td>
					 			<i class="icon-minus" onclick="removeRow('.$row['ref_no'].');"></i>
					 			<input type="hidden" name="ref_no[]" id="ref_no" value="'.$row['ref_no'].'">
					 			<input type="hidden" name="delete[]" id="delete'.$row['ref_no'].'" value="">
				 			</td>
				 			<td>'.$pay_type.'</td>
				 			<td><input type="number" class="input-small" name="amount[]" value="'.$row['amount'].'"></td>
				 			<td><input type="text" class="input-small" name="control_nr[]" value="'.$row['control_nr'].'"></td>
				 			<td><input type="text" name="description[]" value="'.$row['description'].'"></td>
			 			</tr>';
			}
			
			if ($total_payed == $amountWithHdf || $total_payed > $amountWithHdf) {$disabled = '';
			}else{$disabled = 'disabled';}
			$bill = str_replace('-', '',$bill_nr);

			$str .='</tbody><tfoot class="paytfoot">
						<tr>
							<th width="100px">
							<td>Total amount:</td>
							<td>'.$total_payed.'</td>
							<td>
								<button class="btn btn-success '.$disabled.'" type="button" '.$disabled.' id="" onclick="applyPay_js('.$bill.');">
									<i class="icon-search icon-ok-sign"></i>  Apply
								</button>
							</td>
							<td><div class="btn-group">
								  <button class="btn btn-info" type="submit" id="savebtn"><i class="icon-search icon-file"></i>  Save</button>
								  <button class="btn btn-danger" type="button" onclick ="cancel();"><i class="icon-search icon-remove"></i>  Cancel</button>
								</div></td>
						</tr>
					</tfoot></table>';
			$objResponse->assign("data","innerHTML",$str);
		}else{
			$str .= '<tr class="error">
			 			<td></td>
			 		...
			 		<tfoot class="paytfoot">
						<tr>
							<th width="100px">
							<td>Total amount:</td>
							<td></td>
							<td>
								<button class="btn btn-success disabled" type="button" disabled id="">
									<i class="icon-search icon-ok-sign"></i>  Apply
								</button>
							</td>
							<td><div class="btn-group">
		...
		$str = '<h3>Please Select a Bill!</h3>';
		$objResponse->assign("data","innerHTML",'');
		$objResponse->assign("msg","innerHTML",$str);
	}
	return $objResponse;
}


function appendTbl_ajx($rowCount){
	global $db;
	$dialysis_obj = new SegDialysis;
	$objResponse = new xajaxResponse();
	$pay_type_list = $dialysis_obj->getPayType();
	...
	while ($list = $pay_type_list->FetchRow()){
		...
		$value_MAP = $list['name']== "CMAP" ? "MAP" :$list["name"];
		$pay_type .= '<option value="'.$list['pay_id'].'">'.$value_MAP.'</option>';
	}
	$pay_type .= '</select>';

	$str = '<tr class="'.$rowCount.'">                        
				<td>
					<i class="icon-minus" onclick="removeRow('.$rowCount.');"></i>
					<input type="hidden" name="ref_no[]" id="ref_no" value="">
					<input type="hidden" name="delete[]" id="ref_no" value="">
				</td>
				<td>
					'.$pay_type.'
				</td>
				<td><input type="number" class="input-small" name="amount[]" value=""></td>
				<td><input type="text" class="input-small" name="control_nr[]" value=""></td>
				<td><input type="text" name="description[]" value=""></td>
			</tr>';

	#$objResponse->append("paytbody","innerHTML",$str);
	$objResponse->call("appendtbl",$str);
	return $objResponse;
}

function applyPay_ajx($bill_nr){
	global $db;
	$dialysis_obj = new SegDialysis;
	$objResponse = new xajaxResponse();

	$ok = $dialysis_obj->applyPay($bill_nr);
	if ($ok) {
		$objResponse->alert('Payment is successfully applied!');
		$objResponse->call('refreshPage');
		$objResponse->call('preset');
	}else{
		$objResponse->alert('Failed to apply Payment!');
	}

	return $objResponse;
}

function appendTbl_add_new_trxn_ajx($enc_nr){
	global $db;
	$dialysis_obj = new SegDialysis;
	$objResponse = new xajaxResponse();
	$getUnpaidBills = $dialysis_obj->getUnpaidBills($enc_nr);
	if ($getUnpaidBills) {
		$str = '<tr class="lg-alt" style="height: 24px;">
					<td class="lg-td" style="width: 100%;">
						<button id="addbtn" title="Add Transaction" class="segButton" onclick="addTrxn(\'' . $enc_nr . '\');return false;">
							<img src="../../gui/img/common/default/add.gif"/>Add
						</button>
					</td>
					<td colspan="7"></td>
				</tr>';
	...
	$objResponse->call('appendTbl_add_new_trxn',$str);
	return $objResponse;
}
