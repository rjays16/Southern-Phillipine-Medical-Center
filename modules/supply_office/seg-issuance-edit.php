...
if (isset($_POST["submitted"])) {
    ...
    if ($_POST['old_refno'] == '') {        
        ...
        $savetry = $issobj->delIssServed($_POST['old_refno']);        
        if ($saveok) {
        ...
        foreach($_POST["items"] as $i) {
            ...
            if ($_POST['old_refno'] != '') {        
                ...
                if($_REQUEST['expdate'][$counter]!="-"){
                    $inventory_obj->addInventory($_REQUEST['pending'][$counter], $_REQUEST['unitid'][$counter], $_REQUEST['expdate'][$counter], NULL,$_REQUEST['issue_date']); 
                }  
                else if($_REQUEST['serial'][$counter]!="-"){
                    $inventory_obj->addInventory($_REQUEST['pending'][$counter], $_REQUEST['unitid'][$counter], NULL, $_REQUEST['serial'][$counter],$_REQUEST['issue_date']);     
                }  
                else {
                    $inventory_obj->addInventory($_REQUEST['pending'][$counter], $_REQUEST['unitid'][$counter], NULL, NULL,$_REQUEST['issue_date']);
                }
            }
...
if (isset($_GET["refno"])) {    
    ...
    if ($result = $issue_obj->getIssuanceHeader($_GET["refno"])) {                               
        ...
        $_POST['authorizing_id'] = $result['authorizing_id'];
        ...
        if ($result = $issue_obj->getIssuanceDetails($_GET["refno"])) {                    
            ...
            if ($result->RecordCount()) {
                ...
                while ($row = $result->FetchRow()) {  
                    ...
                    $unitnames1[] = $unit_obj->getUnitName($row["unit_id"]);
                    $descs1[] = "1";
                    $avg1[] = $row["avg_cost"]; 
                } 
