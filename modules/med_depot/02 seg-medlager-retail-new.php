...
$title=$LDMedDepot;
$breakfile=$root_path."modules/med_depot/seg-medlager-retail-functions.php".URL_APPEND."&userck=$userck";
$imgpath=$root_path."med/img/";
$thisfile='seg-medlager-retail-new.php';
...
if (eregi('details',$mode)) {
	...
	if ($mode=='save') include($root_path.'include/inc_meddepot_retail_db_save_mod.php'); # burn modified: August 8, 2006
}
//if ($saveok) include($root_path.'include/inc_retail_display_rdetails.php');   # burn commented c/o alvin: August 9, 2006
...
ob_start();
	...
	echo '<script type="text/javascript" src="'.$root_path.'modules/med_depot/js/retail-new-gui-functions.js"></script>'."\r\n";
...
<script type='text/javascript'>
   ...
   # burn added: line 106 var refno='$refno'; August 4, 200
...
if ($saveok){ # burn: codes below, after the user has typed-in the input in VIEW mode
	...
	$smarty->assign('sPurchaseDateInput',$purchasedt.'<input type="hidden" name="purchasedt" id="purchase_date" size="20" maxlength="20"  value="'.$purchasedt.'">');
	$smarty->assign('sPayerNameInput',$pname . '<input type="hidden" id="payer_text" name="pname" size="20" readonly="1" value="'.$pname.'">');   # burn modified: August 9, 2006
...
}else{ # burn: codes below is for the user to type-in the input
	...
	$smarty->assign('sPayerNameInput','<input type="text" id="payer_text" name="pname" size="20" readonly="1" value="'.$pname.'">');   # burn modified: August 9, 2006
	$smarty->assign('sPayerID','<input type="hidden" id="payer_id" name="pencnum"  value="'.$pencnum.'">');
	$smarty->assign('sPayerSelectButton','<input type="button" name="payerselect" value="Select" onclick="window.open(\'' . $root_path."modules/med_depot/seg-medlager-patient-select.php".URL_APPEND."&clear_ck_sid=$clear_ck_sid" . '\',\'patient_select\',\'width=890,height=640,menubar=no,resizable=yes,scrollbars=yes\')">');
	if (!isset($is_cash))
...
if ($saveok){
	...
	$smarty->assign('sTransactionDetailsControls', file_get_contents("seg-medlager-retail-new-rdetails.inc.php"));
    $smarty->assign('sDiscountControls', file_get_contents("seg-medlager-retail-new-discount.inc.php"));
	...
	<table style=\"border:1px solid black;margin:5px 0px;\" width=\"100%\">
		...
		<tr class=\"wardlistrow1\">
			<td nowrap style=\"	\">
				...
				<input type=\"button\" id=\"select_product\" value=\"Select product\" onclick=\"window.open('".$root_path."modules/med_depot/seg-retail-product-search.php".URL_APPEND."&clear_ck_sid=$clear_ck_sid" . "','patient_select','width=890,height=640,menubar=no,resizable=yes,scrollbars=yes')\">
			</td>
			<td nowrap>
...
");
	...
	*/
	...
	if (!$saveok) 
		...
		$smarty->assign('sTailScripts', '<script language="javascript">xajax_populateDetails("'.$refno.'")</script>');
