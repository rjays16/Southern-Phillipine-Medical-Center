<?php
    ...
    $ergebnis=$db->Execute($sql);
		...
		while($result=$ergebnis->FetchRow()) {
//			$objResponse->addScriptCall("retail_addTransaction",$result["refno"], $result["encounter_nr"], $result["fullname"], date("M j, Y",strtotime($result["purchasedte"])),$result["is_cash"]);   # burn comment: Agust 10, 2006
			$objResponse->addScriptCall("retail_addTransaction",$result["refno"], $result["encounter_nr"], $result["fullname"], date($phpfd,strtotime($result["purchasedte"])),$result["is_cash"]);   # burn added: August 10, 2006
		}                               
    ...
    $ergebnis=$db->Execute($sql);
		...
		while($result=$ergebnis->FetchRow()) {
//			$objResponse->addScriptCall("retail_addRefTransaction",$result["refno"], $result["encounter_nr"], $result["fullname"], date("M j, Y",strtotime($result["purchasedte"])),$result["is_cash"]);   # burn comment: Agust 10, 2006
			$objResponse->addScriptCall("retail_addRefTransaction",$result["refno"], $result["encounter_nr"], $result["fullname"], date($phpfd,strtotime($result["purchasedte"])),$result["is_cash"]);   # burn added: August 10, 2006
		}
