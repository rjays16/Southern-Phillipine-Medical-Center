...
function populateProducts($page_num=0, $max_rows=10, $sort_obj=NULL, $args=NULL) {
	...
	$sortColumns = array('prod_class','p.bestellnum','p.barcode','artikelname','p.price_cash','sc_price','c1_price','c2_price','c3_price');
	$sort = array();
	...
	$result=$pclass->searchProducts($codename, $generic, $classification, $prodclass, $offset, $max_rows, $sort_sql,$barcode);
	# $objResponse->alert($pclose(handle)ass->sql);
	if($result) {
		...
		if($data_size=$result->RecordCount()) {
			...
			while($row = $result->FetchRow()) {
				...
				$DATA[$i]['artikelname'] = htmlentities($row['artikelname']);
				$DATA[$i]['generic'] = htmlentities($row['generic']);
