 ...
 */
 ...
 $smarty->assign('sToolbarTitle',"Pharmacy:Create Refund");
...
// $smarty->assign('pbBack',$returnfile);
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"Pharmacy:Create Refund");
	...
	if ($result) {
		...
		while ($row=$result->FetchRow()) {
			...
			$rows .= "		<tr class=\"$class\">
				...
				<td class=\"centerAlign\"><input class=\"jedButton\" type=\"button\" style=\"color:#000060\" value=\"View\" onclick=\"showBill('".$row["bill_nr"]."')\" /></td>
			</tr>\n";
			$count++;
		}
		if (!$rows)
...
if (!$rows) {
	$rows = '		<tr><td colspan="6">Select patient record and click Search to begin query...</td></tr>';
}
...
<div style="width:50%">
	...
	<div style="white-space:nowrap	">
		...
		<input class="jedInput" name="patientname" id="patientname" readonly="readonly" style="width:150px" type="text" value="<?= $_REQUEST['patientname'] ?>" />
		...
		<img id="select-enc" src="../../images/btn_encounter_small.gif" border="0" align="absmiddle" style="cursor:pointer;"
  	    ...
  	    onmouseout="nd();" />
		<input id="search" type="submit" class="jedButton" value="Search" <?= $_POST['patient'] ? '' : 'disabled="disabled"' ?>/>
	</div>
	<hr width="100%" size="1" />
		...
		<table class="jedList" width="100%" border="0" cellpadding="0" cellspacing="0">
			<thead>
				...
				<tr>
					...
					<th width="20%" nowrap="nowrap">Bill Date</th>
					<th width="20%" nowrap="nowrap">Bill From</th>
