...
function showDetails(id, show) {
	...
	dt = $('d'+id);
	if (dt) {
		var mode = $('mode-'+id).value
		if (!dt.src || show) setTimeout("$('d"+id+"').src='seg-pharma-return-tray-refview.php?ref="+id+"&nr="+returnNr+"&mode="+mode+"'",10);
		/*
		if (show) {
		...
		*/
		...
		//if ($('showtd-'+id)) $('showtd-'+id).className = show ? "InnerFill" : "";
	}
	if ($('details-'+id)) { 
		...
		if (details.id) {
			...
			rowSrc = '<tr id="row-'+details.id+'">'+
									...
									'<td align="right">'+
										status+
									'</td>'+
									'<td id="showtd-'+details.id+'" align="center" style="">'+
										'<img id="expand-'+details.id+'" title="Expand" src="../../images/cashier_expand.gif" class="segSimulatedLink" onclick="showDetails(\''+details.id+'\',true)">'+
										'<img id="collapse-'+details.id+'" title="Collapse" src="../../images/cashier_collapse.gif" class="segSimulatedLink" style="display:none" onclick="showDetails(\''+details.id+'\',false)">'+
									'</td>'+
								'</tr>'+
								'<tr id="details-'+details.id+'" style="display:none;">'+
									'<td id="detailstd-'+details.id+'" style="padding:4px;" class="plain" colspan="8" align="center">'+
										'<div>'+
											'<iframe id="d'+details.id+'" frameborder="0" scrolling="yes" style="height:0;overflow-x:hidden;overflow-y:hidden;border:2px solid #003366;width:99%;background-color:white;display:block">\n'+'</iframe>'+
										'</div>'+
									'</td>'+
								'</tr>';
