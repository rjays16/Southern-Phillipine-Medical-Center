...
function showDetails(id, show) {
	...
	dt = $('d'+id);
	if (dt) {
		if (!dt.src) setTimeout("$('d"+id+"').src='seg-pharma-return-tray-orview.php?or="+id+"&nr="+returnNr+"'",300);
		if (show) {
			//$('details-'+id).style.display = '';
		...
		if (details.id) {
			...
			rowSrc = '<tr id="row-'+details.id+'">'+
									...
									'<td class="centerAlign">'+
										'<img id="expand-'+details.id+'" title="Expand" src="../../images/cashier_down_small.gif" class="segSimulatedLink" onclick="showDetails(\''+details.id+'\',true)">'+
										'<img id="collapse-'+details.id+'" title="Collapse" src="../../images/cashier_up_small.gif" class="segSimulatedLink" style="display:none" onclick="showDetails(\''+details.id+'\',false)">'+
									'</td>'+
								'</tr>'+
								'<tr id="details-'+details.id+'" style="display:none;">'+
									'<td id="detailstd-'+details.id+'" style="padding:8px;" class="plain" colspan="8" align="center">'+
										'<div>'+
											'<iframe id="d'+details.id+'" frameborder="0" scrolling="yes" style="height:160px;overflow-x:hidden;overflow-y:scroll;border:3px solid #003366;width:98%;background-color:white;display:block">\n'+'</iframe>'+
										'</div>'+
									'</td>'+
								'</tr>';
