<?php
	...
	class ExcelGen_Causes_Morbidity extends Spreadsheet_Excel_Writer
	{
			var $worksheet;
			var $Headers;
			...
			function ExcelGen_Causes_Morbidity($from, $to, $dept_nr, $sclass)
			{
			...
			function ExcelHeader()
			{
					...
					$this->worksheet->setHeader("".$row['hosp_country']."\n".$row['hosp_agency']."\n".$row['hosp_name']."\n".$row['hosp_addr1']."\n\nTWENTY LEADING CAUSES OF MORBIDITY (OVER-ALL)".$label_dept."\n".$text."\n",0.5);
			...
			function FetchData()
			{
					...
					/*$tot_sql = "SELECT SUM(t.total) as total FROM (SELECT count(ed.code_parent) AS total
										...
										ORDER BY count(ed.code_parent) DESC LIMIT 20) as t";  */
					...
					$sql = "SELECT (SELECT IF(INSTR(p.code_parent,'.'), SUBSTRING(p.code_parent, 1, 3), IF(INSTR(p.code_parent,'/'), SUBSTRING(p.code_parent, 1, 5),p.code_parent))) AS code_parent,
						...
						$result=$db->Execute($sql);
							...
							$col=0;
							 ...
							 $this->worksheet->write($newrow, $col+22, $row['code_parent'], $this->format3);
	...
	$rep = new ExcelGen_Causes_Morbidity($_GET['from'], $_GET['to'], $_GET['dept_nr_sub'], $_GET['sclass']);
	$rep->ExcelHeader();
	$rep->FetchData();
	$rep->AfterData();
	$rep->send('rep_causes_morbidity_parent.xls');
	$rep->close();
?>