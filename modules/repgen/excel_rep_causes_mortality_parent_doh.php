<?php
  ...
  class Excel_Causes_MortalityParent extends Spreadsheet_Excel_Writer
  {
      var $worksheet;
      var $Headers;
      ...
      function Excel_Causes_MortalityParent($fromdate, $todate, $dept_nr)
      {
...
ORDER BY SUM(CASE WHEN (sr.result_code = 8) then 1 else 0 end) DESC LIMIT 20";
  ...
  $rep = new Excel_Causes_MortalityParent($_GET['from'], $_GET['to'], $_GET['dept_nr_sub']);
  $rep->ExcelHeader();
  $rep->FetchData();
  $rep->AfterData();
  $rep->send('excel_rep_causes_mortality_parent_doh.xls');
  $rep->close();
?>