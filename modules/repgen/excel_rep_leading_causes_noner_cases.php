<?php
	...
	class ExcelGen_Causes_NONERCases extends Spreadsheet_Excel_Writer
	{
			var $worksheet;
			var $Headers;
			...
			function ExcelGen_Causes_NONERCases($from, $to, $sclass)
			{
			...
			function ExcelHeader()
			{
					...
					$this->worksheet->setHeader("".$row['hosp_country']."\n".$row['hosp_agency']."\n".$row['hosp_name']."\n".$row['hosp_addr1']."\n\nLEADING CAUSES OF NON-EMERGENCY CASES IN THE ER DEPARTMENT".$label."\n".$text."\n",0.5);
	...
	$rep = new ExcelGen_Causes_NONERCases($_GET['from'], $_GET['to'], $_GET['sclass']);
	$rep->ExcelHeader();
	$rep->FetchData();
	$rep->AfterData();
	$rep->send('rep_leading_causes_noner_cases.xls');
	$rep->close();
?>