<?php
	...
	class ExcelGen_OPD_Consultation extends Spreadsheet_Excel_Writer
	{
			var $worksheet;
			var $Headers;
			...
			function ExcelGen_OPD_Consultation($from, $to, $sclass)
			{
			...
			function ExcelHeader()
			{
					...
					$this->worksheet->setHeader("".$row['hosp_country']."\n".$row['hosp_agency']."\n".$row['hosp_name']."\n".$row['hosp_addr1']."\n\nLEADING CAUSES OF CONSULTATIONS AT OPD".$label."\n".$text."\n",0.5);
	...
	$rep = new ExcelGen_OPD_Consultation($_GET['from'], $_GET['to'], $_GET['sclass']);
	$rep->ExcelHeader();
	$rep->FetchData();
	$rep->AfterData();
	$rep->send('rep_opd_leading_consultation.xls');
	$rep->close();
?>