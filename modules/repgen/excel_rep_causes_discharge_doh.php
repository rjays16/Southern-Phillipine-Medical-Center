<?php
	#created by CHE 12-04-10
	#TWENTY LEADING CAUSES OF MORTALITY - for DMC
	...
	class ExcelGen_Causes_Discharge extends Spreadsheet_Excel_Writer
	{
			var $worksheet;
			var $Headers;
			...
			function ExcelGen_Causes_Discharge($from, $to, $dept_nr, $distribution)
			{
			...
			function ExcelHeader()
			{
					...
					$this->worksheet->setHeader("".$row['hosp_country']."\n".$row['hosp_agency']."\n".$row['hosp_name']."\n".$row['hosp_addr1']."\n\TWENTY LEADING CAUSES OF MORTALITY (".$label_dept.")"."\n".$text."\n",0.5);
	...
	$rep = new ExcelGen_Causes_Discharge($_GET['from'], $_GET['to'], $_GET['dept_nr_sub'], $_GET['age_distribution']);
	$rep->ExcelHeader();
	$rep->FetchData();
	$rep->AfterData();
	$rep->send('rep_causes_discharge_doh.xls');
	$rep->close();
?>