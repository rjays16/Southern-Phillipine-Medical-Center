...
#commented and edited by VAN 03-07-08
#$dept_DOC=$dept_obj->getAllActiveWithStaff();
$dept_DOC=$dept_obj->getAllActive();
#print_r($dept_DOC);

#echo "target : $target";
switch($target){
	
	case 'plist': $title=$LDCreateStaffList;
					  $fileforward='staff-dienst-personalliste.php'.URL_APPEND.'&retpath='.$retpath;
	...
	default: $title=$LDMakeDutyPlan;
					  $fileforward='staff-dienstplan-planen.php'.URL_APPEND.'&retpath='.$retpath;
					  break;
}
...
while(list($x,$v)=each($dept_DOC)){
	...
	if(isset($$v['LD_var'])&&!empty($$v['LD_var'])) echo $$v['LD_var'];
			 ...
			 if (($v['admit_inpatient'] == 0) && ($v['type'] == 2))
			 		echo $v['name_formal']." - Non-Medical";
			 elseif (($v['admit_inpatient'] == 0) && ($v['type'] == 3))
			 	   echo $v['name_formal']." - News";
