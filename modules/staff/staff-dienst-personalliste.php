...
#echo "dept = ".$dept_nr;
#$staff=&$pers_obj->getStaffOfDept($dept_nr);
$staff=&$pers_obj->getStaffOfDept($dept_nr);
#------------------------------------------
...
if(!isset($dept_nr)||!$dept_nr){
	if($cfg['thispc_dept_nr']){
		...
		header('Location:staff-select-dept.php'.URL_REDIRECT_APPEND.'&target=plist&retpath='.$retpath);
		exit;
	}
}
...
#$dept_list=&$dept_obj->getAllNonMedical();
$dept_list=&$dept_obj->getAllActive();
...
if(!empty($ipath)){
	switch($ipath){
		...
		case 'qview': $breakfile="staff-dienst-schnellsicht.php".URL_APPEND."&hilitedept=$dept_nr"; break;
		case 'plan': $breakfile="staff-dienstplan-planen.php".URL_APPEND."&dept_nr=$dept_nr&pmonth=$pmonth&pyear=$pyear&retpath=$retpath"; break;
...
#echo "van: $LDStaffList";
# Prepare page title
 $LDStaffList='Staffs\' List';
 $sTitle = "$LDStaffList :: ";
...
function popinfo(l,d){
	urlholder="staff-dienstplan-popinfo.php<?php echo URL_REDIRECT_APPEND ?>&nr="+l+"&dept_nr="+d+"&user=<?php echo $aufnahme_user.'"' ?>;
...
function deleteItem(nr){
	if(confirm('<?php echo $LDSureToDeleteEntry; ?>')){
		window.location.replace("staff-list-add.php<?php echo URL_REDIRECT_APPEND; ?>&item_nr="+nr+"&dept_nr=<?php echo $dept_nr.'&mode=delete&retpath='.$retpath.'&ipath='.$ipath; ?>");
	}
}
-->
</script>
...
<?php
if(is_object($staff)&&$staff->RecordCount()){
...
<table border=0  bgcolor="#6f6f6f" cellspacing=0 cellpadding=0>
	...
	<td  align=center  class="v13" colspan=2><nobr>&nbsp;Position</nobr></td>
	...
	<td  class="v13" colspan=2><nobr>&nbsp;<?php echo $row['job_position']; ?></nobr></td>
...
<hr>
<?php
	$LDAddStaffToList = "Add a staff to list";
	$LDPlsSearchNurse = "Please search a staff";
?>
<font class="prompt"><b><?php echo "$LDAddStaffToList. $LDPlsSearchNurse"; ?></b></font>
...
<?php
					...
					while($row=$search_result->FetchRow())
					{
						...
						<td>&nbsp;
							<a href="staff-list-add.php'.URL_APPEND.'&nr='.$row['nr'].'&dept_nr='.$dept_nr.'&job_fxn='.$row['job_function_title'].'&mode='.$mode_status.'&retpath='.$retpath.'&ipath='.$ipath.'" title="'.$LDAddStaffToList.'">
...
while(list($x,$v)=each($dept_list))
	{
		...
		if(isset($$v['LD_var'])&&$$v['LD_var']) echo $$v['LD_var'];
						...
						if (($v['admit_inpatient'] == 0) && ($v['type'] == 2))
							echo $v['name_formal']." - Non-Medical(OPD)";
						elseif (($v['admit_inpatient'] == 0) && ($v['type'] == 3))
							echo $v['name_formal']." - News (OPD)";
						else
							echo $v['name_formal']." - Medical";
					}
