...
if(!isset($dept_nr)||!$dept_nr){
	header('Location:staff-select-dept.php'.URL_REDIRECT_APPEND.'&retpath='.$retpath);
	exit;
}

$thisfile=basename(__FILE__);
$breakfile="staff-dienstplan.php".URL_APPEND."&dept_nr=$dept_nr&pmonth=$pmonth&pyear=$pyear&retpath=$retpath";
...
function makefwdpath($path,$dpt,$mo,$yr,$saved)
{
	...
	else return "staff-dienstplan-checkpoint.php";
}
...
function popselect(elem,mode)
{
	...
	urlholder="staff-dienstplan-poppersonselect.php?elemid="+elem + "&dept_nr=<?php echo $dept_nr ?>&month="+tmonth+"&year="+tyear+ "&mode=" + mode + "&retpath=<?php echo $retpath ?>&user=<?php echo $ck_op_dienstplan_user."&lang=$lang&sid=$sid"; ?>";
...
function cal_update()
{
	var filename="staff-dienstplan-planen.php?<?php echo "sid=$sid&lang=$lang" ?>&retpath=<?php echo $retpath ?>&dept_nr=<?php echo $dept_nr; ?>&pmonth="+document.dienstplan.month.value+"&pyear="+document.dienstplan.jahr.value;
