...
function SaveAppliedDiscount($aFormValues, $bill_dt = "0000-00-00 00:00:00") {
	...
	if(array_key_exists("enc_nr", $aFormValues)) {		
		...
		if (!$bolError) {
			...
			if($objBilling->hasDoctorCoverage('T'.$aFormValues['enc_nr'])){
				if($objBilling->clearDoctorCoverage('T'.$aFormValues['enc_nr'])&&$objBilling->clearDoctorCoverageBreakdown('T'.$aFormValues['enc_nr'])){
					$objResponse->addScriptCall("pfResetWarning");
				}
			}
