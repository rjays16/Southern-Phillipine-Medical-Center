...
<script language="javascript" type="text/javascript">
	...
	*/
	...
	function exceededSixty2(days_allowed){
		alert("Filed beyond "+days_allowed+" days is not allowed");
		$j('.exceededSixty').attr('checked', false);
	}
...
//						if (!empty($row["total_computed_discount"])) $n_bill -= $row["total_computed_discount"];
						...
						*/
						...
						if (!(isset($_SESSION['cases'][$row["encounter_nr"]]) && ($_SESSION['cases'][$row["encounter_nr"]]))) {
								...
								$bill_dte = date_create($row['bill_dte']);
								$diff = date_diff($bill_dte,$date_now);
								$diff1 = $diff->format("%a");
								...
								if($diff1 > $days_allowed){
									$btns = '<td align="center"><input type="checkbox" class="exceededSixty" name="exceededSixty" style="cursor:pointer"'.
												' onclick="exceededSixty2('.$days_allowed.')" /></td>';
								}else{
									$btns = '<td align="center"><input type="checkbox" id="cases[]" name="cases[]" style="cursor:pointer" value="'.$row["encounter_nr"].'" '.
													' onclick="prepareSelect(\''.$row["encounter_nr"].'\', \''.$diff1.'\')"/></td>';
								}
										...
										<table width="100%" border="0" cellpadding="2" cellspacing="0">
												<tr>
														<td width="50" align="right">
																<input type="checkbox" id="chkspecific" name="chkspecific" onclick="selrecordOnChange();" <?= ($_REQUEST['chkspecific'] ? 'checked' : '') ?>/>
														</td>
														<!-- <td width="5%" align="right" nowrap="nowrap">Patient/Case No./Bill No./HRN</td> -->
															<td width="5%" nowrap="nowrap">Case No.</td>
														<td>
<script language="javascript" type="text/javascript">
<!--
		function selrecordOnChange() {
				var optSelected = $('chkspecific').checked;
				if(optSelected){
					$('case_no').style.display = "";
					// $('case_no').value = "";
				}else{
					
					$('case_no').style.display = "none";
				}
...
-->
																...
																<select class="jedInput" name="selrecord" id="selrecord" onchange="selrecordOnChange(); keepFilters(0);emptier();"<?= ($_REQUEST["selrecord"]=="case_no") && $_REQUEST['chkspecific'] ? '' : 'style="display:none;"' ?>style="visibility:hidden"/>
																...
																<span name="selrecordoptions" segOption="case_no">
																		<input class="jedInput" name="case_no" id="case_no" onblur="keepFilters(0);" onkeyup="disableSearch();" type="text" size="30" value="<?= $_POST['case_no'] ?>"style="margin-left:-110px;"/>
																</span>
