...
class ReportMonthlyBillsRendered extends RepGen {
	 ...
	 * 
	 */
	public function __construct($month, $year, $format='CSV') {
		global $db;
		...
		$this->startDate = strtotime($year.'-'.$month.'-01');
		if ($this->startDate === false) {
			die('Invalid month/year specified');
		}
		$this->endDate = strtotime('+1 month', $this->startDate);
		$this->format = $format;
...
WHERE
(fb.bill_dte BETWEEN " . $db->qstr(date('YmdHis', $this->startDate)) . " AND " . $db->qstr(date('YmdHis', $this->endDate)) . ")
...
LIMIT 0,999999";
	...
	public function ReportXls() {
		...
		$sheet->setCellValue('A1', 
			sprintf('REPORT OF BILLS RENDERED (%s)', date('M Y', $this->startDate)));
...
$rep = new ReportMonthlyBillsRendered($_GET['month'], $_GET['year'], $_GET['format'], $_GET['formatD']);
//added by pol
if ($formatD == 'SA'){   
$rep->FetchData();
} elseif ($formatD == 'DB'){
