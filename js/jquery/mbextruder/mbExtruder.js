...
(function($) {
	...
	$.mbExtruder= {
		...
		buildMbExtruder: function(options){
			return this.each (function (){
				...
				extruderStyle=
								...
								{position:position,top:0,right:0,marginRight:5,width:this.options.width}:
				...
				if ($.metadata){
					...
					if (extruder.metadata().get) extruder.attr("extGet",extruder.metadata().get);
				}
		...
		populateMbExtruderContent: function(html){
			var where=$(this), voice;
			var container=$("<div>").addClass("container");
			if (!($.browser.msie && $.browser.version<=7))
				container.css({width:$(this).get(0).options.width});
			where.find(".content").empty().wrapInner(container);
			where.find(".container").append(html);
			voice=where.find(".voice");
			voice.hover(function(){$(this).addClass("hover");},function(){$(this).removeClass("hover");});
			where.setExtruderVoicesAction();
		...
		openMbExtruder:function(c){
			...
			{
				extruder.populateMbExtruderContent( opt.populate() );
			}
	...
	$.fn.populateMbExtruderContent=$.mbExtruder.populateMbExtruderContent;
