...
$picpath=$root_path.$news_fotos_path.$news['pic_file'].'.'.$news['pic_mime'];
 $docPath=$root_path.$news_fotos_path.$news['file_name'];
...
$smarty->assign('sImgAlign',$picalign);
			...
			if(file_exists($picpath)&& !empty($news['body']))
		   {
			...
			$sBuffer = $sBuffer.deactivateHotHtml($news['body']);
			if ($news_headline_body_font_bold) $sBuffer = $sBuffer. '</b>';
			$sBuffer = $sBuffer.'</font>';
			$smarty->assign('sNewsPreview',$sBuffer);
			$smarty->assign('linKed','Read File attachtment:>> <a href="'.$docPath.'" target="_blank"> '.$filename.'</a>');

		 	$smarty->assign('sEditorLink',$LDWrittenFrom.' '.$news['author'].' '.$LDWrittenOn.' '.formatDate2Local($news['publish_date'],$date_format)); //edited by Kenneth 05-07-2016
