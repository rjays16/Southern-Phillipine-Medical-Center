	...
	function display($skey=''){
		...
		if(($this->mode=='search' || $this->mode=='paginate') && !empty($searchkey)){
			...
			if($mode=='paginate'){
				...
				$ergebnis=$person->SearchSelectWithCurrentEncounter($searchkey,$pagen->MaxCount(),$pagen->BlockStartIndex(),$oitem,$odir,$firstname_too);
				#$ergebnis=$person->SearchSelectWithCurrentEncounter2($searchkey,$pagen->MaxCount(),$pagen->BlockStartIndex(),$oitem,$odir,$firstname_too);
			...
			if($mode=='paginate'){
				...
				$sql="SELECT pid, name_last, name_first, date_birth, addr_zip, sex, death_date, status ".
			...
			while($zeile=$ergebnis->FetchRow()){
				...
				if($zeile['status']=='' || $zeile['status']=='normal'){
					...
					if ($this->seg_send_to_input) {
						...
						$sTarget .= $docTarget . "getElementById('".$control_id."_id').value='".$zeile['encounter_nr'] . "';";
