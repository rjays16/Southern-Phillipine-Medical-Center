...
class Paginator {
	...
	*/
	var $rootpath;
	/**
	* for duplicate person mode; will be set to TRUE in class_gui_duplicate_person.php
	* @var boolean
	* burn added : May 22, 2007
	*/
	var $duplicate_mode=FALSE;
	...
	*/
	function makePrevLink($txt='',$append=''){
		if($this->duplicate_mode)
			$this->yhPgPrev=$this->page.URL_APPEND.'&mode=save&dup_mode=paginate&pgx='.$x.'&totalcount='.$this->tcount.'&oitem='.$this->sort_item.'&odir='.$this->sort_dir.$append;
		else
			$this->yhPgPrev = $this->page.URL_APPEND.'&mode=paginate&pgx='.$x.'&totalcount='.$this->tcount.'&oitem='.$this->sort_item.'&odir='.$this->sort_dir.$append;
		...
		if (!$this->csx){
			...
			if ($this->duplicate_mode){
				return '<a href="'.$this->page.URL_APPEND.'&mode=save&dup_mode=paginate&pgx='.$x.'&totalcount='.$this->tcount.'&oitem='.$this->sort_item.'&odir='.$this->sort_dir.$append.'"><< '.$txt.'</a>';
			}else{
	...
	*/
	function makeNextLink($txt='',$append=''){
		...
		if($this->duplicate_mode)
			$this->yhPgNext = $this->page.URL_APPEND.'&mode=save&dup_mode=paginate&pgx='.$x.'&totalcount='.$this->tcount.'&oitem='.$this->sort_item.'&odir='.$this->sort_dir.$append;
		else
			$this->yhPgNext = $this->page.URL_APPEND.'&mode=paginate&pgx='.$x.'&totalcount='.$this->tcount.'&oitem='.$this->sort_item.'&odir='.$this->sort_dir.$append;
	...
	*/
	function SortLink($txt,$item,$dir,$flag=FALSE,$append=''){
		#echo "value = ".$txt." , ".$item." , ".$dir." , ".$flag." , ".$append;
		if(empty($txt)) $txt='Sort';
		#echo "sort = ".$this->sort_dir;
		if(empty($dir)){
		...
		#echo "2: txt:$txt, item:$item, dir:$dir, flag:$flag <br>";
		if ($this->duplicate_mode){
			#echo "true = ".$this->duplicate_mode;
			return '<a href="'.$this->page.URL_APPEND.'&mode=save&dup_mode=paginate&pgx='.$this->csx.'&totalcount='.$this->tcount.'&oitem='.$item.'&odir='.$dir.$append.'">'.$img.$txt.'</a>';
		}else{
			#echo "false = ".$this->duplicate_mode;
			#echo $this->page.URL_APPEND.'&mode=paginate&pgx='.$this->csx.'&totalcount='.$this->tcount.'&oitem='.$item.'&odir='.$dir.$append.'">'.$img.$txt;
	...
	*/
	function makeSortLink($txt,$item,$oitem,$odir,$append){
		...
		#echo "1: txt=$txt, item=$item, oitem=$oitem, odir=$odir <br>";
