<?php
  ...
  class SegCashierEditOR extends Core
  {
    ...
    function getORDetails($from_ORNo, $to_ORNo)
    {
      ...
      $checker=0;
      $temp_or_array=array();
      $this->sql="SELECT or_no,or_name,or_date,cancel_date,cancelled_by,create_id from $this->pay_tb WHERE CAST(or_no AS UNSIGNED) BETWEEN ".((int)$from_ORNo)." AND ".((int)$to_ORNo)." ORDER BY or_no ASC LIMIT 15";
      $orno_array=array();
      $orname_array=array();
      $ordate_array=array();
