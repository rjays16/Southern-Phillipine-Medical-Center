...
class Person extends Core {
    ...
    const PEDIA_AGE_LIMIT = 19;
		...
		*/
		...
		function internResolveEncounter_nr($encounter_nr) {
				if (empty($encounter_nr)) {
						if(empty($this->encounter_nr)) {
								return false;
						} else { return true; }
				} else {
		...
		*/
		...
		/**
		 * @access public
		 * @author syboy : 06/27/2015
		 * get mother other occupation
		 */ 
		function getMOccupationother($ref_nr){
			global $db;
			$this->sql = "SELECT m_occupation_other FROM seg_cert_birth WHERE m_occupation = 650 AND pid = $ref_nr";
		 ...
		 * @access public
		 * @author syboy : 06/27/2015
		 * get father other occupation
		 */ 
		function getFOccupationother($ref_nr){
			global $db;
			$this->sql = "SELECT f_occupation_other FROM seg_cert_birth WHERE f_occupation = 650 AND pid = $ref_nr";
			$result = $db->GetOne($this->sql);
			return $result;
		}
...
#echo "msg :  <br>\n $msg <br>\n ";
		...
		*/
		...
		function insertVaccination($data) {
            global $db;
            $pk = array('pid');
            $fields = array(
                'pid' => $data['pid'],
                'vac_details' => $db->qstr($data['vac_details']),
                'vac_date' => $db->qstr($data['vac_date'])
            );

            $rs = $db->Replace('seg_person_vaccination', $fields, $pk);
            return $rs;
        }
        #End

    //added by: Borj Radiology Readers Fee 2014-10-17
    function insertReaders($data){
            global $db;

            	$this->sql1 ="INSERT INTO seg_encounter_privy_dr
            				(encounter_nr, dr_nr, modify_dt, 
            				  modify_id, dr_charge, dr_role_type_nr, 
            				   create_dt, create_id, service_code)
            				VALUES (".$db->qstr($data['encounter_nr3']).", ".$db->qstr($data['dr_nr']).", ".$db->qstr($data['modify_dt']).",
            					     ".$db->qstr($data['modify_id']).", ".$db->qstr($data['dr_charge']).", ".$db->qstr($data['dr_role_type_nr']).",
            					      ".$db->qstr($data['create_dt']).",".$db->qstr($data['create_id']).", ".$db->qstr($data['service_code']).")";
            					      
				$this->sql = "UPDATE seg_doctor_accommodation_type SET accommodation_type = 2 WHERE encounter_nr = "
					.$db->qstr($data['encounter_nr3'])." AND dr_nr = ".$db->qstr($data['dr_nr']).
					" AND dr_role_type_nr = ".$db->qstr($data['dr_role_type_nr']) ." ORDER BY entry_no DESC LIMIT 1";

						if($this->ok=$db->Execute($this->sql1) && $db->Execute($this->sql)){
								return TRUE;
							}else{
								return FALSE;
						}
        ...
        #End
// Added by Matsuu 01262017
        function insertDocumentsPHIC($is_pid,$documents,$encoder){
        	global $db;
        	$docu = $db->qstr($documents);
        	$docu = htmlspecialchars($docu);
        	$sql1="SELECT pid, requirement FROM seg_phic_requirement WHERE pid =".$db->qstr($is_pid);
			$result = $db->GetRow($sql1);
			if($result){
				$get_pid = $result['pid'];
			}
			if(!empty($get_pid)) {
				$this->sql="UPDATE seg_phic_requirement SET requirement =".$docu.",modify_id=".$db->qstr($_SESSION['sess_user_pid'])." WHERE pid =".$db->qstr($is_pid);
			} else {
				$this->sql="INSERT INTO seg_phic_requirement(pid,requirement,modify_id) VALUES(".$db->qstr($is_pid).",".$docu.",".$db->qstr($_SESSION['sess_user_pid']).")";
			}
        	if($this->ok=$db->Execute($this->sql)) {
        		return TRUE;
        	} else {
        		return FALSE;
        	}
        ...
        // Ended by Matsuu 01262017
        //Added by Christian 06082020
        function updatePhicMemberInfo($pid,$encount_nr,$lastName,$firstName,$middleName,$suffix,$sex,$birthDate){
        	global $db;
        	$this->sql="UPDATE 
					    seg_encounter_insurance_memberinfo 
					  SET
					    member_lname = ".$db->qstr($lastName).",
					    member_fname = ".$db->qstr($firstName).",
					    member_mname = ".$db->qstr($middleName).",
					    suffix = ".$db->qstr($suffix).",
					    sex = ".$db->qstr($sex).",
					    birth_date = ".$db->qstr($birthDate)."
					  WHERE pid = ".$db->qstr($pid)." 
					    AND encounter_nr = ".$db->qstr($encount_nr)."
        ...
        // end Christian 06082020
        // Added by Matsuu 01262017
       function insertAuditPHIC($is_pid,$documents,$encoder){
        	global $db;
        	$docu = $db->qstr($documents);
        	$docu = htmlspecialchars($docu);
        	
			$this->sql = "INSERT INTO seg_audit_phic(date_changed,encoder,pid,old_requirement) 
					VALUES(NOW(),".$db->qstr($encoder).",".$db->qstr($is_pid).",".$docu.")";
        	if($this->ok=$db->Execute($this->sql)) {
        		return TRUE;
        	} else {
        		return FALSE;
...
#        echo "<br><br>\n";
		...
		*/
		...
		function getAllInfoObject($pid='') {
				...
				$this->sql="SELECT p.*, addr.name AS addr_citytown_name,ethnic.name AS ethnic_orig_txt,
									 ...
									 sr.region_name, oc.occupation_name, spv.vac_details, spv.vac_date
										...
										LEFT JOIN seg_person_vaccination AS spv ON spv.pid = p.pid
									WHERE $pid_format ";
		...
		function getAllInfoArrayforBB($pid='',$encounter_nr=''){
				global $db;
				$x='';
				$v='';
				$encounter_nr_format='';
				...
				*/
						...
						$pid_format = " p.pid='".$this->pid."' ";
						if($this->internResolveEncounter_nr($encounter_nr)){

							$encounter_nr_format = "AND enc.encounter_nr='".$this->encounter_nr."' ";
						}else{
							$encounter_nr_format = '';
						}
				...
				$this->sql= "SELECT p.*, sb.brgy_name, sm.zipcode, sm.mun_name, sp.prov_name, sr.region_name,
														...
														IF((SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_discharged)),20))=0,(SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.admission_dt)),20)),NULL) AS admission_dt,
														IF((SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_discharged)),20))=0,(SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.encounter_date)),20)),NULL) AS encounter_date,
														IF((SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_discharged)),20))=0,(SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.encounter_nr)),20)),NULL) AS encounter_nr,
														IF((SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_discharged)),20))=0,(SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.current_dept_nr)),20)),NULL) AS current_dept_nr,
														IF((SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_discharged)),20))=0,(SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.current_ward_nr)),20)),NULL) AS current_ward_nr,
														IF((SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_discharged)),20))=0,(SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.current_room_nr)),20)),NULL) AS current_room_nr,
														IF((SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_discharged)),20))=0,(SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.encounter_type)),20)),NULL) AS encounter_type   
												FROM  $this->tb_person AS p
																...
																LEFT JOIN seg_barangays AS sb ON sb.brgy_nr=p.brgy_nr
																		LEFT JOIN seg_municity AS sm ON sm.mun_nr=p.mun_nr
																				LEFT JOIN seg_provinces AS sp ON sp.prov_nr=sm.prov_nr
																						LEFT JOIN seg_regions AS sr ON sr.region_nr=sp.region_nr
												WHERE $pid_format $encounter_nr_format GROUP BY pid ";
		...
		*/
		function getAllInfoArray($pid='') {
				...
				*/
						$pid_format = " p.pid='".$this->pid."' ";
						$encounter_nr_format = $encounter_nr != '' ? 'AND enc.encounter_nr = $db->qstr($encounter_nr)' : '';
						# burn added: October 19, 2007
						#line 687 is added by VAN 05-27-08
				$this->sql= "SELECT p.*, sb.brgy_name, sm.zipcode, sm.mun_name, sp.prov_name, sr.region_name,
														sc.country_name AS country_citizenship,
		...
		function getValueByList($list,$pid='') {
				...
				$from = " AS cp
									LEFT JOIN seg_barangays AS sb ON sb.brgy_nr=cp.brgy_nr
										 ...
										 LEFT JOIN seg_provinces AS sp ON sp.prov_nr=sm.prov_nr
										 LEFT JOIN seg_regions AS sr ON sr.region_nr=sp.region_nr
										 LEFT JOIN care_personell AS ps ON cp.pid=ps.pid AND ps.status NOT IN ('deleted','hidden','inactive','void')
...
#echo"getValueByList : this->sql = '".$this->sql."' <br> \n";
		...
		*/
		...
		//added by jamen 05/30/17
		function Mother_FirstName() {
				return  $this->getValue('mother_fname');
		}

		function Mother_MiddleName() {
				return  $this->getValue('mother_mname');
		}

		function Mother_LastName() {
				return  $this->getValue('mother_lname');
		}
		...
		*/
        ...
        /**
		*Added by Jasper Ian Q. Matunog 10/28/2014
		*Get current encounter of newborn well baby
		*returns encounter_nr
        **/
		function getNewbornEncounter($pid) {
			global $db;
			if (!$pid) return false;
			$pid_format = " pid='$pid' ";
			$strSql = "SELECT encounter_nr, er_opd_diagnosis FROM $this->tb_enc
                                        WHERE $pid_format AND encounter_type = 12 AND status NOT IN ($this->dead_stat)
		...
		*/
		...
		#    else
				...
				$this->sql="SELECT admission_dt, encounter_nr,encounter_date,encounter_class_nr,encounter_type,is_discharged,discharge_date,official_receipt_nr
										 FROM $this->tb_enc WHERE $pid_format AND encounter_type not in (12,5)
										 AND status NOT IN ($this->dead_stat)
										 ORDER BY encounter_date DESC ";
		...
		function EncounterListIPBM($pid,$IPBMfilter=0){
				global $db;
				if(!$pid) return false;
				$encounterFilter="('13','14')";
				if($IPBMfilter==1) $encounterFilter="('13')";
				elseif($IPBMfilter==2) $encounterFilter="('14')";
		...
		#    else
				...
				$this->sql="SELECT admission_dt, encounter_nr,encounter_date,encounter_class_nr,encounter_type,is_discharged,discharge_date,official_receipt_nr
										 FROM $this->tb_enc WHERE $pid_format AND encounter_type IN ".$encounterFilter."
										 AND status NOT IN ($this->dead_stat)
										 ORDER BY encounter_date DESC ";
				if($this->res['_enl']=$db->Execute($this->sql)){
						if($this->rec_count=$this->res['_enl']->RecordCount()) {
		...
		function updatePatientSeniorID($pid,$senior_ID){
			global $db;
	
			$this->sql = "UPDATE $this->tb_person SET senior_ID=".$db->qstr($senior_ID)." WHERE pid=" . $db->qstr($pid);
			$db->Execute($this->sql);
...
#            $cbuffer=explode(' ',$searchkey);
            ...
            $last_name_soundex = $ln;
            $first_name_soundex = $fn;
            #soundex in PHP and MYSQL is not the same
                ...
                $sql2 = " WHERE (soundex_namelast LIKE CONCAT(SOUNDEX(" . $db->qstr($last_name_soundex) . "),'%') AND soundex_namefirst LIKE CONCAT(SOUNDEX(" . $db->qstr($first_name_soundex) . "),'%'))" ;
								if(!empty($bd)){
                                ...
                                $sql2 = " WHERE (soundex_namelast LIKE CONCAT(SOUNDEX(" . $db->qstr($last_name_soundex) . ",'%') AND soundex_namefirst LIKE CONCAT(SOUNDEX(" . $db->qstr($first_name_soundex) . ",'%'))";
																...
																#edited by VAN 06-14-2011
                                $sql2 = " WHERE soundex_namelast LIKE CONCAT(SOUNDEX(" . $db->qstr($last_name_soundex) . ",'%') ";
														}
												}else{
														#$sql2=" WHERE name_last $sql_LIKE '".strtr($suchwort,'+',' ')."%' ";
														#edited by VAN 06-14-2011
				...
				//added by Earl Galope 02/16/2018
				//purpose to sort duplicate person result
				if(sizeof($comp)>1){
					$sql3=" ORDER BY name_last, name_first, name_middle, date_birth";
				}
				//end
						...
						if(sizeof($comp)>1){
								...
								if (sizeof($cbuffer) > 2){
										...
										if ($lnameOnly){
												...
												#$sql2=" WHERE ($sqlOldWhere(name_last $sql_LIKE '".strtr($ln,'+',' ')."%' AND name_first $sql_LIKE '".strtr($fn,'+',' ')."%'))"; commented by art 07/14/14

												# added by art 07/14/14
												if($from== 'medocs'){
													$sql2=  " WHERE ((name_last LIKE '".strtr($ln,'+',' ')."%') OR (n.old_name_last LIKE '".strtr($ln,'+',' ')."%')) ".
													"AND ((name_first LIKE '".strtr($fn,'+',' ')."%') OR (n.old_name_first LIKE '".strtr($fn,'+',' ')."%')) ". 
													"OR n.old_name_first LIKE '".strtr($fn,'+',' ')."%' OR n.old_name_last LIKE '".strtr($ln,'+',' ')."%'" ;
												#end art
												}else{
													$sql2=" WHERE ($sqlOldWhere(name_last $sql_LIKE '".strtr($ln,'+',' ')."%' AND name_first $sql_LIKE '".strtr($fn,'+',' ')."%'))";
												}
...
#                        if(defined('SHOW_FIRSTNAME_CONTROLLER')&&SHOW_FIRSTNAME_CONTROLLER){
				...
				$fromMedRec = ($from == 'medocs' ? 'LEFT JOIN seg_audit_name n ON n.pid = cp.pid ' : '');					
				#edited by art 07/14/14 added seg_audit_name
				$sql2 = " AS cp 
										".$fromMedRec." 
										...
										LEFT JOIN seg_charity_grants_expiry_pid AS scgep ON scgep.pid = scp.pid AND scgep.grant_dte = scp.grant_dte
										...
										LEFT JOIN seg_charity_grants_expiry AS scge ON  scge.encounter_nr = se.encounter_nr AND scge.grant_dte = se.grant_dte
										LEFT JOIN seg_barangays AS sb ON sb.brgy_nr=cp.brgy_nr
										LEFT JOIN seg_municity AS sm ON sm.mun_nr=cp.mun_nr
										LEFT JOIN seg_provinces AS sp ON sp.prov_nr=sm.prov_nr
										LEFT JOIN seg_regions AS sr ON sr.region_nr=sp.region_nr
				...
				#edited by VAN 04-16-08
						...
						$this->sql= " SELECT SQL_CALC_FOUND_ROWS rd.rid, cp.photo_filename,dep.dependent_pid, ps.nr AS personnelID, cp.civil_status, cp.senior_ID, cp.fromtemp, cp.pid, $old_select cp.name_last,cp.name_first,cp.name_middle,cp.date_birth,cp.addr_zip, cp.sex,cp.death_date,cp.death_encounter_nr, cp.status,cp.street_name,
                                                        ...
                                                        SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.er_location)),20) AS er_location,
                                                        SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.er_location_lobby)),20) AS er_location_lobby,
                ...
                "fn_get_phic_number(SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.encounter_nr)),20)) AS `phic_nr`, cw.accomodation_type as acc_type, (SELECT sg.discountid FROM seg_charity_grants_pid sg LEFT JOIN care_person cp ON cp.pid = sg.pid $sg_pid ORDER BY sg.grant_dte DESC LIMIT 1) AS DiscountID, SUBSTRING(MAX(CONCAT(scp.grant_dte,scgep.pwd_expiry)),20) AS pid_expiry,SUBSTRING(MAX(CONCAT(se.grant_dte, scge.pwd_expiry)),20) AS enc_expiry\n".
                "".
				//end cha
												" FROM ".$this->buffer.$sql3;
	 #echo $this->sql;
...
#                        if(defined('SHOW_FIRSTNAME_CONTROLLER')&&SHOW_FIRSTNAME_CONTROLLER){
				...
				if ($enctype==1){
						...
						#                AND (SELECT is_discharged FROM care_encounter AS enc WHERE cp.pid=enc.pid AND enc.encounter_status <> 'cancelled' AND enc.status NOT IN ('deleted','hidden','inactive','void') ORDER BY enc.encounter_nr DESC LIMIT 1)=0";
						$enctype_cond = " AND encounter_type IN (2,14) /*AND is_discharged=0*/";
				}else if ($enctype==3){
						...
						#                AND (SELECT is_discharged FROM care_encounter AS enc WHERE cp.pid=enc.pid AND enc.encounter_status <> 'cancelled' AND enc.status NOT IN ('deleted','hidden','inactive','void') ORDER BY enc.encounter_nr DESC LIMIT 1)=0";
						$enctype_cond = " AND encounter_type IN (3,4,13) /*AND is_discharged=0*/";
				}else if ($enctype==13){
						#$enctype_cond = "AND (SELECT encounter_type FROM care_encounter AS enc WHERE cp.pid=enc.pid AND enc.encounter_status <> 'cancelled' AND enc.status NOT IN ('deleted','hidden','inactive','void') ORDER BY enc.encounter_nr DESC LIMIT 1) IN (3,4)
						#                AND (SELECT is_discharged FROM care_encounter AS enc WHERE cp.pid=enc.pid AND enc.encounter_status <> 'cancelled' AND enc.status NOT IN ('deleted','hidden','inactive','void') ORDER BY enc.encounter_nr DESC LIMIT 1)=0";
						$enctype_cond = " AND encounter_type IN ('13')";
				}else if ($enctype==14){
						$enctype_cond = " AND encounter_type IN ('14')";
				}else if ($enctype==1314){
						$enctype_cond = " AND encounter_type IN ('13','14')";
				}else{
						$enctype_cond = "";
				}
				...
				if(isset($oitem)&&!empty($oitem)) $sql3 =" ORDER BY $oitem $odir";
						...
						if (empty($suchwort)){
							...
							$this->sql= "SELECT SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.encounter_nr)),20) AS encounter_nr,
														...
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.current_room_nr)),20) AS current_room_nr,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.current_dept_nr)),20) AS current_dept_nr,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.er_location)),20) AS area_location,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.er_location_lobby)),20) AS lobby_area,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.is_medico)),20) AS is_medico,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.encounter_date)),20) AS encounter_date,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.admission_dt)),20) AS admission_dt,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.discharge_date)),20) AS discharge_date,
		...
		function SearchSelectWithCurrentEncounter($searchkey='',$maxcount=100,$offset=0,$oitem='name_last',$odir='ASC',$fname=FALSE,$senc_nr=''){
				global $db, $sql_LIKE, $root_path;
				...
				if (strcmp($senc_nr, "") != 0) {
						$sql_enc .= " AND tbenc.encounter_nr = '$senc_nr'";
				}
				#end cha
				$this->buffer=$this->tb_person.$sql2;
				...
				$this->sql="SELECT SQL_CALC_FOUND_ROWS
                                                        ...
                                                        SUBSTRING(MAX(CONCAT(tbenc.encounter_date,tbenc.er_location)),20) AS er_location,
                                                        SUBSTRING(MAX(CONCAT(tbenc.encounter_date,tbenc.er_location_lobby)),20) AS er_location_lobby,
...
function SearchForBilling($searchkey='',$maxcount=100,$offset=0,$oitem='name_last',$odir='ASC',$fname=FALSE, $senc_nr=''){
				...
				// included IPBM encounter types 13 & 14 by Carriane 05/10/18 
				$sql2 .=" AND ((tbenc.is_discharged=0 AND death_date in (null,'0000-00-00','')) OR (tbenc.is_discharged = 0 AND death_date > '0000-00-00') OR (tbenc.is_discharged=1 AND death_date >= '0000-00-00')) AND tbenc.encounter_type IN (1,2,3,4,12,".IPBMIPD_enc.", ".IPBMOPD_enc.") AND tbenc.encounter_status <> 'cancelled' AND tbenc.status NOT IN ($this->dead_stat) ";
...
#----------- added by LST 06-26-2008 to allow search using encounter_nr ---- EOB ----
                ...
                // included IPBM encounter types 13 & 14 by Carriane 05/10/18 
                // included Well Baby encounter types 12 fritz 02/08/19 
				$sql2 .= " AND IF((SELECT COUNT(encounter_nr) FROM care_encounter WHERE pid=tbenc.pid)=1,tbenc.encounter_type IN (1,2,3,4,".WELLBABY.",".IPBMIPD_enc.", ".IPBMOPD_enc."),tbenc.encounter_type IN (1,2,3,4,".WELLBABY.",".IPBMIPD_enc.", ".IPBMOPD_enc.")) ";
				...
				if(isset($oitem)&&!empty($oitem)) $sql3 .= "ORDER BY $oitem $odir";
					 ...
					 $this->sql="SELECT SQL_CALC_FOUND_ROWS tbperson.pid,
						...
						// "fn_get_phic_number(tbenc.encounter_nr) AS `phic_nr`, ". //commented by Nick 05-27-2014
						"fn_get_phic_number(SUBSTRING(MAX(CONCAT(tbenc.encounter_date,tbenc.encounter_nr)),20)) AS `phic_nr`, ".//added by Nick 05-27-2014, Load Insurance Number Properly
		...
		/*
				$this->sql="SELECT pid,name_last,name_first,date_birth,sex FROM $this->tb_person
										WHERE name_last $sql_LIKE '%".$data['name_last']."%'
												...
												$cond ";
		*/
		#echo "name -".$data['name_last'].", ".$data['name_first']."<br>";										
        #$last_name_soundex = soundex($data['name_last']);
        #$first_name_soundex = soundex($data['name_first']);
        #echo "name -".$last_name_soundex.", ".$first_name_soundex."<br>";										
		...
		#edited by VAN 09-29-2016
        $this->sql = "SELECT pid,name_last,name_first,name_middle,date_birth,sex FROM $this->tb_person
										WHERE soundex_namelast = SOUNDEX(" . $db->qstr($data['name_last']) . ")
										AND soundex_namefirst = SOUNDEX(" . $db->qstr($data['name_first']) . ")";
...
#exit();
		...
		*/
		...
		function concatname($slast, $sfirst, $smid, $isfromaffidavit=0) {
				$stmp = "";

				if($isfromaffidavit){
					$slast = utf8_decode($slast);
					$sfirst = utf8_decode($sfirst);
					$smid = utf8_decode($smid);

					if (!empty($sfirst)) $stmp .= $sfirst;
					if (!empty($smid)) {
							if (!empty($stmp)) $stmp .= " ";
							$stmp .= $smid;
					}
		...
		// Update by carriane 03/08/2018
		#----------added by VAN 05-19-08
		function updatePersonInfo($pid,&$data,$fromDeath=0){
			...
			if(!$fromDeath){
				if(isset($this->data_array['street_name'])) unset($this->data_array['street_name']);
				if(isset($this->data_array['brgy_nr'])) unset($this->data_array['brgy_nr']);
			}
		...
		*/
		...
		function selectDeadDateStatus($encounter_nr){
			 global $db;
            $this->sql = "SELECT result_code FROM seg_encounter_result WHERE encounter_nr=".$db->qstr($encounter_nr);
            $this->result=$db->GetRow($this->sql);
            return $this->result;
		}

		function selectDOAstatus($encounter_nr){
			global $db;
			$this->sql= "SELECT is_DOA FROM care_encounter WHERE encounter_nr=".$db->qstr($encounter_nr);
			$this->result=$db->GetRow($this->sql);
			return $this->result;
		}

		// Added 2 function by Matsu 12042016
		function updateDeathDate1($pid='', $encounter_nr=0){
			global $db;
		$deadstatus = "added dead status";	
		$this->sql="UPDATE care_encounter SET
										history = ".$this->ConcatHistory("\n"."Update : ".$deadstatus." ".date('Y-m-d H:i:s')." ".$_SESSION['sess_user_name']."\n")."
										WHERE encounter_nr='$encounter_nr'";
				return $this->Transact($this->sql);
				
		}
		function updateDeathDate2($pid='', $encounter_nr=0){
			global $db;
		$deadstatus = "removed dead status";	
		$this->sql="UPDATE care_encounter SET
										history = ".$this->ConcatHistory("\n"."Update : ".$deadstatus." ".date('Y-m-d H:i:s')." ".$_SESSION['sess_user_name']."\n")."
										WHERE encounter_nr='$encounter_nr'";
				return $this->Transact($this->sql);
		...
		// Ended 2 function by Matsu 12042016
   ...
   function getCreateID($pid='') {
	...
	//created by Kenneth 09/28/2016
     function isPatientInsuranceSet($pid) {
         global $db;
     ...
     //created by Kenneth 03/02/2017
     function isPatientFirstInsurance($pid,$enr) {
         global $db;
         $this->sql = "SELECT * FROM seg_encounter_insurance_memberinfo_first seimf WHERE seimf.`encounter_nr`='{$enr}' AND seimf.`pid`='{$pid}' AND seimf.`hcare_id`=".PHIC_ID;
        if ($result=$db->Execute($this->sql)) {
            if ($result->RecordCount()) {
                if ($row = $result->FetchRow()) {
                    return false;
     ...
     //created by Kenneth 03/02/2017
     function isPatientFirstTemp($pid) {
         global $db;
         ...
         $this->sql = "SELECT 
                        insurance_nr 
                      FROM
                        seg_encounter_insurance_memberinfo_first
                      WHERE pid = '{$pid}' 
                      	AND insurance_nr='TEMP'
                        AND hcare_id = ".PHIC_ID;
        if ($result=$db->Execute($this->sql)) {
            if ($result->RecordCount()) {
                if ($row = $result->FetchRow()) {
    ...
    public function saveInsuranceMembershipInfo($params) 
    {
        global $db;
        //created by Kenneth 09/28/2016
        // $insurens=(($params['insurance_nr']=='TEMP')?'TEMP':'tempelatius');
        $fields = array(
            ...
            'patient_pin' =>$db->qstr($params['patient_pin']),
            'member_lname' =>$db->qstr($params['member_lname']),
            'member_fname' =>$db->qstr($params['member_fname']),
            'member_mname' =>$db->qstr($params['member_mname']),
            'suffix' =>$db->qstr($params['suffix']),
            'sex' => $db->qstr($params['sex']),
            'birth_date' =>$db->qstr($params['birth_date']),
            'street_name' =>$db->qstr($params['street_name']),
            'brgy_nr' =>$db->qstr($params['brgy_nr']),
            'mun_nr' =>$db->qstr($params['mun_nr']),
            'relation' =>$db->qstr($params['relation']),
            'member_type' =>$db->qstr($params['member_type']),
            'employer_no' =>$db->qstr($params['employer_no']),
            'employer_name' =>$db->qstr($params['employer_name']),
            'was_temp'=>$db->qstr($params['was_temp'])
        );
        $fields['create_dt'] = $db->qstr(date('Y-m-d H:i:s'));
        $fields['create_id'] = $db->qstr($_SESSION['sess_login_userid']);
        $pk = array('encounter_nr','pid','hcare_id');
        if($this->isPatientFirstInsurance($params['pid'],$params['encounter_nr'])&&$params['insurance_nr']!='TEMP') //created by Kenneth 09/28/2016
        $db->Replace('seg_encounter_insurance_memberinfo_first', $fields, $pk, false); //created by Kenneth 09/28/2016
        $db->StartTrans();
        ...
        if ($ok && !empty($params['encounter_nr'])) {
            ...
            $data = array(
				...
				'other_remarks' => $db->qstr($params['other_remarks'])
            );
        ...
        $params['employer_name'] = $this->convertEncoding($params['employer_name']);
        //end added by mai

        # Added by jeff 04-27-18 using qstr function instead of autoquote below(false from true) for compatibility issues.
        foreach ($params as $key => $value) {
            $params[$key] = $db->qstr($value);
        }
        # END jeff ---
    ...
    * @param string pid
    * @return boolean
    * edited by art added parameter $type (set death or remove) 01/21/15
    */
    function setDeathDate($pid, $type, $enc, $deathdate)
    {
	   	global $db, $HTTP_SESSION_VARS;
	   	#edited by art 01/21/15
	   	if($type!='0'){ #IF DEAD
	   		$resultCode = '4';
	   		$death_enc = $enc;
	   		$dead_action ='added dead status';
	   		$from_billing = 1;
	   	}
	   	else{ 
	   		$rs = $db->GetRow("SELECT `result_code`,`death_date`,`death_time` FROM seg_medocs_result_code WHERE encounter_nr =".$db->qstr($enc));
	   		if ($rs) {
	   			$resultCode = $rs['result_code'];
	   			$deathdate = $rs['result_code'] == '8' ? $rs['death_date'].' '.$rs['death_time'] : '00-00-00 00:00:00';
	   			$death_enc = $rs['result_code'] == '8'? $enc : 0;
	   			$dead_action = $rs['result_code'] == '8'? 'removed dead status and automatically retain saved result_code from medical records' : 'removed dead status';
	   			$from_billing = 0;
	   		}else{
	   			$resultCode = '3';
	   			$death_enc = 0;
	   			$dead_action ='removed dead status';
	   			$from_billing = 0;
	   		}
	    ...
	    $strSQL = "UPDATE care_person SET
	                                ...
	                                history = CONCAT(history,'\\n Update: $dead_action ',NOW(),' [$userid]\\n'),
	                                modify_id = '$userid',
	                                modify_time = NOW(),
	                                death_encounter_nr = '$death_enc'
	                         WHERE pid = ".$db->qstr($pid);
	    $strSQL1 ="UPDATE care_encounter SET 
    									history = CONCAT(history,'\\n Update: $dead_action ',NOW(),' [$userid]\\n')
	    					WHERE encounter_nr = ".$db->qstr($enc);

	    $bSuccess2 = $db->Execute($strSQL1);
    ...
    function getEncounterInsuranceMemberInfo($encounter_nr,$fields="*"){
        global $db;
        $this->sql = $db->Prepare("SELECT $fields FROM seg_encounter_insurance_memberinfo WHERE encounter_nr = ?");
        return $db->Execute($this->sql,$encounter_nr);
    }

    #added by janken 11/12/2014 for saving additional information in phic membership
    function saveAdditionalDetails($details){
    	global $db;

    	$gender = $db->qstr($details['gender']);
    	$landline = $db->qstr($details['landline']);
    	$mobile = $db->qstr($details['mobile']);
    	$email = $db->qstr($details['email']);
    	$member = $db->qstr($details['member']);
    	$name = $db->qstr($details['name']);
    	$type = $db->qstr($details['type']);
    	$relation = $db->qstr($details['relation']);
    	$pid = $db->qstr($details['pid']);
    	$id = $db->qstr($details['id']);
    	$incap = $db->qstr($details['incapacitated']);
    	$reason = $db->qstr($details['reason']);

    	if (!empty($details['date'])) {
		   $dt = str_replace('-', '/', $details['date']);
    	...
    	$sql = "UPDATE seg_insurance_member_info 
    				SET sex = $gender, landline_no = $landline,
    					mobile_no = $mobile, email_address = $email,
    					is_member = $member, signatory_name = $name,
    					signatory_relation = $relation, relation_type = $type,
    					signatory_date = $bd, reason = $reason, is_incapacitated = $incap
    				WHERE pid = $pid AND hcare_id = $id";
    ...
    //added by Nick 4-12-2015
    public static function setAdmissionDate($pid,$admissionDate){
    	global $db;
    	return $db->Execute("UPDATE care_encounter SET encounter_date=? WHERE pid=? AND encounter_type=?",array(
    		$admissionDate,
    		$pid,
    		WELLBABY
    	));
    ...
    //added by VAS 08/15/2017
    public function isPersonExists($firstName, $lastName){
        global $db;

        #edited by VAS 09/24/2017
        #the soundex result in PHP is sometimes not the same with soundex in MySQL
        $this->sql = "SELECT pid,
                             name_last,
                             name_first,
                             name_middle,
                             ...
                             sex
                      FROM care_person
                      WHERE soundex_namelast LIKE CONCAT(SOUNDEX(?),'%')
                      AND soundex_namefirst LIKE CONCAT(SOUNDEX(?),'%')
                      ";
        
        #$result = $db->Execute($this->sql, array(soundex($lastName),soundex($firstName)));
        $result = $db->Execute($this->sql, array($lastName,$firstName));
     ...
     function encReligion($enc){
    	global $db;
    	$this->sql = "SELECT r.religion_name FROM seg_religion AS r
						INNER JOIN care_person AS cp 
						ON cp.`religion` = r.`religion_nr`
    ...
    //  Get patient's age with no. of days ...
    function getPatientAgeYrwDays($pid) {
        global $db;
        ...
        $curdte = $dob->add(new DateInterval("P".$yrs."Y"));
        $days = $curdte->diff($to)->days;
        $age["d"] = $days;
        
        return $age;
    }

    # Added by fritz 02182019
	  function getPatientInfoByPid($pid){
	  	global $db;

	  	$sql = "SELECT pid, fn_get_person_name_first_mi_last(pid) AS patient_name, fn_get_person_lastname_first(pid) AS patient_lastnamefirst, civil_status,
	  			fn_get_complete_address(pid) AS address,fn_get_birth_date(pid) as birth_date,
	  			CASE WHEN (sex = 'm') THEN('Male') WHEN(sex = 'f') THEN('Female') ELSE ('') END AS gender,
	  			fn_get_age(NOW(),date_birth) AS age
	  			FROM care_person
	  			 WHERE pid =".$db->qstr($pid);
	  			 
	  	return $rs = $db->GetRow($sql);
	  }
	  #Ended here by fritz 
	  #
    function saveDoctorPf($data){
		global $db;
		$this->sql ="INSERT INTO seg_radio_doctors_pf
								(refno,dr_nr,pf_amount,accomodation_type,service_code,create_id,create_dt)
						 VALUES(".$db->qstr($data['refno']).",".$db->qstr($data['dr_no']).",".$db->qstr($data['amount']).",".$db->qstr($data['accomodation_type']).",".$db->qstr($data['service_code']).",".$db->qstr($_SESSION['sess_temp_userid']).",".$db->qstr($data['create_dt']).")";
		if($this->ok=$db->Execute($this->sql)){
					return TRUE;
			}else{
					return FALSE;
	...
	function getPatientInfoByEncounter($enc){
	  	global $db;

	  	$sql = "SELECT cp.pid, fn_get_person_name_first_mi_last(cp.pid) AS patient_name,ce.current_att_dr_nr as attending_dr_note,ce.consulting_dept_nr as dept_dr_note
	  			FROM care_person cp LEFT JOIN care_encounter ce on cp.pid=ce.pid
	  			 WHERE ce.encounter_nr =".$db->qstr($enc);
    ...
    function getPatientAddressInfo($pid) {
        global $db;
        $sql = "SELECT citizenship FROM care_person WHERE pid =".$db->qstr($pid);

        return $rs = $db->GetRow($sql);
    }

    #Added by Mats 03112020
    function insertReadersOB($data){

            global $db;

            $rs = $db->GetRow("SELECT  * FROM seg_encounter_pf_dr AS sepf WHERE sepf.`encounter_nr` =".$db->qstr($data['encounter_nr3'])." AND sepf.`dr_nr` = ".$db->qstr($data['dr_nr'])."  AND sepf.`dr_role_type_nr` = ".$db->qstr($data['dr_role_type_nr']));
            if($rs){
            	$entry_no = (int)$rs['entry_no'] + 1;
            }else{
            	$entry_no = 1;
            }
            	$this->sql1 ="INSERT INTO seg_encounter_pf_dr
            				(encounter_nr, dr_nr, modify_dt, 
            				  modify_id, dr_charge, dr_role_type_nr, 
            				   create_dt, create_id, service_code,entry_no)
            				VALUES (".$db->qstr($data['encounter_nr3']).", ".$db->qstr($data['dr_nr']).", ".$db->qstr($data['modify_dt']).",
            					     ".$db->qstr($data['modify_id']).", ".$db->qstr($data['dr_charge']).", ".$db->qstr($data['dr_role_type_nr']).",".$db->qstr($data['create_dt']).",".$db->qstr($data['create_id']).", ".$db->qstr($data['service_code']).", ".$db->qstr($entry_no).")";
            					      	// var_dump($this->sql1);die();

						if($this->ok=$db->Execute($this->sql1)){
								return TRUE;
							}else{
								return FALSE;
						}
