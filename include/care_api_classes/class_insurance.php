...
# added by shandy 10-01-2014
#per encounter inssurance
#edited by poliam
function getPatientInsuranceInfoEnc($pid, $encounter_nr, $insurance_nr){
	global $db;

		$this->sql = "SELECT IF(sei.`remarks` = '1', 'TEMP', seim.`insurance_nr`) AS insurance_nr,
						...
						WHERE seim.`encounter_nr` = ".$db->qstr($encounter_nr)."
						AND seim.`pid` = ".$db->qstr($pid)."
						AND seim.hcare_id = ".$db->qstr($insurance_nr);
 ...
 */
function getDialysisPatientLastInsuranceInfo($pid, $insurance_nr) {
	global $db;

	$this->sql = "SELECT IF(sei.`remarks` = '1', 'TEMP', seim.`insurance_nr`) AS insurance_nr,
  					IF(seim.`relation` = 'M', '1', '0') AS is_principal, seim.`pid` 
					FROM seg_encounter_insurance_memberinfo `seim` 
  					INNER JOIN seg_encounter_insurance sei 
    					ON sei.`encounter_nr` = seim.`encounter_nr` 
					WHERE seim.`pid` = ".$db->qstr($pid)." 
  						AND seim.hcare_id = ".$db->qstr($insurance_nr)." 
...
function getDialysisPatientLastEncounter($pid, $enc_nr) {
	global $db;

	$encounterNr = $db->GetOne("SELECT ce.`encounter_nr` FROM care_encounter ce 
								INNER JOIN seg_encounter_insurance_memberinfo seim 
    							ON seim.`encounter_nr` = ce.`encounter_nr`
								WHERE ce.`pid` = ".$db->qstr($pid)." 
								AND ce.`encounter_nr` != ".$db->qstr($enc_nr)."
...
function GetPatientInsurancePid($pid, $encounter_nr, $insurance_nr){
	global $db;

	$this->sql = "SELECT simi.`insurance_nr`, 
							IF(simi.`relation` = 'M', '1', '0') AS is_principal
				...
				ON simi.`pid` = ce.`pid`
				WHERE sei.`encounter_nr` = ".$db->qstr($encounter_nr)."
				AND ce.`pid` = ".$db->qstr($pid)."
				AND sei.`hcare_id` = ".$db->qstr($insurance_nr);
	if($this->result = $db->GetRow($this->sql)){
		return $this->result;
	}else{
		return false;
...
function getPatientInsuranceInfo($pid, $encounter_nr, $insurance_nr){
	global $db;

	$this->sql = "SELECT cpi.`insurance_nr`,
						cpi.`is_principal` 
					FROM care_person_insurance
					WHERE (pid=".$db->qstr($pid)."
					OR pid=(SELECT parent_pid
							 FROM seg_dependents `dep`
							 LEFT JOIN  care_person_insurance  `i` 
							 ON (i.`pid` = dep.`parent_pid` OR i.`pid` = dep.`dependent_pid`)
							 AND i.`is_void` = '0'
							 WHERE (dep.`parent_pid` = ".$db->qstr($pid)." OR dep.dependent_pid = ".$db->qstr($pid).")
								AND i.`hcare_id` = ".$db->qstr($insurance_nr)."
								AND dep.`status` NOT IN ('cancelled','deleted','expired') LIMIT 1)
							AND hcare_id = ".$db->qstr($insurance_nr)." 
							AND is_void = '0'";
	if($this->result = $db->GetRow($this->sql)){
		return $this->result;
			} else{
...
function getPatientInfoEnc($encounter_nr){
	global $db;

	$this->sql = "SELECT seim.`member_lname`,
							seim.`member_fname`,
							seim.`member_mname`,
							seim.`patient_pin`,
							seim.`suffix`,
	...
	function SearchSelectPHIC($searchkey='',$maxcount=100,$offset=0){
		global $db, $sql_LIKE, $root_path, $date_format;
		if(empty($maxcount)) $maxcount=100;
		if(empty($offset)) $offset=0;
...
//			} else { return false; }
	...
	*/
	...
	function is_member_info_editableENC($pid, $firm_id, $insurance_nr) {
		global $db;

		if($insurance_nr == '')
			$insurance_cond = "AND simi.insurance_nr IS NULL";
		else
			$insurance_cond = " AND simi.insurance_nr=".$db->qstr($insurance_nr)."";

		$this->sql = "SELECT simi.hcare_id, simi.insurance_nr, simi.member_lname, simi.member_fname, simi.member_mname, simi.street_name,
									simi.brgy_nr, simi.mun_nr, cp.pid FROM seg_encounter_insurance_memberinfo simi LEFT JOIN care_encounter cp
									 ON simi.`encounter_nr` = cp.`encounter_nr`
									WHERE simi.pid=".$db->qstr($pid)." AND simi.hcare_id= ".$db->qstr($firm_id)." $insurance_cond";
		#print_r($this->sql);
		if ($this->result = $db->Execute($this->sql)) {
...
//									WHERE simi.pid=$pid AND simi.hcare_id=$firm_id AND simi.insurance_nr='$insurance_nr' AND cp.pid IS NULL";
    ...
    function getHistoryInsurance($enc) {
		global $db;
		$enc = $db->qstr($enc);
		$this->sql = "SELECT history FROM seg_encounter_insurance_memberinfo WHERE encounter_nr = $enc";
	...
	//Added by EJ 11/20/2014
    function insertIssuanceHistory($enc,$encdr,$act) {
		global $db;
		
		$text = $act." by ".$encdr." on ".date("F j, Y, g:i a")."\nencounter_nr =".$enc."\n\n";
		$text = $db->qstr($text);
		$enc = $db->qstr($enc);
