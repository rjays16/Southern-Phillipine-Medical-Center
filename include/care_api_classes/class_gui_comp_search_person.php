...
#echo "search = ".$searchkey;
	...
	//	}
		...
		if(defined('SHOW_FIRSTNAME_CONTROLLER')&&SHOW_FIRSTNAME_CONTROLLER){
			...
			if ($this->ptype=='ipd' && !$isIPBM){
                $enctype = 3;
            }elseif ($this->ptype=='er' && !$isIPBM){
                $enctype = 1;
            }elseif ((($this->ptype=='opd')||($this->ptype=='phs')) && !$isIPBM){
                $enctype = 2;
            }
            elseif (($isIPBM && $this->ptype == "ipd")) {
            	$enctype = 13;
            }
            elseif (($isIPBM && $this->ptype == "opd")) {
            	$enctype = 14;
            }
            elseif (($isIPBM && $this->ptype == "ipbm")) {
            	$enctype = 1314;
            }else{
                $enctype = 0;    
            }
			...
			#additional option for IPBM
			if($isIPBM){
			$this->smarty->assign('sCheckAll','<input type="radio" name="enctype" id="enctype" value="1314" '.(($enctype==1314)?'checked="checked" ':'').' onClick="UpdateQuery(this.value);" >');
			$this->smarty->assign('LDCheckAll',"All");

			$this->smarty->assign('sCheckOPD','<input type="radio" name="enctype" id="enctype" value="14" '.(($enctype==14)?'checked="checked" ':'').' onClick="UpdateQuery(this.value);" >');
			$this->smarty->assign('LDCheckOPD',"IPBM - OPD");
			
			$this->smarty->assign('sCheckIPD','<input type="radio" name="enctype" id="enctype" value="13" '.(($enctype==13)?'checked="checked" ':'').' onClick="UpdateQuery(this.value);" >');
			$this->smarty->assign('LDCheckIPD',"IPBM - IPD");
			}
			else{
			...
			*/
			...
			while($zeile=$ergebnis->FetchRow()){
				...
				if($zeile['status']=='' || $zeile['status']=='normal'){
                    ...
                    */
                    //echo "<pre>".print_r($zeile,true)."</pre>";
                    if($zeile['encounter_type']==1){
                        ...
                        $label =    '<img '.createComIcon($root_path,'flag_green.gif').'>'.
                                    '<font size=1 color="green">IPD</font>';
                    }elseif($zeile['encounter_type']==6){ //added by Macoy, July 10, 2014
                        $label =    '<img '.createComIcon($root_path,'flag_blue.gif').'>'.
                                    '<font size=1 color="blue">IC</font>';                   
                    }elseif($zeile['encounter_type']==13){ //added by Kenneth 08/15/2017
                        $label =    '<img '.createComIcon($root_path,'flag_green.gif').'>'.
                                    '<font size=1 color="green">IPBM-IPD</font>';       
                    }elseif($zeile['encounter_type']==14){ //added by Kenneth 08/15/2017
                         $label =    '<img '.createComIcon($root_path,'flag_blue.gif').'>'.
					...
					if (($zeile['encounter_type']==1)||($zeile['encounter_type']==2)){
						...
						if ($zeile['encounter_type']==1){
							#$location = "ER";
							$loc_num = ($zeile['area_location'] == null) ? '0' : $zeile['area_location'];
							$lobby = $encounter_obj->getERLobbyInfo($zeile['lobby_area']); 
							$area_location = $encounter_obj->getERLocationInfo($loc_num);
							$location = ($area_location['area_location']) ? 'ER - ' . $area_location['area_location'] . ' (' . $lobby['lobby_name'] . ')' : 'ER'; # Added by Gervie 02/21/2016
						}
						elseif ($zeile['encounter_type']==2){
					...
					elseif($zeile['encounter_type'] == 13){
						//for Ipbm
						$admission_date = date("m/d/Y",strtotime($zeile['encounter_date']));
						$location = "IPBM-IPD";
					}
					elseif($zeile['encounter_type'] == 14){
						//for Ipbm
						$admission_date = date("m/d/Y",strtotime($zeile['encounter_date']));
						$location = "IPBM-OPD";
					}
					...
					*/
					...
					$this->smarty->assign('sOptions','<img type="image" name="select-enc" id="select-enc" '.createLDImgSrc($root_path,'detail.gif','0').' title="'.$LDShowDetails.'" border="0" style="cursor:pointer;"
										...
										OLiframeContent(\''.$root_path.'modules/registration_admission/seg-patient-history.php?pid='.$zeile['pid'].$IPBMextend.'\', 800, 400, \'fSelEnc\', 1, \'auto\'),
                    ...
                    $hasSocialService=$objSS->getSocServPatientThroughEncounter($zeile['encounter_nr']);
                    $originalView='<img type="image" name="select-enc" id="select-enc" '.createLDImgSrc($root_path,'information.png','0').' title="View Information" border="0" style="cursor:pointer;"
                                        onclick="return overlib(
                                        OLiframeContent(\''.$root_path.'modules/registration_admission/seg-patient-info.php?enctype='.$zeile['encounter_type'].'&pid='.$zeile['pid'].'&encounter_nr='.$zeile['encounter_nr'].$IPBMextend.'\', 820, 490, \'fSelEnc\', 1, \'auto\'),
                                    ...
                                    onmouseout="nd();">';
                    $PDPUremarker='<img type="image" name="select-enc" id="select-enc" '.createLDImgSrc($root_path,'page.png','0').' title="Print Assessment" border="0" style="cursor:pointer;"
                                        onclick="window.open(\''.$root_path.'modules/reports/reports/PDPU_Assessment_Tool.php?enc_nr='.$zeile['encounter_nr'].$IPBMextend.'\');"
                                    onmouseout="nd();">';
                    if($_GET['isPDPU']==1 && $hasSocialService) $this->smarty->assign('sOptions2',$originalView.$PDPUremarker);
                    else $this->smarty->assign('sOptions2',$originalView);
