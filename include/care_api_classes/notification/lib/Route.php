...
include __DIR__ . '/../NotificationEventmanager.php';
class Route
{
    ...
    {
        $this->_CONFIG = include __DIR__ . '/notification-config.php';
        $this->_extractURI();
        $this->_extractHeader();
        $this->_IS_Notification = $this->isNotificationReq();
        $this->_initInputs();
        $this->initDB();
    }
    ...
    private function _extractURI(){
        $path = explode($this->_CONFIG['Notification_PATH'],$_SERVER['REQUEST_URI']); // get URI
    ...
    public function isNotificationReq(){
        return  $this->getHeader('REQ') == 'Notification';
    }
    ...
    public function getEvent(){
        if(self::$eventObj == null){
            self::$eventObj = new NotificationEventmanager();
        }
