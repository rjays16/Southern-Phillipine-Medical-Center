...
class Product extends Core {
	...
	*/
	...
	function getAllPharmaAreas() {
		global $db;
			$this->sql="SELECT  *  FROM seg_pharma_areas WHERE is_deleted='0'";
			return $db->Execute($this->sql);	
	}

	function getAllPharmaAreasWithInvKey() {
		global $db;
		$this->usePharmaAreas();

		$this->sql="SELECT * FROM $this->coretable WHERE inv_api_key IS NOT NULL ORDER BY area_name";
        ...
        //addedByMark 06-05-2016
	function selectFromAreaBB() {
			global $db;
			$this->sql="SELECT  *  FROM seg_pharma_areas WHERE area_code ='BB' AND is_deleted='0'";
			return $db->Execute($this->sql);		
	}
	...
	function getAllChargeType()
{
	global $db;
			...
			$this->sql = "SELECT id,charge_name FROM seg_type_charge WHERE in_pharmacy = 1";
			return $db->Execute($this->sql);	
}
}
?>
