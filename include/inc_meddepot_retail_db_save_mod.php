 <?php
 # burn: this file was created on August 8, 2006
/*------begin------ This protection code was suggested by Luki R. luki@karet.org ---- */
if (eregi('inc_meddepot_retail_db_save_mod.php',$PHP_SELF)) 
	die('<meta http-equiv="refresh" content="0; url=../">');
/*------end------*/

//if(isset($cat)&&($cat=='med'))
	$dbtable='seg_med_retail';
...
/*    $bestellnum=trim($bestellnum);
	...
	*/
    ...
    if(!$update){	
		...
		if($med_obj->TransactionExists($refno)){
			$error='refno_exists';
			$refno='';
		}
	}
		...
		// save the uploaded picture
			...
			if(!($update)){
			  $data=array(
					...
					'modify_dt'=>date('YmdHis'),   # burn added: August 10, 2006; modified: Sept 11, 2006
					'modify_id'=>$_SESSION['sess_temp_userid'],   # burn added: August 9, 2006
//					'create_id'=>$HTTP_SESSION_VARS['sess_user_name'],
					'create_dt'=>date('YmdHis')   # burn comment c/o alvin: August 9, 2006
				);
//				$objResponse->addAlert($data);				
				# Set core to main products
				$med_obj->useMedRetail();
				$med_obj->setDataArray($data);
				$saveok=$med_obj->insertDataFromInternalArray();
				$oktosql=false;
				//print("iscash:".$is_cash."<br>");

				if (!saveok) print "no save<p>".$sql."<p>$LDDbNoSave:".$med_obj->sql;
			}else{
					 	$updateok=true;                                     
						if($med_obj->UpdateMedTransaction($refnoex, $refno, date("Y-m-d",strtotime(formatDate2STD($purchasedt, $date_format))), $pencnum, $is_cash?1:0, $_SESSION['sess_temp_userid']))   # burn modified/added: August 9, 2006
//						if($med_obj->UpdateMedTransaction($refnoex, $refno, $purchasedt, $pencnum, $is_cash?1:0, $HTTP_SESSION_VARS['sess_user_name']))
							$saveok=true;
						else{print "no save<p>".$sql."<p>$LDDbNoSave:".$med_obj->sql;};
