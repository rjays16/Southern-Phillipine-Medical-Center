...
function formatDate2Local($stdDate, $localFormat, $retTime=FALSE, $timeOnly=FALSE, &$sepChars)
{
   ...
   if(strchr($stdDate,':'))
   {
	  ...
	  if($timeOnly) return convert24HourTo12HourLocal($stdTime); /* If time only is needed */
   }
...
#   if($retTime) return implode($localSeparator,$localArray).' '.$stdTime;   # burn commented: March 28, 2007
   if($retTime) return implode($localSeparator,$localArray).' '.convert24HourTo12HourLocal($stdTime);   # burn added: March 28, 2007
...
function convertTimeToLocal($time_val)
{
   ...
   return substr($time_val,0,strrpos($time_val,':'));
	...
	/*
	*	convert24HourTo12HourStandard() will return a time in the format HH:mm:ss AM/PM
	*	param $time_val = the time value to be converted
	*	return = the time in the format HH:mm:ss AM/PM
	*	burn added: March 28, 2007	
	*/
function convert24HourTo12HourStandard($time_val){

	$time_val = convertTimeToStandard($time_val);
	$time = explode(":", $time_val);
	$hours = $time[0];
	if ($hours>=12){
		if ($hours==12)
			return $hours.":".$time[1].":".$time[2]." PM";
		else
			return ($hours-12).":".$time[1].":".$time[2]." PM";
	}else{
		if ($time[0]=='00')
			$time_val = "12:".$time[1].":".$time[2]." AM";
		else
			$time_val = $time[0].":".$time[1].":".$time[2]." AM";
		return $time_val;
	}
}

	/*
	*	convert24HourTo12HourStandard() will return a time in the format HH:mm AM/PM
	*	param $time_val = the time value to be converted
	*	return = the time in the format HH:mm AM/PM
	*	burn added: March 28, 2007	
	*/
function convert24HourTo12HourLocal($time_val){

	$time_val = convertTimeToLocal($time_val);
	$time = explode(":", $time_val);
	$hours = $time[0];
	if ($hours>=12){
		if ($hours==12)
		...
		if ($time[0]=='00')
			$time_val = "12:".$time[1]." AM";
		else
			$time_val = $time[0].":".$time[1]." AM";
		return $time_val;
	}
}
