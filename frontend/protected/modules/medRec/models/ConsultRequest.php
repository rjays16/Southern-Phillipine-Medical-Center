...
class ConsultRequest extends CareActiveRecord
{
     ...
     * 
     */
    public function getTriagedOnlineRequests(){
        $criteria = new CDbCriteria();
...
//        $all_access = ($_a_1_opdonlinerequest && !($_a_2_opdonlineregister || $_a_2_opdonlinecreateconsult ));
        $all_access = ($_a_2_opdonlineregister || $_a_2_opdonlinecreateconsult);

        if($_GET['filter'] == 'with_consult'){
            $criteria->addCondition('encounter_nr IS NOT NULL');
            $criteria->order = "scm.create_dt ASC";
        }else {
            if(!$all_access){
                    if($_a_2_opdonlineregister && !$_a_2_opdonlinecreateconsult){
                        $criteria->addCondition('t.pid IS NULL');   
                    }
                     if(!$_a_2_opdonlineregister && $_a_2_opdonlinecreateconsult){
                        $criteria->addCondition('t.pid IS NOT NULL');   
                    }
...
//        $criteria->addCondition('DATE(t.create_dt)="'.date("Y-m-d").'"');
        ...
        $criteria->addCondition("t.request_status = '".self::TRIAGED."'");
