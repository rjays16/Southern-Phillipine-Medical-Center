 ...
 * @property HospEod[] $hospEods
 * @property HospInventory[] $hospInventories
 * @property OrRequest[] $orRequests
 * @property OrSchedulerDeckingLimit $orSchedulerDeckingLimit
 * @property Reptbl[] $reptbls
 */
class CareDepartment extends CareActiveRecord
{
    /**
    ...
    public function getAllOPDepartment(){
        $criteria=new CDbCriteria;

        $criteria->addCondition("is_inactive = 0 ");
        $criteria->addCondition("type = '1' ");
        $criteria->addCondition("admit_outpatient = 1 ");
        $criteria->addNotInCondition("status",array('deleted','hidden','invalid','void'));

        $criteria->order = "name_formal ASC";

        return $this->findAll($criteria);
    }

    public function getSubDepartment($dept_nr){
