...
class CarePerson extends CareActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            array('pid, date_reg, name_first, name_last,religion,sex, history', 'required'),
            ...
            array('employer, mother_fname, mother_maidenname, mother_mname, mother_lname, father_fname, father_mname, father_lname, spouse_name, guardian_name, modify_id, create_id', 'length', 'max' => 50),
            array('phone_1_nr, phone_2_nr, cellphone_1_nr, cellphone_2_nr, fax, civil_status', 'length', 'max' => 35),
    ...
    public function relations()
    {
        ...
        return array(
            ...
            'work'                    => array(self::BELONGS_TO,'Occupation','occupation'),
            'country'                 => array(self::BELONGS_TO,'AddressCountry',array('country_code'=>'citizenship')),
            'religiondata'            => array(self::BELONGS_TO, 'Religion', 'religion')
        );
    }
    ...
    public function search()
    {
        ...
        $criteria->compare('custom_middle_initial', $this->custom_middle_initial, true);
        ...
        $criteria->compare('death_encounter_nr', $this->death_encounter_nr, true);
    ...
    public function getPersonAll()
    {
        ...
        $persons = $this->findAll($criteria);

        return $persons;
    }

    public function SearchInfo($lname, $fname, $mname)
    {
        $criteria         = new CDbCriteria();
        $criteria->select = array("t.*");
        // $criteria->addCondition('name_first = "'.$_GET['fname'].'"');
        $criteria->addCondition('name_first = "' . $_GET['fname'] . '"');
        $criteria->addCondition('name_middle = "' . $_GET['mname'] . '"');
        $criteria->addCondition('name_last = "' . $_GET['lname'] . '"');
    ...
    public function getPersonInformation()
    {
        ...
        $criteria = new CDbCriteria();
        $criteria->addCondition('pid = "' . $_GET['pid'] . '"');
        $persons = $this->find($criteria);
        return $persons;
    }

    public function getPersonDetails($details)
    {
        $criteria = new CDbCriteria();
        $criteria->addCondition('t.name_last = "' . $details['name_last'] . '"');
        $criteria->addCondition('t.name_first = "' . $details['name_first'] . '"');
        $criteria->addCondition('t.name_middle = "' . $details['name_middle'] . '"');

        $criteria = $this->findAll($criteria);

        return $criteria;
    }

    public function isPHS($hrn){

        $queryDEP = " SELECT fn_get_person_lastname_first(parent_pid) as parent_pid from seg_dependents WHERE dependent_pid=".$hrn." AND status IN('member')";

        $isDependent = \Yii::app()->db->createCommand($queryDEP);

        $isDep = $isDependent->queryScalar();


        if($isDep){
                $msg = "Dependent";
        }
        $condition = ' AND status NOT IN("deleted","expired")';
        $queryPHS = " SELECT pid from care_personell WHERE pid='".$hrn."'";
        $isPHSEmployee= \Yii::app()->db->createCommand($queryPHS.$condition);

        $isPHS = $isPHSEmployee->queryScalar();

        if($isPHS){
            $msg = "Active PHS";
        }else{
            $condition = ' AND status IN("deleted","expired")';
            $isPHSEmployee= \Yii::app()->db->createCommand($queryPHS.$condition);
            $notPHS = $isPHSEmployee->queryScalar();
            if($notPHS){
                 $msg = "Inactive PHS";
            }
