...
if(($_a_2_opdonlinerequest && !($_a_2_opdonlineregister)  ||  $_a_2_opdonlinecreateconsult)){
    $access_consult = true;
...
$("#btn_createconsult").on('click', function(){
   ...
   $('#btn_assign').on('click',function(){
        ...
        var pid = $('#pid').val();
         ...
         var baseUrl = loc.protocol + "//"+ loc.host + "/" + loc.pathname.split('/')[1]+'/index.php?r=onlineConsult/online';
              Alerts.confirm({
            title: "Are you sure you want to assign to this patient ?",
            content: "Details will be saved as assign patient",
            callback: function(result) {
                if(result){
                    $.ajax({
...
<div class="container-fluid">
    ...
    <div class="span7">
        <div class="row-fluid">
            ...
            <div class="span6">
                <?php 
                if($access_consult && $is_assign == $personInfo->pid){  $this->widget(
                    'bootstrap.widgets.TbButton',
                    array(
...
$this->widget(
    ...
    array(
        'label'       => 'Update',
        'type'        => 'info',
        'url'         =>  Yii::app()->createUrl("onlineConsult/online/updateInfoPatient",array('pid'=> $personInfo->pid,'consultId'=>$consultId)),
        'disabled'    => '',
