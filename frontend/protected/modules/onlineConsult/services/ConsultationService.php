...
class ConsultationService
{
     ...
     * 
     */
    public function updatePersonnelChatId($personell_nr, $fb_userid)
    {
        try {
            $command = \Yii::app()->db->createCommand(
                "UPDATE care_personell 
                    SET fb_userid = :fb_userid WHERE nr = :personnel_id"
            );
            $command->bindValue(':personnel_id', $personell_nr);
            $command->bindValue(':fb_userid', $fb_userid);        
            $command->execute();
        } catch (\Exception $e) {
            throw new \Exception('Unable to update chat id in HIS!');
        }

        // --------------------------------
        try {
            $command = \Yii::app()->getComponent('ehrDb')->createCommand(
                "UPDATE smed_personnel_catalog 
                            SET fb_userid = :fb_userid, modify_dt = :modified_dt WHERE personnel_id = :personnel_id");
            ...
            throw new \Exception('Unable to update chat id in EHR!');
        }        
    }
     ...
     */
    public function getConsultRequestCount() {
        $date_today = date('Y-m-d');        
        $nRequests = ConsultRequest::model()->count("(request_status IS NULL OR request_status = '') AND (create_dt BETWEEN :startDt AND :endDt)",
                                    array(':startDt' => $date_today.' 00:00:00',
     ...
     * 
     */
    public function isOnlineConsultTriage() {
        require_once($root_path . 'include/care_api_classes/class_acl.php');
        $objAcl = new \Acl(\Yii::app()->SESSION['sess_temp_userid']);
   
        return $objAcl->checkPermissionRaw('_a_2_opdonlinerequest');
    }
     ...
     * 
     */
    public function isBlockedConsultRequest($consult_id)
    {
        $notify = NotificationActiveResource::instance();
        $status = $notify->isOngoingTriage($consult_id);
