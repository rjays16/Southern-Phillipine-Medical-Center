 ...
 * @property integer $is_in_inventory
 * @property integer $is_fs
 * @property integer $is_restrictAntibio
 * @property string $drug_code
 *
 * The followings are the available model relations:
 * @property BloodComponent[] $segBloodComponents
 * @property DeliveryDetails[] $deliveryDetails
 * @property EodInventory[] $eodInventories
 * @property EquipmentOrders[] $segEquipmentOrders
 * @property ExpiryInventory[] $expiryInventories
 * @property InternalRequestDetails[] $internalRequestDetails
 * @property Inventory[] $inventories
 * @property InventoryAdjustmentDetails[] $inventoryAdjustmentDetails
 * @property IssuanceDetails[] $issuanceDetails
 * @property ItemExtended $itemExtended
 * @property MorePhorderDetails[] $morePhorderDetails
 * @property OrSponge[] $orSponges
 * @property PharmaOrders[] $segPharmaOrders
 * @property PharmaPrices $pharmaPrices
 * @property PharmaProductsAvailability[] $pharmaProductsAvailabilities
 * @property ProductClassification[] $segProductClassifications
 * @property PharmaRdetails[] $pharmaRdetails
 * @property PharmaReturnItems[] $pharmaReturnItems
 * @property RequestsServed[] $requestsServeds
 */
class CarePharmaProductsMain extends CareActiveRecord
{
    /**
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('minorder, maxorder, lock_flag, is_socialized, is_restricted, classification, is_deleted, category_id, is_in_inventory, is_fs, is_restrictAntibio', 'numerical', 'integerOnly'=>true),
            array('unit, bestellnum', 'length', 'max'=>12),
            array('item_name, item_code, barcode, drug_code', 'length', 'max'=>50),
            array('prod_class', 'length', 'max'=>1),
            array('status', 'length', 'max'=>20),
            array('price_cost, price_cash, price_charge, ref_source', 'length', 'max'=>10),
            array('modify_id, create_id', 'length', 'max'=>35),
            array('create_time, remarks', 'safe'),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('unit, item_name, item_code, barcode, bestellnum, artikelnum, industrynum, artikelname, generic, description, packing, prod_class, minorder, maxorder, proorder, picfile, encoder, enc_date, enc_time, lock_flag, is_socialized, is_restricted, classification, medgroup, cave, status, price_cost, price_cash, price_charge, history, modify_id, modify_time, create_id, create_time, ref_source, is_deleted, category_id, remarks, is_in_inventory, is_fs, is_restrictAntibio, drug_code', 'safe', 'on'=>'search'),
        );
    }
    ...
    public function relations()
    {
        ...
        return array(
            'segBloodComponents' => array(self::MANY_MANY, 'BloodComponent', 'seg_blood_products_item(item_code, service_code)'),
            'deliveryDetails' => array(self::HAS_MANY, 'DeliveryDetails', 'item_code'),
            'eodInventories' => array(self::HAS_MANY, 'EodInventory', 'item_code'),
            'segEquipmentOrders' => array(self::MANY_MANY, 'EquipmentOrders', 'seg_equipment_order_items(equipment_id, refno)'),
            'expiryInventories' => array(self::HAS_MANY, 'ExpiryInventory', 'item_code'),
            'internalRequestDetails' => array(self::HAS_MANY, 'InternalRequestDetails', 'item_code'),
            'inventories' => array(self::HAS_MANY, 'Inventory', 'item_code'),
            'inventoryAdjustmentDetails' => array(self::HAS_MANY, 'InventoryAdjustmentDetails', 'item_code'),
            'issuanceDetails' => array(self::HAS_MANY, 'IssuanceDetails', 'item_code'),
            'itemExtended' => array(self::HAS_ONE, 'ItemExtended', 'item_code'),
            'morePhorderDetails' => array(self::HAS_MANY, 'MorePhorderDetails', 'bestellnum'),
            'orSponges' => array(self::HAS_MANY, 'OrSponge', 'sponge_code'),
            'segPharmaOrders' => array(self::MANY_MANY, 'PharmaOrders', 'seg_pharma_order_items(bestellnum, refno)'),
            'pharmaPrices' => array(self::HAS_ONE, 'PharmaPrices', 'bestellnum'),
            'pharmaProductsAvailabilities' => array(self::HAS_MANY, 'PharmaProductsAvailability', 'bestellnum'),
            'segProductClassifications' => array(self::MANY_MANY, 'ProductClassification', 'seg_pharma_products_classification(bestellnum, class_code)'),
            'pharmaRdetails' => array(self::HAS_MANY, 'PharmaRdetails', 'bestellnum'),
            'pharmaReturnItems' => array(self::HAS_MANY, 'PharmaReturnItems', 'bestellnum'),
            'requestsServeds' => array(self::HAS_MANY, 'RequestsServed', 'item_code'),
        );
    }
    ...
    public function attributeLabels()
    {
        return array(
            ...
            'remarks' => 'Remarks',
            'is_in_inventory' => 'Is In Inventory',
            'is_fs' => 'Is Fs',
            'is_restrictAntibio' => 'Is Restrict Antibio',
            'drug_code' => 'Drug Code',
        );
    }
    ...
    public function search()
    {
        ...
        $criteria->compare('is_in_inventory',$this->is_in_inventory);
        $criteria->compare('is_fs',$this->is_fs);
        $criteria->compare('is_restrictAntibio',$this->is_restrictAntibio);
     ...
     * @return CarePharmaProductsMain the static model class
     */
    ...
    public function getMedsByName(){
        ...
        $criteria->addCondition('prod_class="M"');
        $model =  $this->findAll($criteria);
        return $model;
    }

    public function  searchPharmaItem($name, $limit=20){
        $criteria = new CDbCriteria();
        ...
        $criteria->addCondition('is_deleted=0');
        $criteria->addCondition('prod_class="M"');
        $criteria->addCondition('gen_code LIKE :name');
