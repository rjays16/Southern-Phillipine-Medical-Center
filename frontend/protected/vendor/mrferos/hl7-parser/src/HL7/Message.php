...
class Message {
    ...
    const START_OF_BLOCK = "\x0B";
    const END_OF_BLOCK = "\x1C";
    const END_OF_MSG = "\x0D";
    ...
    public function __construct($msgStr = "", $hl7Globals = array())
    {
        ...
        if ($msgStr) {
            ...
            for ($i = 0; $i < count($segments); $i++) {
                $segments[$i] = preg_replace("/".self::START_OF_BLOCK."/", '', $segments[$i]);
                $segments[$i] = preg_replace("/".self::END_OF_BLOCK."/", '', $segments[$i]);
                $segments[$i] = preg_replace("/".self::END_OF_MSG."/", '', $segments[$i]);                
    ...
    public function toString($pretty = False)
    {
        ...
        $msg = self::START_OF_BLOCK.$msg.self::END_OF_BLOCK.self::END_OF_MSG;        
        return $msg;
    }
