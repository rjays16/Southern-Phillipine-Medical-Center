...
class Calendar {
	/** CONFIG **/
		...
		// Flag to deactivate "future" link 
		var $deactivate_future=0;
		// future date color
		var $strFutureDayColor='gray';
		
	/* Function to set the activate "future" day */
...
--->
		";
		echo "
			<!--<TABLE $calWidth border=\"" . $this -> intCalBorder . "\" cellpadding=\"1\" cellspacing=\"0\" >-->
			<TABLE width=\"5%\" border=\"" . $this -> intCalBorder . "\" cellpadding=\"1\" cellspacing=\"0\" >
				<TR>
					<TD>
						<TABLE width=\"100%\" bgcolor=\"" . $this -> strCalColor . "\" border=\"" . $this -> intNavBorder . "\" cellpadding=\"1\" cellspacing=\"1\">
							...
							<TR bgcolor=\"" . $this -> strNavColor . "\">
								...
								<TD align=\"center\" nowrap=\"nowrap\" width=\"3%\"><a href=\"$PHP_SELF".URL_REDIRECT_APPEND."&currYear=$prevYear&currMonth=$prevMonth'\"><img src=\"images/backarrow.gif\" border=\"0\" align=\"absmiddle\"></TD>
								<TD align=\"center\" nowrap=\"nowrap\">
									<SELECT style=\"font:Arial, Helvetica, sans-serif; font-size:10px\" name=\"currMonth\" onChange=\"location.href = '$PHP_SELF".URL_REDIRECT_APPEND."&currYear=' + document.CalOptions.currYear.value + '&currMonth=' + this.value;\">
		";
		for ($m = 1; $m <= 12; $m++) {
			if ($intMonth == $m) {
...
*/			
			echo "
										<OPTION style=\"font-family:Arial, Helvetica, sans-serif; font-size:10px\" value=\"" . sprintf ("%02d", $m) . "\" $statusMonth>" . $LDMonthFullName[$m]; // modified by ELpidio Latorilla 2003-04-05
			;
	}
		echo "						</SELECT>
									<SELECT style=\"font:Arial, Helvetica, sans-serif; font-size:10px\" name=\"currYear\" onChange=\"location.href = '$PHP_SELF".URL_REDIRECT_APPEND."&currYear=' + this.value + '&currMonth=' +  + document.CalOptions.currMonth.value;\">
		";
		for ($y = date ("Y") - 6; $y <= date ("Y") + 10; $y++) {
			...
			echo "
										<OPTION style=\"font-family:Arial, Helvetica, sans-serif; font-size:10px\" value=\"$y\" $statusYear>$y"
			;
		}
		echo "					</TD>
								...
								<TD align=\"center\" nowrap=\"nowrap\" width=\"3%\"><a href=\"$PHP_SELF".URL_REDIRECT_APPEND."&currYear=$nextYear&currMonth=$nextMonth'\"><img src=\"images/nextarrow.gif\" border=\"0\" align=\"absmiddle\"></TD>
							</TR>
		...
		for ($i = 1; $i <= $intDaysInMonth; $i++) {
			...
			#echo "id = ".$_SESSION['datesel'];
			if ($_SESSION['datesel']){
				$intDay = date('d',strtotime($_GET['date']));
			}	
...
*/			
			...
			#echo "day = ".$DayPrepend." - ".$i." - ".$DayAppend;
			if($this->deactivate_future&&("$intYear-$intMonth-$i">date('Y-m-d'))){
				echo "
								<TD align=\"center\" bgcolor=\"$currBGColor\"><font size=1 face=\"verdana,arial\" color=\"$this->strFutureDayColor\"><p>$DayPrepend$i$DayAppend</p></font></TD>
				";
			}else{ 
				echo "
								<TD align=\"center\" bgcolor=\"$currBGColor\"><font size=1 face=\"verdana,arial\"><A href=\"$PHP_SELF".URL_APPEND."&currYear=$intYear&currMonth=$intMonth&currDay=$i&dept_nr=$dept_nr&aux=$aux\"><p>$DayPrepend$i$DayAppend</p></A></font></TD>
				";
			}
