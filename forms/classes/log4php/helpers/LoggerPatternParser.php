...
class LoggerPatternParser {
	...
	/**
	* @author Alvin Quinones (06-18-2010)
	*/
	const MULTILINE_MESSAGE_CONVERTER = 2010;
	...
	public function finalizeConverter($c) {
		...
		switch($c) {
			...
			case 'C':
				/**
				* LoggerClassNamePatternConverter is just returning the fqcn of the Logger class, which is probably
				* not the behavior we want for this pattern token. Instead we use LoggerLocationPatternConverter
				* to get the full name of the class which logs the error.
				* 
				* @todo Test this behavior modification for instances when logging outside of a class
				* @author Alvin Quinones (06-18-2010)
				*/
				...
				// New code:
				$pc = new LoggerLocationPatternConverter($this->formattingInfo, self::CLASS_LOCATION_CONVERTER);
				$this->currentLiteral = '';
				break;
			case 'd':
			...
			*/
			case 'J':
				$pc = new LoggerBasicPatternConverter($this->formattingInfo, self::MULTILINE_MESSAGE_CONVERTER);
				$this->currentLiteral = '';
				break;
			case 'm':
