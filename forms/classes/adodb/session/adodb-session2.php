	...
	*/
	...
	global $ADODB_SESSION_READONLY;
		...
		if ($crc !== false && $crc == (strlen($val) . crc32($val))) {
			...
			$sql = "SELECT * FROM $table WHERE $binary sesskey = '$key' AND expiry >= $sysTimeStamp AND get_lock('sess_lock', 10) FOR UPDATE";
			if ( $conn->Execute($sql) ) {
				...
				$sql = "select RELEASE_LOCK('sess_lock')";
				$conn->Execute($sql);
			}
			return true;
		}
		...
		if (!$clob) {	// no lobs, simply use replace()
			...
			if ($rs && reset($rs->fields) > 0) {
				$sql = "SELECT * FROM $table WHERE sesskey = '$key' AND get_lock('sess_lock', 10) FOR UPDATE";
				$rs = $conn->Execute($sql);
				if ($rs) {
