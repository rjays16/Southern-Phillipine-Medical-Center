...
#	$ok =& $conn->Execute('UPDATE '. ADODB_Session::table(). ' SET sesskey='. $conn->qstr($new_id). ' WHERE sesskey='.$conn->qstr($old_id));
	$ok =& $conn->CacheExecute('UPDATE '. ADODB_Session::table(). ' SET sesskey='. $conn->qstr($new_id). ' WHERE sesskey='.$conn->qstr($old_id));
...
#		assert('$host');
		...
		# Added by LST for session handling using memcache.
		$conn->memCache = true;
		#$conn->memCacheHost = array('192.168.2.121','192.168.2.122'); /// $db->memCacheHost = $ip1; will work too
        $conn->memCacheHost = '192.168.2.122'; /// $db->memCacheHost = $ip1; will work too
	...
	*/
	...
	global $ADODB_SESSION_READONLY;
		...
		if ($crc !== false && $crc == (strlen($val) . crc32($val))) {
			...
			$rs =& $conn->CacheExecute($sql,array($expirevar,$key));
			return true;
		}
		...
		if (!$clob) {	// no lobs, simply use replace()
			...
			$rs =& $conn->CacheExecute("SELECT COUNT(*) AS cnt FROM $table WHERE $binary sesskey = ".$conn->Param(0),array($key));
			if ($rs) $rs->Close();
			...
			$rs =& $conn->CacheExecute($sql,array($val,$expireref,$key));
			...
			$rs =& $conn->CacheExecute($sql,array($expireref,$key));
		...
		if ($expire_notify) {
			...
			if ($rs) {
				...
				while (!$rs->EOF) {
					...
					$del = $conn->CacheExecute("DELETE FROM $table WHERE sesskey=".$conn->Param('0'),array($key));
					$rs->MoveNext();
				}
				$rs->Close();
			...
			if (0) {
				...
				foreach ($arr as $row) {
					...
					$conn->CacheExecute($sql2,array(reset($row)));
				}
			} else {
